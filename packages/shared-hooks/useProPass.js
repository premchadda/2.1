...
export function useProPass() {
  ...
  const isProUser = isAdmin || user?.isProUser || user?.is_pro || user?.is_pro_user || false
  const proPassExpiry = user?.proPassExpiry || user?.pro_expiry || null
  ...
  // Format start date (assuming 1 year before expiry for display)
  const formattedStartDate = useMemo(() => {
    if (!proPassExpiry) return null
    
    const expiryDate = new Date(proPassExpiry)
    // Assume subscription started 1 year before expiry (for yearly plans)
    // In real app, you'd store the start date in DB
