...
function toStagesError(status, message, details, retryAfter = null) {
  const msg = message || `Request failed${status ? ` with status ${status}` : ''}`
...
function toActionableError(err, caller) {
  ...
  const retryAfter =
    err?.retryAfter ?? err?.response?.headers?.['retry-after'] ?? err?.response?.headers?.['Retry-After'] ?? null
...
export function useStages(options = {}) {
  ...
  const fetchStages = useCallback(async (optionsOrSignal = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const fetchStagesWithCategories = useCallback(async (optionsOrSignal = {}) => {
    setLoading(true)
    setError(null)
    try {
      const url = '/stages/with-categories'
      if (!apiClient) warnMissingClient('useStages.fetchStagesWithCategories')
      const signal = resolveSignal(optionsOrSignal)
      const data = await request('GET', url, null, {
        apiClient,
        ...(signal ? { signal } : {}),
  ...
  const fetchStagesWithTestCounts = useCallback(async (optionsOrSignal = {}) => {
    setLoading(true)
    setError(null)
    try {
      const url = '/stages/with-test-counts'
      if (!apiClient) warnMissingClient('useStages.fetchStagesWithTestCounts')
      const signal = resolveSignal(optionsOrSignal)
      const data = await request('GET', url, null, {
        apiClient,
        ...(signal ? { signal } : {}),
  ...
  const fetchStageById = useCallback(async (id, optionsOrSignal = {}) => {
    try {
  ...
  const fetchStageBySlug = useCallback(async (slug, optionsOrSignal = {}) => {
    try {
  ...
  const fetchCategoriesForStage = useCallback(async (stageId, optionsOrSignal = {}) => {
    try {
      if (!apiClient) warnMissingClient('useStages.fetchCategoriesForStage')
      const signal = resolveSignal(optionsOrSignal)
      const data = await request('GET', `/stages/${stageId}/categories`, null, {
        apiClient,
        ...(signal ? { signal } : {}),
      })
      if (data.success) {
  ...
  const fetchCategoryTreeForStage = useCallback(async (stageId, optionsOrSignal = {}) => {
    try {
      if (!apiClient) warnMissingClient('useStages.fetchCategoryTreeForStage')
      const signal = resolveSignal(optionsOrSignal)
      const data = await request('GET', `/stages/${stageId}/categories/tree`, null, {
        apiClient,
        ...(signal ? { signal } : {}),
      })
      if (data.success) {
  ...
  const fetchTestsForStage = useCallback(async (stageId, optionsOrSignal = {}) => {
    try {
  ...
  const fetchStageDetailsAdmin = useCallback(async (stageId, optionsOrSignal = {}) => {
    try {
      if (!apiClient) warnMissingClient('useStages.fetchStageDetailsAdmin')
      const signal = resolveSignal(optionsOrSignal)
      const data = await request('GET', `/admin/stages/${stageId}/details`, null, {
        apiClient,
        ...(signal ? { signal } : {}),
      })
  ...
  // CRUD methods
  const createStage = useCallback(async (stageData, optionsOrSignal = {}) => {
  ...
  const updateStage = useCallback(async (id, stageData, optionsOrSignal = {}) => {
  ...
  const deleteStage = useCallback(async (id, optionsOrSignal = {}) => {
