> **HISTORICAL — re-verified 2026-09-06.** The 3-step flow below describes the original 008-era
> fix. Step 2's old path (`src/database/migrations/008…`) does not exist — use the corrected
> Step 2. Current chain: `apps/backend/src/infrastructure/database/migrations/000→135`
> (124 `.sql` files; `004–017` live in `098_reconstructed_baseline.sql`). Not covered here:
> migrations 102–135. Always run `scripts/run-database-audit.js` before any DDL.
...
```bash
# Historical 008-era path (reference only — this file does NOT exist at src/database/migrations/):
#   docs/legacy-migrations/008-standardize-ids-and-fix-relations.sql
# Its content is incorporated in the active chain via 098_reconstructed_baseline.sql.
# Apply the active chain in numeric order with your migration runner:
...
2. Go to SQL Editor
3. Run pending migrations from `apps/backend/src/infrastructure/database/migrations/` in numeric order
4. Run the migration
...
│   │   ├── 000 → 135  # Active chain (124 .sql files; 004–017 in 098_reconstructed_baseline.sql)
...
├── 008-standardize-ids-and-fix-relations.sql  # Historical fix migration (reference; content in 098)
├── maintenance-scripts.sql                      # Ongoing maintenance
└── README.md / QUICKSTART.md / SCHEMA_DIAGRAM.md  # Historical docs (this set)
```
...
**Re-verified (historical framing + Step 2 path fix)**: 2026-09-06  
**Version**: 1.0  
**Status**: Historical reference (active chain: migrations 000→135) ✅
