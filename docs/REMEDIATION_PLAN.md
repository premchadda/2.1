...
1. **Never edit in dependency order that breaks live.** Phase 1 fixes things that are broken _right now_ (nginx won't boot, console logs leak, etc.) — these are no-regret changes.
...
6. **Migrations are append-only.** New migrations get the next sequential number (124 files spanning 000–135 as of 2026-09-06; was 112 on Aug 23, 093 in Jul 2026). Never edit a shipped migration.
...
- **Fix:**
  1. Removed outer `http { }` wrapper (lines 1 and 78). _(Kept wrapper — mounted as `/etc/nginx/nginx.conf` instead of `conf.d/`)_
...
## Phase 3 — P0: Backend correctness / data-integrity bugs ✅ RESOLVED (re-verified 2026-09-06: results dual-write, junction sync, webhook persistence)
...
## Phase 4 — P0: Frontend correctness bugs ✅ RESOLVED (Checkout/coupon/loop fixes per fix logs)
...
### 4.4 Calculator `Function()` (Calculator.jsx:35-38, CRITICAL) — ✅ DONE

- `Function('"use strict";return (' + sanitized + ')')()` was `eval`-equivalent. Forces CSP `'unsafe-eval'`.
- **Status:** FIXED. The Calculator component has been removed; no `Function()`/`eval()` usage remains in the codebase. Math rendering is now handled by `MathRenderer.jsx` (KaTeX). The nginx CSP still carries `'unsafe-eval'` — once confirmed no transitive dep uses `eval`/`Function`/`new Function`, drop `'unsafe-eval'` from nginx CSP and vercel.json CSP.
...
### 4.7 Dead `localStorage` token reads (CRITICAL × 2) — ✅ DONE
...
- **Status:** FIXED. `WebSocketProvider.jsx` has been deleted. `aiStreaming.js` no longer reads `localStorage` for tokens — it uses cookie auth (`credentials: 'include'`) + CSRF header only, with a SECURITY comment documenting the change. Verified by reading the current files.
...
## Phase 5 — P0: Admin panel component bugs ✅ RESOLVED (admin live-tests router mounted at /admin/live-tests)
...
- **Fix:** In `admin-users.js`, parse all four params and add to the query:
  ...
  if (status === "inactive" || includeInactive === "true")
...
## Phase 6 — High: Code quality / refactor ✅ RESOLVED
...
## Phase 7 — Medium: Database schema fixes ✅ RESOLVED (000→135 on disk; 098/099/100/101/116/120 closures — see DATABASE_SCHEMA_AUDIT.md §7 ledger)
...
## Phase 8 — Medium: Frontend cleanup ✅ RESOLVED
...
## Phase 9 — Medium: Backend cleanup ✅ RESOLVED (live-tests composition mounts; rate-limiter + admin-chain hardening)
...
## Phase 11 — Final rollout 🟡 OPEN (rollout checklist + git-history scrub pending — see addendum 2026-09-06)
...
| Admin panel                 | 8                                                         |
| Database migrations         | 8 (094–101) + 34 (102–135)                                |
| Infrastructure              | 12                                                        |
| Code quality                | 10                                                        |
| Lint/Test fixes             | 512                                                       |
...
| **Total**                   | **~670** (was ~635; +34 migrations 102–135 + doc refresh) |
...
## Addendum 2026-09-06 — waves 17–20, migrations 102–135, workflow closures

**Migrations 102–135 (34 files, all on disk):** attempt_number + reattempt support (102),
bookmarks type fix (103), pgcrypto resilience (104), admin perf indexes (105), **Node Engine
v2 learning graph (106)**, **practice-engine redesign (107)**, baseline completion (108),
assessment forms (109), banners (110), backups/FAQs/soft-delete (111), tag configs +
permission matrix (112), leaderboards config (113), live-test series totals (114),
function search_path + PII permissions (115), **comprehensive RLS backfill (116)**,
helper search_path (117), attempt flags (118), revoked attempts (119),
**`120_create_webhook_events`**, junction guard (121), response-time + option-default +
subject-perf + negative-marking waves (122–126), perf backfills + taxonomy cascades +
cutoffs + slow-endpoint/speed indexes (127–133), audit remediation (134),
test-lifecycle + shuffle seed (135). Regen: `dir "apps\backend\src\infrastructure\database\migrations" /b`.

**Waves 17–20 + avatar:** shipped per `ARCHITECTURE.md` waves log and the SECURITY_POSTURE
fix logs (avatar upload hardening: crop/size validation, Profile/Settings entries).

**Workflow closures (see `docs/audit/D1_WORKFLOW_VERIFICATION.md` re-verified 2026-09-06):**
W1 results dual-write (`attempt.service.js:346-372`) + leaderboard fallback chain; W3 coupon
alias (`payments.js:235-272`) + real Razorpay Checkout with non-prod-gated mock; W4
save-answer persistence + real rank + admin live-tests router. Residuals: achievements
`dbHelpers` camelCase-mapping check; `admin-stats.js:107` legacy fallback; legacy
`test_questions` rows predating the sync helper.
