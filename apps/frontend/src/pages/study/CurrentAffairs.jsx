...
export default function CurrentAffairs() {
  ...
  const fetchArticles = async (signal) => {
    ...
    try {
      ...
      const response = await api.get(`/api/current-affairs?date=${dateStr}`, {
        signal,
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <Helmet>
        ...
        <meta
          name="description"
          content="Stay updated with daily current affairs for competitive exam preparation on Trstprep."
        />
        <meta property="og:title" content="Daily Current Affairs | Trstprep" />
        <meta
          property="og:description"
          content="Stay updated with daily current affairs for competitive exam preparation."
        />
          ...
          <div className="flex items-center gap-3 text-center sm:text-left">
            ...
            <div>
              <h3 className="font-bold text-base">
                Daily Current Affairs Speed Quiz
              </h3>
              <p className="text-xs text-indigo-100 mt-0.5">
                Test your retention with today's 10-question timed quiz & climb
                the leaderboard.
              </p>
            </div>
          </div>
          ...
          <div className="text-center py-12 bg-white rounded-xl">
            ...
            <button
              onClick={() => {
                setLoading(true);
                setError(null);
                fetchArticles(new AbortController().signal);
              }}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredArticles.map((article, idx) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    navigate(`/current-affairs/${article.id || article._id}`)
                  }
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-indigo-100">
                Download the complete current affairs for the month as PDF
              </p>
            </div>
            <p className="text-sm text-indigo-100" aria-live="polite">
              Monthly PDF compilation is not available yet.
            </p>
          </div>
        </div>
      </div>
