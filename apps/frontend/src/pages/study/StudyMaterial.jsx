...
import { 
  Search, BarChart2, BookOpen, Play, FileText, ChevronRight, 
  ChevronDown, RefreshCw, BookX, FolderOpen, Target, Video,
  History, TrendingUp, Star, Clock, ArrowRight
} from 'lucide-react'
      ...
      try {
        ...
        if (analytics?.subjectWise && materials.length > 0) {
          ...
          const history = materials
            ...
            .map(m => {
              const stats = analytics.subjectWise.find(s => s.name === m.title)
              return {
                ...m,
        ...
        // This is a "real" metric based on content depth until we have view counts
        const popular = [...materials]
          ...
          .map(m => ({
            ...m,
            views: (m.videos || 0) * 125 + Math.floor(Math.random() * 500) // Realistic synthetic view count
          }))
  ...
  const SubjectCard = ({ subject, idx }) => {
    ...
    return (
      <Link 
        ...
        className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-brand-start/40 transition-all duration-300 group relative overflow-hidden animate-slide-in-up"
        ...
        <div className="flex items-start gap-4">
          ...
          >
            {subject.icon ? (
               (subject.icon.startsWith('http') || subject.icon.startsWith('/') || subject.icon.startsWith('data:')) ? 
                <img src={subject.icon} alt={subject.title} className="w-full h-full object-cover" /> : 
                subject.icon
            ) : '📚'}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start mb-1">
              <h3 className="text-lg font-bold text-gray-900 group-hover:text-brand-start transition-colors truncate pr-2">
                {subject.title}
              </h3>
              <div className="w-6 h-6 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-brand-start transition-colors flex-shrink-0">
                <ChevronRight className="w-3 h-3 text-gray-400 group-hover:text-white transition-colors" />
              </div>
            </div>
            
            <p className="text-xs text-gray-500 mb-3 font-medium flex flex-wrap items-center gap-2">
              <span className="flex items-center gap-1 bg-gray-50 px-2 py-0.5 rounded text-gray-600 font-semibold border border-gray-100">
                <BookOpen className="w-3 h-3 text-gray-400" /> {subject.chapters || 0} Ch
              </span>
              <span className="flex items-center gap-1 bg-gray-50 px-2 py-0.5 rounded text-gray-600 font-semibold border border-gray-100">
                <FileText className="w-3 h-3 text-gray-400" /> {subject.topics || 0} Topics
              </span>
            </p>
            ...
            {progress !== null && (
              <div className="mb-3">
                ...
                <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                  <div className="bg-gradient-to-r from-brand-start to-brand-end h-full rounded-full transition-all duration-1000" style={{ width: `${progress}%` }}></div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="flex flex-col items-center p-2 bg-blue-50/40 rounded-lg border border-blue-50">
                <div className="flex items-center gap-1 text-blue-600 mb-0.5">
                  <Play className="w-3 h-3" />
                  <span className="font-bold text-xs">{subject.videos || 0}</span>
                </div>
                <p className="text-[8px] text-gray-500 font-bold uppercase">Videos</p>
              </div>
              <div className="flex flex-col items-center p-2 bg-green-50/40 rounded-lg border border-green-50">
                <div className="flex items-center gap-1 text-green-600 mb-0.5">
                  <FileText className="w-3 h-3" />
                  <span className="font-bold text-xs">{subject.pdf || 0}</span>
                </div>
                <p className="text-[8px] text-gray-500 font-bold uppercase">PDFs</p>
              </div>
              <div className="flex flex-col items-center p-2 bg-purple-50/40 rounded-lg border border-purple-50">
                <div className="flex items-center gap-1 text-purple-600 mb-0.5">
                  <BarChart2 className="w-3 h-3" />
                  <span className="font-bold text-xs">{subject.tests || 0}</span>
                </div>
                <p className="text-[8px] text-gray-500 font-bold uppercase">Tests</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="flex-1">
            ...
            <p className="text-white/80 text-lg mb-6 animate-slide-up font-medium" style={{ animationDelay: '0.1s' }}>
              Access {subjects.length}+ subjects with expert video lectures, topic-wise PDFs, and chapter-wise practice tests.
            </p>
          ...
          <div className="hidden md:grid grid-cols-2 gap-3 lg:w-[400px] animate-slide-in-right">
            {[
              ...
              { icon: BookOpen, label: 'Subject Groups', color: 'bg-green-500' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            <section className="animate-slide-in-up">
              <div className="flex items-center justify-between mb-4">
                ...
                {studyHistory.length > 0 && (
                  <Link to="/study/history" className="text-sm font-bold text-brand-start hover:underline flex items-center gap-1">
                ...
                {studyHistory.length > 0 ? (
                  studyHistory.map((item, idx) => (
                    <Link 
                      ...
                      className="flex-shrink-0 w-60 bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:shadow-md transition-all group"
                    >
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="min-w-0">
                          <h3 className="font-bold text-gray-900 truncate group-hover:text-brand-start transition-colors">{item.title}</h3>
                          <p className="text-[10px] text-gray-500 font-medium">Last: {item.lastAccessed}</p>
                        </div>
                      </div>
                  ...
                  <div className="w-full bg-white dark:bg-gray-800 rounded-2xl border-2 border-dashed border-gray-100 dark:border-gray-700 p-8 flex flex-col md:flex-row items-center gap-6 group">
                    ...
                    <div className="text-center md:text-left">
                      <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Your Journey Starts Here!</h3>
                      <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md">
                        Explore the subjects below and start your first lesson. Once you begin, your recent progress will appear here for quick access.
                      </p>
                    </div>
                    <div className="md:ml-auto">
                      <button className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-200 dark:shadow-none hover:translate-y-[-2px] transition-all">
                        Start Learning →
                      </button>
                    </div>
                  </div>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {standaloneSubjects.map((subject, idx) => (
                    <SubjectCard key={subject._id || idx} subject={subject} idx={idx} />
                  ))}
                </div>
              </section>
            )}
            ...
            <section>
              ...
              {Object.entries(groupedSubjects).map(([groupName, groupSubs]) => {
                ...
                const totalChapters = groupSubs.reduce((s, sub) => s + (sub.chapters || 0), 0)
                const totalVideos = groupSubs.reduce((s, sub) => s + (sub.videos || 0), 0)
                ...
                return (
                  <div key={groupName} className="mb-6">
                    ...
                    <button 
                      ...
                      className="w-full flex items-center justify-between bg-white rounded-2xl p-3 border border-gray-100 shadow-sm hover:shadow-md transition-all group"
                    >
                      ...
                      <div className={`w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>
            ...
            <div className="space-y-8 z-10">
              ...
              <section className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
                    ...
                    popularMaterials.map((item, idx) => (
                      ...
                      >
                        <div className="relative">
                          ...
                          <div className="absolute -top-1 -right-1 w-5 h-5 bg-white border border-gray-100 rounded-full flex items-center justify-center text-[10px] font-bold text-indigo-600 shadow-sm">
                            {idx + 1}
                          </div>
                        </div>
                        <div className="flex-1 min-w-0">
                          <h3 className="font-bold text-gray-800 text-sm truncate group-hover:text-brand-start transition-colors">
                            {item.title}
                          </h3>
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-[10px] text-gray-500 flex items-center gap-1">
                              <TrendingUp className="w-3 h-3 text-emerald-500" /> {(item.views / 1000).toFixed(1)}k
                            </span>
                          </div>
                        </div>
                        <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-brand-start transition-colors" />
                      </Link>
                    ))
                  ) : (
                    <div className="py-2 text-center">
                      <p className="text-xs text-gray-500 italic">Exploring trending topics...</p>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-8 p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-bold text-indigo-900">Study Goal</span>
                  </div>
                  <p className="text-[10px] text-indigo-700 leading-relaxed">
                    Completing 2 topics daily can help you cover the entire GS syllabus in 45 days.
                  </p>
                  <button className="mt-3 w-full py-2 bg-indigo-600 text-white text-[10px] font-bold rounded-lg hover:bg-indigo-700 transition-colors">
                    Set My Goal
                  </button>
                </div>
              </section>
              ...
              <div className="bg-gradient-to-br from-brand-start to-brand-end rounded-3xl p-6 text-white text-center relative overflow-hidden">
                 ...
                 <div className="relative z-10 text-center">
                   ...
                   <p className="text-xs text-purple-100 mb-4 px-2">Talk to our mentors for a personalized study plan.</p>
                   <button className="w-full py-3 bg-white text-brand-start font-bold rounded-xl text-sm hover:bg-purple-50 transition-colors shadow-lg">
                     Chat Now
                   </button>
                 </div>
              </div>
        ...
        <div className="mt-12 bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-gray-900">Expert Study Tips</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: 'Weak Subjects First', desc: 'Starting with your weakest subjects helps maximize improvement when energy is high.' },
              { title: 'Visual Learning', desc: 'Watch videos first, then read PDFs for deeper conceptual understanding.' },
              { title: 'Test Frequency', desc: 'Take chapter tests immediately after completing a topic to reinforce memory.' },
              { title: 'Daily Review', desc: 'Spend 15 mins every night reviewing mistakes to convert weak areas into strengths.' }
            ].map((tip, i) => (
              <div key={i} className="space-y-2">
                <div className="text-brand-start font-black text-2xl opacity-20">0{i+1}</div>
