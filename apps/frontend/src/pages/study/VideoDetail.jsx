  ...
  useEffect(() => {
    ...
    if (chapters && currentVideoId) {
      for (const chapter of chapters) {
        ...
        if (
          allVideos.some(
            (v) => (v.publicId || v._id || v.id) === currentVideoId,
          )
        ) {
              ...
              {isExpanded && (
                <div className="bg-gray-50/50">
                  {allVideos.map((video) => {
                    ...
                    const isActive =
                      (video.publicId || video._id || video.id) ===
                      currentVideoId;
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${
                          isActive
                            ? "bg-brand-start/10 border-l-2 border-brand-start"
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-xs font-medium truncate ${isActive ? "text-brand-start" : "text-gray-700"}`}
                          >
                            {video.title}
                          </p>
                          {video.duration && (
              ...
              try {
                ...
                if (hierResponse.data.success) {
                  ...
                  for (const subject of subjects) {
                    ...
                    if (
                      allVids.some(
                        (v) => (v.publicId || v._id || v.id) === resolveId,
                      )
                    ) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center bg-white rounded-2xl p-8 shadow-sm border border-gray-100 max-w-md">
          ...
          <Link
            to="/videos"
            className="inline-flex items-center gap-2 px-6 py-2.5 bg-brand-start text-white font-semibold rounded-lg hover:opacity-90 transition"
          >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <Breadcrumb
            items={[
              ...
              ...(currentChapter
                ? [{ label: currentChapter.title, path: "/videos" }]
                : []),
              { label: video.title },
            ]}
          />
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-emerald-800">
                        Protected by FortSpy
                      </h4>
                      <p className="text-xs text-emerald-700 mt-1">
                        This video is encrypted at the pixel level using AES-256
                        military-grade encryption. Frames are decrypted in
                        real-time during playback for secure viewing.
                      </p>
              ...
              {prevVideo ? (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm font-semibold text-gray-900 truncate group-hover:text-brand-start transition-colors">
                      {prevVideo.title}
                    </p>
                  </div>
                </Link>
