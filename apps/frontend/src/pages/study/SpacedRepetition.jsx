...
function Flashcard({ card, onKnow, onNeedsReview, index }) {
  ...
  const priorityLabel = {
    high: { text: 'High Priority', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' },
    medium: { text: 'Medium Priority', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' },
    low: { text: 'Low Priority', color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' },
  }
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <span className={`px-2.5 py-1 text-[10px] font-bold rounded-full ${priority.color}`}>
                {priority.text}
              </span>
              {card.topic_name && (
            ...
            {card.difficulty && (
              <div className="mt-4 flex items-center justify-center gap-1.5">
                ...
                <span className={`text-[10px] font-bold ${
                  card.difficulty === 'hard' ? 'text-red-500' :
                  card.difficulty === 'medium' ? 'text-amber-500' : 'text-green-500'
                }`}>
                  {String(card.difficulty || '').charAt(0).toUpperCase() + String(card.difficulty || '').slice(1)}
                </span>
              </div>
            )}
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm font-semibold text-indigo-700 dark:text-indigo-400">Answer</span>
            </div>

            <div className="flex-1 flex flex-col justify-center text-center">
              <div className="text-gray-900 dark:text-white text-lg font-medium leading-relaxed">
                <MathRenderer text={card.correct_option || 'No answer available'} />
              </div>
            </div>

            <div className="mt-4">
              <p className="text-xs text-gray-400 dark:text-gray-500 text-center mb-3">Did you know this?</p>
              <div className="flex gap-3">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); handleNeedsReview() }}
                  disabled={answered}
...
function StatsBar({ stats }) {
  ...
  const items = [
    { label: 'Due Now', value: stats.due_now || 0, icon: Clock, color: 'text-red-500' },
    { label: 'High Priority', value: stats.high_priority || 0, icon: AlertTriangle, color: 'text-amber-500' },
    { label: 'In Queue', value: stats.total_in_queue || 0, icon: BookOpen, color: 'text-blue-500' },
  ]

  return (
    <div className="grid grid-cols-3 gap-3">
      {items.map((item) => (
        <div key={item.label} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-3 text-center">
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
      <div className="p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-bold text-gray-900 dark:text-white">AI Revision Plan</h3>
          {plan.model && (
        ...
        {plan.weakAreas && plan.weakAreas.length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Focus Areas</p>
            <div className="flex flex-wrap gap-2">
              {plan.weakAreas.map((area, i) => (
                <span key={i} className="px-2.5 py-1 text-xs font-medium rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">
        ...
        {plan.wrongQuestionsCount > 0 && (
          <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-xl">
            <p className="text-xs text-red-600 dark:text-red-400">
              <span className="font-bold">{plan.wrongQuestionsCount}</span> questions need review
            </p>
          </div>
        )}

        {plan.revisionPlan ? (
          <div className={`text-sm text-gray-700 dark:text-gray-300 leading-relaxed ${expanded ? '' : 'line-clamp-4'}`}>
            {Array.isArray(plan.revisionPlan) ? (
              plan.revisionPlan.map((item, i) => (
                <p key={i} className="whitespace-pre-wrap">{item}</p>
              ))
            ) : (
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">No revision plan yet.</p>
        )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20 md:pb-8 page-transition fade-in">
      <Helmet>
        ...
        <meta name="description" content="Smart revision with spaced repetition - review questions at optimal intervals." />
      </Helmet>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="text-white">
              ...
              <p className="text-purple-100 text-sm mt-1">Review at optimal intervals for lasting memory</p>
            </div>
          </div>
          <button
            ...
            {loading ? (
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-8 text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                  No revisions due right now. Keep practicing to build your revision queue.
                </p>
                <Link
                  to="/practice"
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-8 text-center">
                <div className="text-5xl mb-4">💪</div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Session Complete!</h3>
                <div className="flex justify-center gap-6 my-6">
                  <div className="text-center">
                    <p className="text-3xl font-bold text-green-600 dark:text-green-400">{sessionStats.known}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Known</p>
                  </div>
                  <div className="text-center">
                    <p className="text-3xl font-bold text-amber-600 dark:text-amber-400">{sessionStats.review}</p>
          ...
          <div className="lg:col-span-1">
            <div className="space-y-4">
              ...
              <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800 p-5">
                <h3 className="font-semibold text-indigo-900 dark:text-indigo-200 mb-2 text-sm">How Spaced Repetition Works</h3>
