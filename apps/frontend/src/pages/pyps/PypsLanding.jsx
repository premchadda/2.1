...
function PypsLanding() {
  ...
  const totalExamsAttempts = exams.reduce((s, e) => s + (e.totalAttempts || 0), 0)
    ...
    <div className="min-h-screen bg-gray-50 page-transition fade-in">
      ...
      >
        <div className="flex flex-wrap gap-2 mt-2">
          ...
          <div className="bg-white/20 backdrop-blur-md px-3 py-1.5 rounded-lg text-white text-xs font-bold">
            {categories.length} Exam {categories.length === 1 ? 'Category' : 'Categories'}
          </div>
          ...
          <div className="lg:w-64 flex-shrink-0">
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden sticky top-4">
              <div className="px-4 py-3 border-b border-gray-100">
                <h2 className="text-sm font-bold text-gray-900">Exam Categories</h2>
              </div>
              <div className="max-h-[60vh] lg:max-h-[70vh] overflow-y-auto">
                {categories.map((cat, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-9 h-9 rounded-lg bg-gradient-to-br ${color} flex items-center justify-center text-lg flex-shrink-0`}>
                        {cat.icon || '📋'}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-gray-500">
                          {cat.paperCount} {cat.paperCount === 1 ? 'paper' : 'papers'}
                        </div>
                      </div>
                      {cat.totalAttempts > 0 && (
          ...
          <div className="flex-1 min-w-0">
            {loadingCats ? (
              ...
              <div className="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">Choose a category from the left to browse its exams</p>
              </div>
              ...
              <div className="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">No exams in {selectedCat?.name} yet</p>
              </div>
            ) : (
                  ...
                  <div className="flex items-center justify-between gap-4 px-4 py-3 border-b border-gray-100">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {exams.length} {exams.length === 1 ? 'exam' : 'exams'} · {totalExamsPapers} {totalExamsPapers === 1 ? 'paper' : 'papers'}
                    ...
                    {exams.map((exam) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`w-10 h-10 mx-auto rounded-lg bg-gradient-to-br ${selectedCatColor} flex items-center justify-center text-base mb-1.5 group-hover:scale-110 transition-transform`}>
                            📋
                          </div>
                          <h3 className="text-[11px] font-bold text-gray-900 truncate leading-tight">{exam.title}</h3>
                          <p className="text-[10px] text-cyan-600 mt-0.5 font-semibold">
                            {exam.paperCount} {exam.paperCount === 1 ? 'test' : 'tests'}
                          </p>
                          {exam.yearRange && (
                            <p className="text-[10px] text-gray-400 mt-0.5">{exam.yearRange}</p>
                          )}
                          ...
                          {exam.paperCount === 0 && (
                            <p className="text-[9px] text-gray-400 mt-0.5 italic">Coming soon</p>
                          )}
                        </Link>
                      )
                    })}
