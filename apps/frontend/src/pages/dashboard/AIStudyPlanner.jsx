...
import { Calendar, ChevronLeft, ChevronRight, Brain, Clock, CheckCircle, Target, ArrowLeft, Sparkles, BookOpen, Zap, BarChart3 } from 'lucide-react'
...
function AIStudyPlanner() {
  ...
  const mockSchedule = {
    '2026-06-22': [
      { time: '09:00', title: 'Quantitative Aptitude Practice', duration: '1h', type: 'practice', completed: true },
      { time: '11:00', title: 'Mock Test - Math Section', duration: '2h', type: 'test', completed: false },
      { time: '14:00', title: 'Revision: Number System', duration: '1h', type: 'revision', completed: false },
    ],
    '2026-06-23': [
      { time: '10:00', title: 'Reasoning Practice Set', duration: '1.5h', type: 'practice', completed: false },
      { time: '14:00', title: 'GK Current Affairs', duration: '1h', type: 'study', completed: false },
    ],
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20 md:pb-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-4 relative z-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { icon: Brain, label: 'Study Sessions', value: '12', color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-900/20' },
            { icon: Clock, label: 'Hours This Week', value: '18.5', color: 'text-purple-500', bg: 'bg-purple-50 dark:bg-purple-900/20' },
            { icon: CheckCircle, label: 'Completed', value: '8', color: 'text-green-500', bg: 'bg-green-50 dark:bg-green-900/20' },
            { icon: Target, label: 'Tests This Week', value: '3', color: 'text-orange-500', bg: 'bg-orange-50 dark:bg-orange-900/20' },
          ].map((stat, i) => (
          ...
          <Card variant="gradient" size="lg" className="mb-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white/70 text-sm mt-1">Let AI analyze your performance and create an optimized study plan</p>
              </div>
              <Button variant="primary" size="lg" className="bg-white text-brand-start hover:bg-gray-100">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {schedule.slice(0, 2).map((item, j) => {
                        const config = typeConfig[item.type] || typeConfig.practice
                        return (
                          <div
                            key={j}
                            className={`text-[9px] px-1 py-0.5 rounded ${config.bg} ${config.border} border ${item.completed ? 'opacity-60' : ''}`}
                            title={`${item.title} (${item.time})`}
                          >
                            <div className="flex items-center gap-0.5">
                              {item.completed && <CheckCircle className="w-2 h-2 text-green-500" />}
              ...
              <div className="space-y-3">
                {getDaySchedule(today).map((item, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`flex items-center gap-4 p-3 rounded-xl ${config.bg} border ${config.border} ${item.completed ? 'opacity-70' : ''}`}
                    >
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${config.bg}`}>
                        <config.icon className={`w-5 h-5 ${config.color}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`font-semibold text-sm ${item.completed ? 'line-through text-gray-500' : 'text-gray-900 dark:text-white'}`}>
                          {item.title}
                        </p>
        ...
        <ScrollReveal delay={0.25}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { icon: Target, label: 'Weak Areas', route: '/analysis', color: 'text-orange-500' },
