...
import {
  Trophy, TrendingUp, TrendingDown, Minus, Download, FileSpreadsheet,
  FileText, Filter, Search, ChevronRight, Video, BookOpen, Target,
  Flame, Users, ChevronDown, Medal,
  BarChart3, Play, CheckCircle2
} from "lucide-react";
    ...
    <GlassCard className="p-4 relative overflow-hidden group hover:border-cyan-400/30 transition-colors">
      ...
      <div className="flex items-start justify-between relative">
        <div>
          ...
          <p className="text-2xl font-semibold text-white mt-1" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{value}</p>
      ...
      {trend !== undefined && (
        <div className={`flex items-center gap-1 text-xs mt-2 ${trend > 0 ? "text-emerald-400" : trend < 0 ? "text-rose-400" : "text-slate-400"}`}>
          {trend > 0 ? <TrendingUp size={13} /> : trend < 0 ? <TrendingDown size={13} /> : <Minus size={13} />}
          <span>{trend !== 0 ? `${Math.abs(trend)} this week` : "No change"}</span>
        </div>
      )}
    </GlassCard>
  );
...
export default function UserLeaderboard() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      userAPI.getAnalytics().then((r) => r.data).catch(() => null),
      userAPI.getAttempts().then((r) => (r.data?.data || [])).catch(() => []),
      getTopPerformersLeaderboard(5).then((r) => (r.data?.data || r.data || [])).catch(() => []),
      getIntelligenceLeaderboard({ type: "overall", limit: 200 }).then(
        (r) => (r.data?.entries || r.data || []),
      ).catch(() => []),
    ]).then((results) => {
      ...
      if (analytics) {
        const rank = analytics.rank !== null && analytics.rank !== undefined ? `#${analytics.rank}` : "-"
        const percentile = analytics.percentile !== null && analytics.percentile !== undefined ? analytics.percentile : 0
        const streak = analytics.streak !== null && analytics.streak !== undefined ? analytics.streak : 0
        const examCategory = analytics.strongSubjects?.[0]?.name || "General Prep"
      ...
      if (attempts && attempts.length > 0) {
        const recent = attempts
          ...
          .map((a) => ({
            ...
            timeTaken: a.timeTaken || (a.timeSpent ? `${Math.round(a.timeSpent / 60)}m` : null),
            accuracy:
  ...
  if (loading) {
    return (
      <div className="min-h-screen w-full text-slate-100" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
        <div className="min-h-screen w-full relative" style={{ background: "#060b14" }}>
          <div className="max-w-6xl mx-auto px-5 py-8">
            ...
            <GlassCard className="p-5 mb-6">
              <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Loading Leaderboard...</p>
              <h1 className="text-2xl font-semibold text-white">My Performance & Rankings</h1>
              <p className="text-sm text-slate-400 mt-0.5">Track your tests, practice, and video progress against fellow aspirants</p>
            </GlassCard>
  ...
  if (error) {
    return (
      <div className="min-h-screen w-full text-slate-100" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
        <div className="min-h-screen w-full relative" style={{ background: "#060b14" }}>
          <div className="max-w-6xl mx-auto px-5 py-8">
            <p className="text-center text-rose-400 mb-4">Failed to load leaderboard data</p>
            <button
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              ...
              <div className="flex-1 min-w-0">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-400/30 text-cyan-300 text-xs font-bold uppercase tracking-wider mb-2">
                  <Trophy size={14} className="text-cyan-400" /> Leaderboard & Rankings
                </div>
                <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">My Performance & Rankings</h1>
                <p className="text-sm text-slate-300 mt-1 mb-4 max-w-lg">Track your tests, practice, and video progress against fellow aspirants</p>
              ...
              <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5 shadow-inner flex-shrink-0 lg:max-w-2xl">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-cyan-400 to-teal-500 flex items-center justify-center text-slate-950 font-black text-base shrink-0 shadow-md">
                    {currentUser?.avatar || (currentUser?.name?.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() || 'AU')}
                  </div>
                  <div className="min-w-0">
                    <p className="font-extrabold text-white text-sm truncate">{currentUser?.name || 'Admin User'}</p>
                    <p className="text-xs text-slate-400 font-medium truncate mt-0.5">@{currentUser?.handle || 'admin'} · {currentUser?.examCategory || 'Competitive Exam'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl bg-cyan-400/10 border border-cyan-400/30 flex items-center justify-center text-cyan-300 font-black text-sm shadow-inner" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                    #{currentUser?.overallRank || 1}
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Overall Rank</p>
                    <p className="text-xs font-black text-white whitespace-nowrap">
                      of { (currentUser?.totalUsers || 4).toLocaleString() } aspirants · <span className="text-cyan-300">Top {currentUser?.overallRank && currentUser?.totalUsers ? Math.max(1, Math.min(100, Math.round((currentUser.overallRank / currentUser.totalUsers) * 100))) : 25}%</span>
                    </p>
                  </div>
                </div>
...
<StatCard
  ...
  trend={currentUser.improvement != null ? `${currentUser.improvement > 0 ? '+' : ''}${currentUser.improvement}%` : undefined}
/>
<StatCard
  icon={BookOpen}
...
<StatCard
  ...
  value={currentUser.avgAccuracy != null ? `${Math.round(currentUser.avgAccuracy * 100)}%` : "—%"}
...
/>
            ...
            {!currentUser && (
              <div>
                <StatCard icon={Target} label="Tests Taken" value="0" sub="—" trend={0} />
                <StatCard icon={BookOpen} label="Practice Qs Solved" value="0" sub="—" trend={0} />
                <StatCard icon={Video} label="Videos Watched" value="—" sub="—" trend={0} />
                <StatCard icon={CheckCircle2} label="Overall Accuracy" value="—%" sub="—%" trend={0} />
              </div>
            )}
          </div>
            ...
            {[
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                  activeTab === t.id ? "bg-cyan-400/10 text-cyan-300 border border-cyan-400/30" : "text-slate-400 hover:text-slate-200"
                }`}
              >
              ...
              <div className="grid md:grid-cols-3 gap-5">
                <GlassCard className="p-5 md:col-span-2">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-semibold text-white">Rank Progress (Last 8 Weeks)</h3>
                    <ExportMenu />
                  </div>
                  <ResponsiveContainer width="100%" height={220}>
                    {rankHistory.length > 0 ? (
                      <LineChart data={rankHistory} margin={{ left: -20 }}>
                        ...
                        <YAxis reversed stroke="#64748b" fontSize={11} tickLine={false} axisLine={false} />
                        <Tooltip
                          contentStyle={{ background: "#0b1220", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, fontSize: 12 }}
                          labelStyle={{ color: "#94a3b8" }}
                        />
                        <Line type="monotone" dataKey="rank" stroke="#22d3ee" strokeWidth={2.5} dot={{ fill: "#22d3ee", r: 3 }} />
                      </LineChart>
                    ) : (
                      <p className="text-xs text-slate-500 mt-1">Weekly rank progress will appear here as you complete tests.</p>
                    )}
                  </ResponsiveContainer>
                  {rankHistory.length === 0 && (
                    <p className="text-xs text-slate-500 mt-1">Complete tests to see your rank progression over time.</p>
                  )}
                </GlassCard>

                <GlassCard className="p-5">
                  <h3 className="text-sm font-semibold text-white mb-4">You vs Peer Average</h3>
                  <ResponsiveContainer width="100%" height={220}>
                    {radarData.length > 0 ? (
                      <RadarChart data={radarData}>
                        <PolarGrid stroke="rgba(255,255,255,0.1)" />
                        <PolarAngleAxis dataKey="subject" stroke="#94a3b8" fontSize={10.5} />
                        <Radar name="You" dataKey="you" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.3} />
                        {radarData.length > 0 && (
                          <Radar name="Your Accuracy" dataKey="you" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.15} />
                        )}
                      </RadarChart>
                    ) : (
                      <p className="text-xs text-slate-500 mt-1">Subject data will appear here as you practice.</p>
                    )}
                  </ResponsiveContainer>
                </GlassCard>
                ...
                <GlassCard className="p-5 md:col-span-3">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-semibold text-white flex items-center gap-2"><Users size={15} className="text-cyan-400" /> Nearby Rivals</h3>
                    {nearbyLeaderboard && nearbyLeaderboard.length > 0 ? (
                      <span className="text-xs text-slate-500">Ranks #{Math.max(1, nearbyLeaderboard[0]?.rank)}-#{Math.max(1, nearbyLeaderboard[nearbyLeaderboard.length - 1]?.rank)}</span>
                    ) : (
                      <span className="text-xs text-slate-500">No rival data available yet</span>
                    )}
                  </div>
                  <div className="space-y-1.5">
                    {nearbyLeaderboard && nearbyLeaderboard.length > 0 ? (
                      nearbyLeaderboard.map((u) => (
                        ...
                        >
                          <span className="w-8 text-xs font-mono text-slate-400" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{u.rank}</span>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-medium truncate ${String(u.userId) === String(currentUserId || user?.id || user?._id) ? "text-cyan-300" : "text-slate-200"}`}>
                              {u.name || "Unknown"}{String(u.userId) === String(currentUserId || user?.id || user?._id) && " (You)"}</p>
                            <p className="text-xs text-slate-500">@{u.handle || "unknown"}</p>
                          </div>
                          <div className="text-right hidden sm:block">
                            <p className="text-sm font-medium text-white" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{u.score || "—"}</p>
                            <p className="text-xs text-slate-500">score</p>
                          </div>
                          <div className="text-right hidden sm:block w-16">
                            <p className="text-sm font-medium text-white">{u.accuracy != null ? `${u.accuracy}%` : "—"}%</p>
                      ...
                      <div className="text-center py-6 bg-white/[0.03] dark:bg-gray-800/50 rounded-xl border border-dashed border-gray-200 dark:border-gray-700/60">
                        <p className="text-sm text-slate-400">No nearby rival data available. Complete tests to appear on leaderboards.</p>
                      </div>
                    )}
                  </div>
                </GlassCard>
                      ...
                      {testHistory.length > 0 ? (
                        testHistory.map((t) => (
                          <tr key={t.id} className="border-b border-white/[0.05] hover:bg-white/[0.03]">
                            ...
                            <td className="py-3 font-medium text-white" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{t.score}/{t.maxScore}</td>
                            <td className="py-3">
                              <span className="text-cyan-300 font-medium">{t.rank}</span>
                              <span className="text-slate-500 text-xs"> /{t.totalParticipants.toLocaleString()}</span>
                            </td>
                            <td className="py-3 text-slate-300">{t.percentile}%</td>
                            <td className="py-3">
                              <div className="flex items-center gap-2">
                                <div className="w-16 h-1.5 rounded-full bg-white/10 overflow-hidden">
                                  <div className="h-full bg-gradient-to-r from-cyan-400 to-teal-400" style={{ width: `${t.accuracy != null ? t.accuracy : 0}%` }} />
                                </div>
                                <span className="text-xs text-slate-400">{t.accuracy != null ? `${t.accuracy}%` : "—"}%</span>
                              </div>
                            </td>
                            <td className="py-3 text-slate-400 text-xs">{t.timeTaken || "—"}</td>
<td className="py-3 text-right">
                        ...
                        <tr>
                          <td colSpan="8" className="text-center py-8 text-slate-400">No tests attempted yet. Start practicing to see progress here.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
          ...
          {activeTab === "practice" && (
              <GlassCard className="p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold text-white">Practice Questions — Subject-wise Breakdown</h3>
                  <ExportMenu label="Export Practice" />
                </div>
                <div className="space-y-4">
                  {practiceSubjects.length > 0 ? (
                    practiceSubjects.map((s) => (
                      <div key={s.subject} className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-sm font-medium text-white">{s.subject}</p>
                          <span className="text-xs text-slate-500">{s.solved} solved</span>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <div className="h-2 rounded-full bg-white/10 overflow-hidden relative">
                              <div className="h-full bg-gradient-to-r from-cyan-400 to-teal-400" style={{ width: `${Math.min(100, (s.solved / 100) * 100)}%` }} />
                            </div>
                          </div>
                          <div>
                            <div className="flex justify-between text-xs text-slate-400 mb-1">
                              <span>Accuracy</span>
                              <span className="text-emerald-300">{s.accuracy}%</span>
                            </div>
                            <div className="h-2 rounded-full bg-white/10 overflow-hidden">
                              <div className="h-full bg-gradient-to-r from-emerald-400 to-teal-400" style={{ width: `${s.accuracy}%` }} />
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <p className="text-sm text-slate-500">No practice data yet. Complete practice sessions to track subject progress.</p>
                  )}
                </div>
              </GlassCard>
          )}
          ...
          {activeTab === "videos" && (
              <GlassCard className="p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold text-white">Videos Watched</h3>
                  <ExportMenu label="Export Videos" />
                </div>
                <div className="space-y-2.5">
                  {videosWatched.length > 0 ? (
                    videosWatched.map((v) => (
                      <div key={v.title} className="flex items-center gap-4 p-3.5 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:border-cyan-400/20">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500">{v.topic} · watched {v.watched} of {v.duration} · {v.watchedOn}</p>
                        </div>
                        <div className="w-28 hidden sm:block">
                          <div className="flex justify-between text-xs text-slate-400 mb-1">
                            <span>You</span><span>{v.completion}%</span>
                          </div>
                          <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                            <div className="h-full bg-cyan-400" style={{ width: `${v.completion}%` }} />
                          </div>
                        </div>
                        <div className="w-28 hidden md:block">
                          <div className="flex justify-between text-xs text-slate-500 mb-1">
                            <span>Peer avg</span><span>{v.peerCompletion}%</span>
                          </div>
                          <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                            <div className="h-full bg-slate-500" style={{ width: `${v.peerCompletion}%` }} />
                          </div>
                        </div>
                      </div>
                    ))
                  ) : (
                    <p className="text-sm text-slate-500">Video watch progress is being tracked. Check back later for details.</p>
                  )}
                </div>
              </GlassCard>
          )}
          ...
          {activeTab === "global" && (
              <GlassCard className="p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold text-white">Top Performers — {currentUser?.examCategory || "All Users"}</h3>
                  <ExportMenu label="Export Leaderboard" />
                </div>
                <div className="space-y-1.5 mb-3">
                  {globalLeaderboard.map((u) => (
                    <div key={u.rank} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/[0.03]">
                      <span className="w-7 text-sm font-semibold text-slate-300" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{u.rank}</span>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500">@{u.handle} · {u.tests} tests</p>
                      </div>
                      <p className="text-sm font-medium text-white w-16 text-right" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{u.score}</p>
                      <p className="text-sm text-slate-400 w-16 text-right">{u.accuracy != null ? `${u.accuracy}%` : "—"}%</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-2 space-y-1.5">
                  {nearbyLeaderboard && nearbyLeaderboard.length > 0 ? (
                    nearbyLeaderboard.map((u) => (
                      <div key={u.rank} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl ${String(u.userId) === String(currentUserId || user?.id || user?._id) ? "bg-cyan-400/10 border border-cyan-400/30" : "hover:bg-white/[0.03]"}`}>
                        <span className="w-7 text-sm font-semibold text-slate-300" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{u.rank}</span>
                        <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-xs font-semibold text-slate-200">
                          {u.name?.split(" ").map((n) => n[0]).join("")}
                        </div>
                    ...
                    <div className="text-center py-6 bg-white/[0.03] dark:bg-gray-800/50 rounded-xl border border-dashed border-gray-200 dark:border-gray-700/60">
                      <p className="text-sm text-slate-400">No leaderboard data available yet. Complete tests to appear on global rankings.</p>
                    </div>
                  )}
                </div>
              </GlassCard>
