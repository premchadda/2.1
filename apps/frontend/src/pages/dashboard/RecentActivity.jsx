...
function RecentActivity({ recentActivity = [] }) {
  ...
  const currentItems = recentActivity.slice((safePage - 1) * ITEMS_PER_PAGE, safePage * ITEMS_PER_PAGE)
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transition-all duration-300">
      ...
      <div className="bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-700 p-3.5 flex items-center justify-between">
        ...
        {recentActivity.length > 0 && (
          <RouterLink
            to="/attempted-tests"
            className="text-[11px] font-semibold text-white bg-white/20 hover:bg-white/30 px-2 py-1 rounded-lg backdrop-blur-sm flex items-center gap-1 transition-all group"
          >
            <span>History</span>
...
const accuracy = item.accuracy !== null && Number.isFinite(item.accuracy) ? Math.round(Number(item.accuracy)) : null
  const score = item.score !== null && Number.isFinite(item.score) ? item.score : null
  const percentage = item.percentage !== null && Number.isFinite(item.percentage) ? Number(item.percentage) : accuracy

              // Color badge logic based on score/percentage
              let badgeBg = 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-800/50'
              let scoreColor = 'text-emerald-600 dark:text-emerald-400'

              if (percentage !== null && percentage < 50) {
                badgeBg = 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/40 dark:text-rose-400 dark:border-rose-800/50'
                scoreColor = 'text-rose-600 dark:text-rose-400'
              } else if (percentage !== null && percentage < 75) {
                badgeBg = 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800/50'
                scoreColor = 'text-amber-600 dark:text-amber-400'
              }

              // Clickable URL for card navigation
              const testUrl = (item.seriesId || item.rawAttempt?.seriesId) && (item.testId || item.id || item.rawAttempt?.testId)
              ...
              return (
                <RouterLink
                  key={item.id || index}
                  to={testUrl}
                  className="block group relative bg-gray-50/80 dark:bg-gray-900/50 hover:bg-white dark:hover:bg-gray-700/80 rounded-xl p-2.5 border border-gray-100 dark:border-gray-700/60 hover:border-indigo-300 dark:hover:border-indigo-500/50 shadow-none hover:shadow-sm transition-all duration-200 cursor-pointer"
                >
                  <div className="flex items-center justify-between gap-2.5">
                    ...
                    <div className="flex flex-col items-end shrink-0">
                      {score !== null && (
                        <div className={`px-2 py-0.5 rounded-md border text-[10px] font-bold ${badgeBg} flex items-center gap-1 shadow-xs`}>
                      ...
                      {accuracy !== null && (
                        <span className={`text-[9px] font-medium mt-0.5 ${scoreColor}`}>
                          {accuracy}% Acc
                        </span>
                      )}
                    </div>
