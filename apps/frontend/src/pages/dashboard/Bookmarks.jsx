...
export default function Bookmarks() {
    ...
    if (user) {
      ...
      const loadBookmarks = async () => {
        try {
          ...
          if (err?.name !== 'CanceledError' && err?.code !== 'ERR_CANCELED' && !cancelledRef.current) {
            console.error('Error fetching bookmarks:', err)
            if (!cancelledRef.current) setError('Failed to load saved items. Please try again.')
          }
        } finally {
  ...
  const handleUpdateNote = async (id, newNotes) => {
    try {
      ...
      setBookmarks(prev =>
        prev.map(b => ((b._id === id || b.id === id) ? { ...b, notes: newNotes } : b))
      )
    } catch (err) {
  ...
  const availableSubjects = useMemo(() => {
    ...
    questionBookmarks.forEach(b => {
      const sub = extractText(b.item?.subject || b.subject || b.item?.category || 'General').trim()
      if (sub) set.add(sub)
    })
  ...
  const filteredQuestions = useMemo(() => {
    return questionBookmarks.filter(b => {
      const qText = extractText(b.item?.question_text || b.item?.questionText || b.item?.question || b.title || '').toLowerCase()
      const sub = extractText(b.item?.subject || b.subject || 'General').toLowerCase()
      const top = extractText(b.item?.topic || b.item?.chapter || '').toLowerCase()
      const diff = (b.item?.difficulty || 'medium').toLowerCase()
      const query = searchQuery.trim().toLowerCase()

      const matchesSearch = !query || qText.includes(query) || sub.includes(query) || top.includes(query)
      const matchesSubject = selectedSubject === 'all' || sub === selectedSubject.toLowerCase()
      const matchesDiff = selectedDifficulty === 'all' || diff === selectedDifficulty.toLowerCase()
      const matchesNotes = !hasNotesOnly || Boolean(b.notes && b.notes.trim())

      return matchesSearch && matchesSubject && matchesDiff && matchesNotes
    })
  }, [questionBookmarks, searchQuery, selectedSubject, selectedDifficulty, hasNotesOnly])
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'test': return <FileText className="w-3.5 h-3.5" />
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-gray-900 py-4 sm:py-6">
      <Helmet>
        ...
        <meta name="description" content="Access your saved questions, launch instant active recall revision tests, and track dispute statuses." />
      </Helmet>
      ...
      <div className="container mx-auto px-3 sm:px-6 max-w-5xl space-y-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                  Click any question to view interactive options, active recall testing, verified solutions, and pro shortcuts.
                </p>
              </div>
            </div>
        ...
        {error && (
          <div className="p-3 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 rounded-xl flex items-center justify-between text-xs text-rose-700 dark:text-rose-300">
            ...
            <button onClick={fetchBookmarks} className="font-bold underline hover:opacity-80">
              Retry
            </button>
          </div>
        )}
        ...
        <div className="bg-gray-100 dark:bg-gray-800/90 p-1 rounded-xl flex items-center gap-1">
          ...
          >
            <HelpCircle className={`w-4 h-4 ${activeTab === 'questions' ? 'text-indigo-600 dark:text-indigo-400' : ''}`} />
          ...
          >
            <Bookmark className={`w-4 h-4 ${activeTab === 'other' ? 'text-emerald-600 dark:text-emerald-400' : ''}`} />
          ...
          >
            <Flag className={`w-4 h-4 ${activeTab === 'reported' ? 'fill-amber-500 text-amber-500' : ''}`} />
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-gray-800 p-2.5 sm:p-3.5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-xs space-y-2 sm:space-y-3">
              <div className="flex items-center gap-1.5 sm:gap-2.5 w-full">
                ...
                {availableSubjects.length > 0 && (
                  ...
                  >
                    <option value="all">All Subjects ({questionBookmarks.length})</option>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar py-0.5 text-xs border-t border-gray-100 dark:border-gray-700/60 flex-nowrap sm:flex-wrap">
                <span className="text-[11px] font-bold text-gray-400 shrink-0">Filter:</span>
                
                <button
                  onClick={() => { setSelectedSubject('all'); setSelectedDifficulty('all'); setHasNotesOnly(false); setSearchQuery('') }}
                  className={`px-2.5 py-0.5 rounded-full font-bold text-[11px] shrink-0 transition ${
                    selectedSubject === 'all' && selectedDifficulty === 'all' && !hasNotesOnly && !searchQuery
            ...
            {!user && (
              <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 max-w-sm mx-auto mb-4">
                  Log in to review your bookmarked doubts, test your active recall, and see verified solutions.
                </p>
                <div className="flex justify-center gap-3">
                  <Link to="/login" className="px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-lg hover:bg-indigo-700 transition">
                    Sign In
                  </Link>
                </div>
              </div>
            ...
            {user && loading ? (
              <div className="space-y-2.5">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 animate-pulse h-20" />
                ))}
              </div>
              ...
              <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6">
                ...
                <h3 className="text-base font-bold text-gray-800 dark:text-gray-200 mb-1">
                  {questionBookmarks.length === 0 ? 'No Saved Questions Yet' : 'No Matching Questions'}
                </h3>
                ...
                {layoutMode === 'list' ? (
                  ...
                  <div className="space-y-2">
                    {filteredQuestions.map((b, idx) => {
                      const item = b.item || {}
                      const sub = extractText(item.subject || b.subject || 'General')
                      const top = extractText(item.topic || item.chapter || '')
                      const qText = extractText(item.question_text || item.questionText || item.question || b.title || `Question #${b.itemId || b.id}`)
                            ...
                            <div className="min-w-0 flex-1 space-y-1">
                              <div className="flex items-center gap-2 flex-wrap text-[10px]">
                                ...
                                <span className={`font-bold px-1.5 py-0.2 rounded uppercase ${
                                  diff === 'easy' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300' :
                                  diff === 'hard' ? 'bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300' :
          ...
          <div className="space-y-4">
            {otherBookmarks.length === 0 ? (
              <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6">
                ...
                <h3 className="text-base font-bold text-gray-800 dark:text-gray-200 mb-1">No Other Saved Resources</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 max-w-sm mx-auto">
                  Saved test series, study materials, and video lessons will appear here.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border capitalize ${getBadgeStyle(b.itemType)}`}>
                          {getIcon(b.itemType)}
                          <span>{b.itemType === 'study-material' ? 'Study' : b.itemType}</span>
                        </span>
                      ...
                      <h3 className="text-xs sm:text-sm font-bold text-gray-900 dark:text-gray-100 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors line-clamp-2">
                        <MathRenderer text={sanitizeHtml(extractText(b.title) || `Saved ${b.itemType}`)} />
                      </h3>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-3.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-xl flex items-center justify-between gap-2 text-xs text-amber-800 dark:text-amber-300">
              <div className="flex items-center gap-2">
                ...
                <span>Track status and resolution feedback for questions you reported.</span>
              </div>
            ...
            {reportsLoading ? (
              <div className="space-y-3">
                {[1, 2].map(i => (
                  <div key={i} className="bg-white dark:bg-gray-800 p-4 rounded-xl border animate-pulse h-28" />
                ))}
              </div>
            ) : myReports.length === 0 ? (
              <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6">
                <CheckCircle2 className="w-10 h-10 text-emerald-500 mx-auto mb-2 opacity-90" />
                <h3 className="text-base font-bold text-gray-800 dark:text-gray-200 mb-1">No Reported Questions</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 max-w-sm mx-auto">
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-extrabold ${
                          ...
                          {isResolved ? '✓ Resolution Pushed Live' : isDismissed ? 'Closed / Clarified' : '⏳ Under Expert Review'}
                        </span>
                      ...
                      <div className="text-xs font-semibold text-gray-900 dark:text-gray-100 leading-relaxed bg-gray-50 dark:bg-gray-950/60 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
                        <MathRenderer text={sanitizeHtml(report.questionText || `Question ID: ${report.questionId}`)} />
                      </div>
                      ...
                      <div className="py-1">
                        <div className="grid grid-cols-4 gap-2 text-center">
                          <div className="space-y-1">
                            <div className="w-5 h-5 mx-auto rounded-full bg-emerald-600 text-white font-bold text-[10px] flex items-center justify-center">✓</div>
                            <span className="text-[10px] font-bold text-gray-700 dark:text-gray-300 block">Submitted</span>
                          </div>
                          <div className="space-y-1">
                            <div className={`w-5 h-5 mx-auto rounded-full font-bold text-[10px] flex items-center justify-center ${
                              isResolved || isDismissed ? 'bg-emerald-600 text-white' : 'bg-amber-500 text-white animate-pulse'
                            }`}>
                              {isResolved || isDismissed ? '✓' : '2'}
                            </div>
                            <span className="text-[10px] font-bold text-gray-700 dark:text-gray-300 block">QA Audit</span>
                          </div>
                          <div className="space-y-1">
                            <div className={`w-5 h-5 mx-auto rounded-full font-bold text-[10px] flex items-center justify-center ${
                              isResolved ? 'bg-emerald-600 text-white' : isDismissed ? 'bg-gray-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-400'
                            }`}>
                              {isResolved || isDismissed ? '✓' : '3'}
                            </div>
                            <span className="text-[10px] font-bold text-gray-700 dark:text-gray-300 block">Decision</span>
                          </div>
                          <div className="space-y-1">
                            <div className={`w-5 h-5 mx-auto rounded-full font-bold text-[10px] flex items-center justify-center ${
                              isResolved ? 'bg-emerald-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-400'
                            }`}>
                              {isResolved ? '✓' : '4'}
                            </div>
                            <span className="text-[10px] font-bold text-gray-700 dark:text-gray-300 block">Live DB</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-[11px] bg-gray-50 dark:bg-gray-700/40 px-3 py-2 rounded-lg border border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-300">
                        <div>
                          <span className="font-semibold text-gray-500">Report Reason:</span>{' '}
                          <span className="text-rose-600 dark:text-rose-400 font-bold">{report.reason}</span>
                        </div>
                        <span className="text-gray-400 font-mono text-[10px]">ID: {report.questionId}</span>
                      </div>
                    </div>
                  )
                })}
