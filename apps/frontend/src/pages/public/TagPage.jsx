...
function TagPage({ tagProp }) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (configRes && configRes.success && configRes.data) {
          ...
          setTagConfig({
            id: tag,
            label: tag?.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()) || 'Tests',
            icon: '📋',
  ...
  const allTests = useMemo(() => {
    ...
    return testsData.filter(test => {
      ...
      if (tag === 'live-tests' && !tagConfig.filterKey) {
        ...
        return test.category === 'Mock Tests' && test.subCategory?.includes('Full')
      }
  ...
  const seriesOptions = useMemo(() => {
    ...
    return Array.from(unique).map(id => seriesData.find(s => s._id === id)).filter(Boolean)
          ...
          <div className="flex flex-row items-center gap-2">
            ...
            >
              ...
              {sortedSeriesOptions.map(s => (
                <option key={s._id || s.id} value={s._id || s.id}>{s.isPinned ? '📌 ' : ''}{s.title}</option>
              ))}
            </select>
            ...
            {/* Results Count */}
            <span className="text-xs text-gray-500 flex-shrink-0 hidden md:inline">{filteredTests.length} found</span>

            {/* Clear Button */}
            {(selectedSeries !== 'all' || selectedType !== 'all' || searchQuery) && (
              <button 
                onClick={() => { setSearchQuery(''); setSelectedSeries('all'); setSelectedType('all'); }}
