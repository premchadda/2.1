...
function ExamUpdates() {
    ...
    const fetchData = async () => {
      try {
        ...
        for (const category of categories) {
          ...
          if (found) {
            currentExam = { ...found, category: category.label || category.title }
            break
          }
        }
  ...
  const getPriorityBadge = (priority) => {
    switch (priority) {
      case 'high':
        return <span className="px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs font-medium rounded-full">High Priority</span>
      case 'medium':
        return <span className="px-2 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-xs font-medium rounded-full">Medium</span>
      default:
        return <span className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-medium rounded-full">General</span>
    }
  }

  const filteredUpdates = filter === 'all' 
    ? updates 
    : updates.filter(u => u.type === filter || (filter === 'high' && u.priority === 'high'))
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div className="animate-slide-in-right">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black mb-4 tracking-tight">
                {exam?.title || examId} {' '}
                <span className="block mt-1 text-transparent bg-clip-text bg-gradient-to-r from-pink-300 to-indigo-300">Announcements</span>
              </h1>
              <p className="text-white/80 text-lg md:text-xl max-w-2xl font-medium leading-relaxed">
                Stay ahead with all official notifications, syllabus updates, exam dates, and result declarations.
              </p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="space-y-2">
                ...
                >
                  <span className="text-gray-700 dark:text-gray-300">Exam Details</span>
                ...
                <Link
                  ...
                  className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition"
                >
                  <span className="text-gray-700 dark:text-gray-300">2026 Information</span>
                  <ChevronRight className="w-4 h-4 text-gray-400" />
                </Link>
                <Link
                  ...
                  className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition"
                >
                  <span className="text-gray-700 dark:text-gray-300">Year Comparison</span>
                  <ChevronRight className="w-4 h-4 text-gray-400" />
                </Link>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">Notification Release</p>
                    <p className="text-xs text-gray-500">Expected: March 2026</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-2 h-2 rounded-full bg-blue-500 mt-2"></div>
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">Application Start</p>
                    <p className="text-xs text-gray-500">Expected: April 2026</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
