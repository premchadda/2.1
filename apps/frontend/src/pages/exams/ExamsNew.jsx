...
export const getCategoryConfig = (category, index = 0) => {
  ...
  return {
    label: category.name || category.title || category.label || 'Unknown Category',
    ...
    description: category.description || category.shortDesc || `Comprehensive preparation material for ${category.name || category.title || category.label} exams.`
  };
};
...
function ExamsNew() {
  ...
  const processedExams = useMemo(() => {
    return allExams.map((exam, idx) => {
      const categoryIndex = examCategories.findIndex(cat => 
        (cat.categoryId || cat.slug || String(cat.id || cat._id)) === exam.categoryId
      )
      const category = categoryIndex >= 0 ? examCategories[categoryIndex] : null
      const config = getCategoryConfig(category, categoryIndex >= 0 ? categoryIndex : idx)
      return {
        ...exam,
        categoryLabel: config.label,
        categoryIcon: config.icon,
        categoryColor: config.color,
        testSeriesCount: testSeries.filter(s => s.category === exam.categoryId).length
      }
    })
  ...
  const filteredExams = useMemo(() => {
    ...
    if (selectedFilters.level.length > 0) {
      result = result.filter(exam => selectedFilters.level.includes(exam.level))
    }
    
    // Frequency filter
    if (selectedFilters.frequency.length > 0) {
      result = result.filter(exam => selectedFilters.frequency.includes(exam.frequency))
    }
    
    // Status filter
    if (selectedFilters.status.length > 0) {
      result = result.filter(exam => selectedFilters.status.includes(exam.status))
    }
    ...
    switch (sortBy) {
      ...
      case 'upcoming':
        result.sort((a, b) => {
          const dateA = a.nextExamDate ? new Date(a.nextExamDate) : new Date('9999-12-31')
          const dateB = b.nextExamDate ? new Date(b.nextExamDate) : new Date('9999-12-31')
          return dateA - dateB
        })
        break
      case 'tests':
  ...
  const categoryStats = useMemo(() => {
    ...
    const otherExams = processedExams.filter(e => !examCategories.some(cat => 
      (cat.categoryId || cat.slug || String(cat.id || cat._id)) === e.categoryId
    ))
  ...
  const filterOptions = {
    ...
    status: ['Notification Released', 'Application Open', 'Exam Soon', 'Result Declared']
  }
    ...
    <div className="min-h-screen bg-gray-50 pb-20 md:pb-8">
      ...
      <AnimatedHero pageType="exams" compact>
        <div className="text-center">
          ...
          <p className="text-white/80 max-w-xl mx-auto animate-slide-up" style={{ animationDelay: '0.1s' }}>
            Explore comprehensive test series for SSC, Railways, Banking, UPSC and more
          </p>
        </div>
      </AnimatedHero>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex flex-row items-center gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1.5 text-xs rounded-md border transition flex-shrink-0 ${
                showFilters || Object.values(selectedFilters).some(arr => arr.length > 0)
              ...
              <div className="flex flex-wrap gap-3">
                {Object.entries(filterOptions).map(([filterType, options]) => (
                  <div key={filterType} className="flex items-center gap-1">
                    <span className="text-[10px] font-semibold text-gray-500 uppercase">{filterType}:</span>
                    <select
                ...
                {Object.values(selectedFilters).some(arr => arr.length > 0) && (
                  <button
                    onClick={() => setSelectedFilters({ level: [], frequency: [], status: [] })}
        ...
        <div className="flex gap-6">
          {/* Sidebar */}
          <aside className={`${sidebarCollapsed ? 'w-16' : 'w-64'} flex-shrink-0 hidden lg:block`}>
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    <div className="space-y-1">
                      ...
                      {examCategories.map((cat, index) => {
                        const id = cat.categoryId || cat.slug || String(cat.id || cat._id)
                      ...
                      {categoryStats['other'] && categoryStats['other'].totalExams > 0 && (
                        <button
                            key="other"
                  ...
                  {bookmarkedExams.length > 0 && (
                    <div className="bg-white rounded-xl border border-gray-200 p-4">
                      ...
                      <div className="space-y-2">
                        {processedExams
                          .filter(exam => bookmarkedExams.includes(exam.examId))
                          .slice(0, 5)
                          .map(exam => (
                            <Link
                              key={exam.examId}
                            ...
                            >
                              ...
                              <p className="text-xs text-gray-500">{exam.categoryLabel}</p>
                            </Link>
                          ))
                        }
                      </div>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-semibold">{user.testsTaken || 0}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">Series Enrolled</span>
                          <span className="font-semibold">{user.enrolledSeries?.length || 0}</span>
                        </div>
                        <Link
                          to="/analysis"
          ...
          <main className="flex-1 min-w-0">
            ...
            {hasActiveFilters && (
              <div className="mb-4 flex flex-wrap items-center gap-2">
                ...
                {selectedCategory !== 'all' && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-indigo-50 text-indigo-700 rounded-full text-sm">
                    Category: {selectedCategory === 'other' ? 'Other' : getCategoryConfig(examCategories.find(c => (c.categoryId || c.slug || String(c.id || c._id)) === selectedCategory), 0)?.label}
              ...
              <div className="space-y-12 animate-fade-in">
                ...
                <section>
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-gray-500 text-sm mt-1">Explore exams grouped by top central and state categories.</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
                    {examCategories.map((cat, index) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`absolute -right-6 -top-6 w-24 h-24 bg-gradient-to-br ${config.color} opacity-5 rounded-full blur-2xl group-hover:opacity-15 transition-opacity duration-300`}></div>
                          
                          <div className="flex items-start justify-between mb-4 relative z-10">
                            <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${config.color} flex items-center justify-center text-2xl shadow-md text-white group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-300`}>
                              {config.icon}
                            </div>
                          ...
                          <p className="text-xs text-gray-500 mt-1 line-clamp-2 h-8 relative z-10">{config.description}</p>
                          
                          <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between relative z-10">
                            <span className="text-[11px] font-semibold text-gray-500 bg-gray-50 px-2 py-1 rounded-md border border-gray-100">{stats.totalTests || platformStats.mockTests} Tests</span>
                            <span className="text-[11px] font-semibold text-gray-500 bg-gray-50 px-2 py-1 rounded-md border border-gray-100">{stats.totalUsers > 1000 ? `${(stats.totalUsers/1000).toFixed(1)}k` : (stats.totalUsers || platformStats.activeLearners)} Users</span>
                          </div>
                        </button>
                      )
                    })}
                ...
                <div className="space-y-10">
                  {examCategories.map((cat, index) => {
                    const catId = cat.categoryId || cat.slug || String(cat.id || cat._id);
                    const config = getCategoryConfig(cat, index);
                    const catExams = processedExams.filter(e => e.categoryId === catId).slice(0, 4) // Show 4 for full grid
                    if (catExams.length === 0) return null
                    
                    return (
                      <section key={catId} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-gray-50">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${config.color} flex items-center justify-center text-xl shadow-sm text-white`}>
                              {config.icon}
                            </div>
                            <div>
                              ...
                              <p className="text-xs text-gray-500 mt-0.5">Top picks for {config.label} preparation</p>
                            </div>
                          </div>
                          <button
                          ...
                          >
                            View All {categoryStats[catId].totalExams} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                          </button>
                        </div>
                ...
                {filteredExams.length > 0 ? (
                  ...
                  <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-gray-200">
                    ...
                    <p className="text-gray-500 max-w-md mx-auto mb-6">
                      We couldn't find any exams matching your criteria. Try adjusting your filters.
                    </p>
                    <button
                      onClick={clearFilters}
...
// Exam Card Component
function ExamCard({ exam, isBookmarked, onToggleBookmark, viewMode, platformStats = {} }) {
  const config = { label: exam.categoryLabel, icon: exam.categoryIcon, color: exam.categoryColor }
  ...
  if (viewMode === 'list') {
    return (
      ...
      >
        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${config.color} flex items-center justify-center text-xl shadow-sm text-white flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}>
          {config.icon}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-[10px] px-2.5 py-1 bg-gradient-to-r ${config.color} bg-opacity-10 text-gray-700 bg-clip-text font-bold uppercase tracking-wider rounded-md border border-gray-100`}>
              {exam.categoryLabel}
            </span>
          </div>
          ...
          <p className="text-sm text-gray-500 truncate mt-0.5">{exam.fullName || exam.description || "Comprehensive test packages and previous year papers."}</p>
        </div>
        <div className="flex items-center gap-4 sm:gap-6 text-sm flex-shrink-0 mt-3 sm:mt-0 sm:mr-2">
          <div className="text-center sm:text-right">
            <p className="font-bold text-gray-900">{exam.testCount || platformStats.mockTests || '10+'}</p>
            <p className="text-[10px] text-gray-500 uppercase tracking-wide">Tests</p>
          </div>
          <div className="w-px h-8 bg-gray-200 hidden sm:block"></div>
          <div className="text-center sm:text-right">
            <p className="font-bold text-gray-900">{exam.userCount > 1000 ? `${(exam.userCount/1000).toFixed(1)}k` : (exam.userCount || platformStats.activeLearners || '500+')}</p>
        ...
        <div className="flex items-center justify-between sm:justify-start gap-3 mt-3 sm:mt-0 pt-3 sm:pt-0 border-t border-gray-100 sm:border-0 w-full sm:w-auto">
          <button
            onClick={(e) => { e.preventDefault(); onToggleBookmark(exam.examId) }}
  ...
  return (
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4 relative z-10">
          <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${config.color} flex items-center justify-center text-2xl shadow-md text-white group-hover:rotate-6 transition-transform duration-300`}>
            {config.icon}
          </div>
        ...
        <div className="flex-1 min-w-0 relative z-10">
          <span className={`text-[10px] px-2.5 py-1 bg-gray-50 text-gray-600 rounded-md font-bold uppercase tracking-wider mb-3 inline-block border border-gray-100 shadow-sm`}>
            {exam.categoryLabel}
          </span>
          ...
          <p className="text-sm text-gray-500 line-clamp-2 mb-5 h-10">
             {exam.fullName || exam.description || `Comprehensive mock tests and papers for ${exam.title}.`}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 p-3 bg-gray-50 rounded-xl text-xs text-gray-600 mt-auto relative z-10 border border-gray-100">
          <div className="flex flex-col items-center justify-center p-1">
             <span className="font-bold text-gray-900 text-sm">{exam.testCount || platformStats.mockTests || '10+'}</span>
             <span className="text-[10px] text-gray-500 uppercase tracking-wide font-medium mt-0.5">Tests</span>
          </div>
          <div className="flex flex-col items-center justify-center p-1 border-l border-gray-200">
             <span className="font-bold text-gray-900 text-sm">{exam.userCount > 1000 ? `${(exam.userCount/1000).toFixed(1)}k` : (exam.userCount || platformStats.activeLearners || '500+')}</span>
             <span className="text-[10px] text-gray-500 uppercase tracking-wide font-medium mt-0.5">Users</span>
          </div>
        </div>
    </Link>
  )
