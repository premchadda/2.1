...
const CATEGORY_CONFIG = {
  ssc: {
    ...
    description: "CGL, CHSL, MTS, CPO and more",
  },
  railways: {
    label: "Railway",
    icon: "🚂",
  ...
  banking: {
    ...
    description: "PO, Clerk, SO, RRB and more",
  },
  upsc: {
    label: "UPSC",
    icon: "🎖️",
    ...
    description: "CSE, CDS, NDA and more",
  },
  teaching: {
    label: "Teaching",
    icon: "🎓",
    ...
    description: "CTET, UTET and State TET",
  },
  defence: {
    label: "Defence",
    icon: "⚔️",
    ...
    description: "NDA, AFCAT, CDS and more",
  },
  state: {
    label: "State",
    icon: "🏛️",
    ...
    description: "State PSC, Police, Teaching",
  },
  other: {
    label: "Other",
    icon: "📚",
    ...
    description: "NEET, CLAT, CMAT and more",
  },
};
...
const DIFFICULTY_COLORS = {
  ...
  Medium:
    "bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-800/60",
  Hard: "bg-red-50 dark:bg-red-900/20 text-red-500 border-red-100 dark:border-red-800/60",
  "Very Hard":
    "bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 border-purple-100 dark:border-purple-800/60",
};
...
function getCatKey(cat) {
  ...
  const idStr = (
    cat.slug ||
    cat.categoryId ||
    (typeof (cat.id || cat._id) === "string" ? cat.id || cat._id : null) ||
    ""
  ).toLowerCase();
  ...
  if (name && name.length >= 3) {
    const nameMatch = Object.keys(CATEGORY_CONFIG).find(
...
function ExamCard({ exam, catConfig }) {
  ...
  const _diffColor =
    ...
    "bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border-gray-100 dark:border-gray-700";
  ...
  return (
    ...
    >
      {/* Avatar */}
      <div
        className={`w-9 h-9 md:w-11 md:h-11 rounded-lg md:rounded-xl bg-gradient-to-br ${config.color} flex items-center justify-center shadow-sm flex-shrink-0`}
      >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          {badge && (
            <span
              className={`${BADGE_COLORS[badge] || "bg-gray-400 dark:bg-gray-600"} text-white text-[8px] md:text-[9px] font-extrabold px-1.5 md:px-2 py-0.5 rounded-full uppercase tracking-wide flex-shrink-0`}
            >
              {badge}
            </span>
          )}
    ...
    >
      ...
      <div className="p-2.5 md:p-3.5">
        <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-2.5">
          <div
            className={`w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-gradient-to-br ${config.color} flex items-center justify-center shadow`}
          >
            <span className="text-white font-black text-[9px] md:text-[11px]">
              {abbrev}
            </span>
          </div>
          <span className="text-[9px] md:text-[10px] font-black text-gray-300 dark:text-gray-500">
            #{rank}
          </span>
        </div>
...
function _CategoryCard({ category: _category, count, config, onClick }) {
  return (
    ...
    >
      ...
      <div
        className={`absolute -right-6 -bottom-6 w-28 h-28 bg-gradient-to-br ${config.color} opacity-[0.10] rounded-full blur-2xl group-hover:opacity-[0.20] group-hover:scale-110 transition-all duration-500`}
      />
      <div
        className={`absolute -left-4 -top-4 w-16 h-16 bg-gradient-to-br ${config.color} opacity-[0.06] rounded-full blur-xl group-hover:opacity-10 transition-all duration-500`}
      />
      {/* Top accent */}
      <div
        className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${config.color} opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-t-2xl`}
      />

      <div className="relative z-10">
        <div className="flex items-start justify-between mb-3">
          <div
            className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${config.color} flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300 text-xl`}
          >
            {config.icon}
          </div>
          <div className="text-right">
            ...
            <p className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wide font-bold">
              Exams
            </p>
          </div>
        </div>
...
function Exams() {
  ...
  const trendingExams = useMemo(
    () =>
      allExams
        ...
        .filter(
          (e, i, arr) => arr.findIndex((x) => x.examId === e.examId) === i,
        )
        .slice(0, 6),
    [allExams],
  );
  ...
  const siteStats = useMemo(() => {
    const examsCount =
      examCategories.length > 0 ? examCategories.length : allExams.length;
    const mockTests =
  ...
  return (
    <div className="min-h-screen bg-[#f6f7fb] dark:bg-gray-900">
      <Helmet>
        ...
        <meta
          name="description"
          content="Explore competitive exams - SSC, Railway, Banking, UPSC, and more on Trstprep."
        />
        <meta property="og:title" content="Exams | Trstprep" />
        <meta
          property="og:description"
          content="Explore competitive exams - SSC, Railway, Banking, UPSC, and more."
        />
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
          ...
          <div className="flex flex-wrap gap-1.5 md:gap-2.5 md:justify-end">
            {[
              {
                label: "Exams",
                value: siteStats.exams,
                icon: <Target className="w-3 h-3 text-indigo-300" />,
              },
              {
                label: "Tests",
                value: siteStats.tests,
                icon: <Zap className="w-3 h-3 text-yellow-300" />,
              },
              {
                label: "Aspirants",
                value: siteStats.aspirants,
                icon: <Users className="w-3 h-3 text-blue-300" />,
              },
              {
                label: "Free Tests",
                value: siteStats.free,
                icon: <Star className="w-3 h-3 text-emerald-300" />,
              },
            ].map((stat, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 bg-white/10 backdrop-blur-sm border border-white/15 rounded-lg px-2.5 py-1 md:py-1.5 hover:bg-white/15 transition-colors cursor-default"
              >
                {stat.icon}
                <div className="flex flex-col">
                  <span className="text-[10px] md:text-xs font-black text-white leading-tight">
                    {stat.value}
                  </span>
                  <span className="text-[8px] md:text-[9px] font-bold text-indigo-100 uppercase tracking-tight">
                    {stat.label}
                  </span>
                </div>
              </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 flex flex-col gap-4 sm:gap-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 ml-10">
                  Top picks by thousands of students daily
                </p>
              </div>
            </div>
          ...
          <section className="animate-fade-in">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 ml-10">
                  Select a stream to see top exams and details
                </p>
              </div>
            </div>
              ...
              {examCategories.map((cat, index) => {
                ...
                return (
                  <button
                    key={catId}
                    onClick={() =>
                      setActiveCategoryModal({ id: catId, ...cfg })
                    }
                  ...
                  >
                    <div
                      className={`w-12 h-12 rounded-xl bg-gradient-to-br ${cfg.color} flex items-center justify-center text-xl shadow-md group-hover:scale-110 group-hover:rotate-3 transition-transform`}
                    >
                      {cfg.icon}
                    </div>
                    <div className="text-center">
                      <p className="font-black text-gray-900 dark:text-white text-sm leading-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors uppercase">
                        {cfg.label}
                      </p>
                      <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 mt-0.5 uppercase tracking-tighter">
                        {count} Exams
                      </p>
                    </div>
                  </button>
        ...
        <section className="bg-white dark:bg-gray-800 rounded-[2.5rem] p-6 shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="lg:w-56 flex-shrink-0 lg:border-r lg:border-gray-100 lg:pr-10">
              <div className="mb-5">
                <h2 className="text-base font-black text-gray-900 dark:text-white mb-1.5">
              ...
              <div className="flex lg:flex-col gap-1.5 overflow-x-auto lg:overflow-visible pb-3 lg:pb-0 scrollbar-hide">
                {categoryPills.map((pill) => (
                  ...
                  >
                    <span className="text-sm md:text-base transform group-hover:scale-110 transition-transform duration-300">
                      {pill.icon}
                    </span>
                    ...
                    <ArrowRight
                      className={`w-3 h-3 transition-opacity ${selectedCategory === pill.id ? "opacity-100" : "opacity-0"} hidden md:block`}
                    />
                  </button>
                ))}
              </div>
        ...
        {!searchQuery && (
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-indigo-950 rounded-2xl md:rounded-[2.5rem] p-6 md:p-4 sm:p-6 text-white">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-6 md:gap-10">
              <div className="flex-1 text-center md:text-left">
                ...
                <h2 className="text-2xl md:text-xl sm:text-2xl lg:text-3xl lg:text-2xl sm:text-3xl lg:text-4xl font-black mb-2 md:mb-4 leading-tight">
                ...
                <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                  <Link
                    to="/test-series"
                    className="px-5 md:px-8 py-2 md:py-3.5 bg-white dark:bg-gray-800 text-indigo-950 dark:text-white text-xs md:text-sm font-black rounded-xl md:rounded-2xl hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-all shadow-xl shadow-black/20"
                  >
                    Get Started Now
                  </Link>
                  <Link
                    to="/study"
                    className="px-5 md:px-8 py-2 md:py-3.5 bg-white/10 border border-white/20 text-white text-xs md:text-sm font-black rounded-xl md:rounded-2xl hover:bg-white/20 transition-all"
                  >
                    Explore Materials
                  </Link>
                </div>
              ...
              <div className="w-20 h-20 md:w-40 lg:w-56 md:h-40 lg:h-56 bg-indigo-500/20 rounded-full flex items-center justify-center p-4 md:p-8 backdrop-blur-2xl border border-white/10 shadow-2xl animate-pulse">
                <div className="text-xl sm:text-2xl lg:text-3xl md:text-6xl lg:text-7xl">
                  🎁
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                >
                  ...
                  <p className="text-white/60 text-[10px] mt-0.5 relative z-10">
                    {s}
                  </p>
                </Link>
              ))}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                [
                  ...
                  "Study by topic with linked notes, videos & quizzes",
                ],
                [
                  "🔗",
                  "Smart Weak Area Engine",
                  "AI identifies your gaps after every test",
                ],
                [
                  "📅",
                  "Deep PYQ Archive",
                  "Chapter-wise PYQs going back 10 years",
                ],
                [
                  "📱",
                  "Mobile First",
                  "Full prep experience on any screen size",
                ],
              ].map(([ic, t, d]) => (
                <div
                  key={t}
                  className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-4 shadow-sm text-center hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
                >
                  <p className="text-2xl mb-2">{ic}</p>
                  <p className="font-extrabold text-gray-800 dark:text-gray-200 text-xs mb-1">
                    {t}
                  </p>
          ...
          <div className="relative overflow-hidden bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-700 rounded-2xl p-7 text-white text-center">
            ...
            <div className="relative z-10">
              ...
              <p className="text-indigo-200 text-sm mb-5">
                We add new exams every week. Sign up and request yours.
              </p>
              <Link
                to="/signup"
      ...
      {activeCategoryModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          />
          <div
            className="relative bg-white dark:bg-gray-800 w-full max-w-[95vw] sm:max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-zoom-in max-h-[90vh] max-h-[90dvh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div
              className={`h-32 md:h-40 bg-gradient-to-br ${activeCategoryModal.color} relative overflow-hidden flex-shrink-0`}
            >
              ...
              <div className="absolute bottom-0 left-0 p-8 w-full bg-gradient-to-t from-black/20 to-transparent">
                <div className="flex items-center gap-5">
                  <div className="w-16 h-16 rounded-2xl bg-white dark:bg-gray-800 shadow-xl flex items-center justify-center text-xl sm:text-2xl lg:text-3xl">
                    {activeCategoryModal.icon}
                  </div>
                  <div>
                    <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-white">
                      {activeCategoryModal.label} Exams
                    </h2>
                    <p className="text-white/80 text-sm font-medium">
                      {activeCategoryModal.description ||
                        `Prepare for top ${activeCategoryModal.label} competitive exams.`}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-8 scrollbar-hide">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">
                  Available Exams ({categoryCounts[activeCategoryModal.id]})
                </h3>
            ...
            <div className="p-6 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center flex-shrink-0">
              <p className="text-xs text-gray-500 dark:text-gray-400 font-medium italic">
                Click on an exam to view test series & materials
              </p>
              <button
                onClick={() => {
