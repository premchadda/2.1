...
const getCategoryConfig = (category) => {
  ...
  const label =
    category?.label || category?.title || category?.name || "Category";
  ...
  if (category?.color) {
    ...
    bgColor =
      category.bgColor ||
      `${category.color.split(" ")[0].replace("from-", "bg-")}`;
  } else {
    ...
    try {
      ...
      if (categoryInfo.exams) {
        categoryInfo.exams.forEach((exam) => {
          ...
          testCounts[examId] = tests.filter(
            (t) =>
          ...
          seriesCounts[examId] = testSeries.filter(
            (s) =>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16 relative z-10">
          <Breadcrumb
            items={[
              ...
              {
                label:
                  categoryData.label || categoryData.title || categoryData.name,
                path: "",
              },
            ]}
            light
          ...
          <div className="mt-8 md:mt-12">
            <div className="flex flex-col md:flex-row md:items-center gap-6">
              <div className="w-20 h-20 md:w-24 md:h-24 bg-white/20 backdrop-blur-md rounded-3xl flex items-center justify-center text-3xl sm:text-4xl lg:text-5xl md:text-6xl shadow-xl border border-white/20 animate-bounce-subtle flex-shrink-0">
                {config.icon}
              </div>
              <div>
                <h1 className="text-2xl sm:text-3xl lg:text-4xl md:text-3xl sm:text-4xl lg:text-5xl lg:text-6xl font-extrabold tracking-tight mb-3 animate-slide-in-right">
                  {categoryData.label ||
                    categoryData.title ||
                    categoryData.name}{" "}
                  Exams
                ...
                <p
                  className="text-white/90 text-lg md:text-xl max-w-[95vw] sm:max-w-2xl animate-slide-in-up"
                  style={{ animationDelay: "0.1s" }}
                >
                  Select an exam below to view available test series, live
                  tests, previous year papers, and study material.
                </p>
              </div>
            </div>
          </div>
            ...
            categoryData.exams.map((exam, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div
                      className={`absolute -right-20 -top-20 w-64 h-64 bg-gradient-to-br ${config.color} opacity-0 group-hover:opacity-10 rounded-full blur-3xl transition-opacity duration-500`}
                    ></div>

                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                      <div className="flex items-start md:items-center gap-4">
                        <div
                          className={`w-14 h-14 rounded-xl bg-gradient-to-br ${config.color} p-[1px] shadow-sm group-hover:scale-105 transition-transform duration-300 flex-shrink-0`}
                        >
                        ...
                        <div className="flex-1 min-w-0 pr-4">
                          ...
                          <p className="text-[13px] text-gray-500 dark:text-gray-400 mb-3 line-clamp-1">
                            {exam.fullName}
                          </p>
                  ...
                  <div
                    className={`transition-all duration-500 ease-in-out ${isExpanded ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}
                  >
                    {isExpanded && (
                          ...
                          {examYears.map((year, yIdx) => (
                            ...
                            >
                              <div className="flex items-center justify-between relative z-10">
                                <span className="text-xl font-black text-gray-900 dark:text-white group-hover:text-brand-start transition-colors">
                                  {year}
                                </span>
                                {year === 2026 && (
