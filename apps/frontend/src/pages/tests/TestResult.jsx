...
function TestResult() {
  ...
  // Scroll spy for continuous scroll experience across all 6 sections in the right column
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          // Check if scrolled near the bottom of main scroll container
          const isAtBottom =
          ...
          const containerRect = container.getBoundingClientRect();
          const checkOffset = containerRect.top + 140;
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-gray-900 overflow-hidden">
      ...
      {/* ═══ TOP BAR ═══ */}
      <div className="shrink-0 bg-slate-900 text-white z-40 shadow-md">
        <div className="flex items-center justify-between min-h-[4.5rem] py-3 px-4 md:px-6">
          <div className="flex items-center gap-3 min-w-0 pr-2">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="lg:hidden w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-xl bg-slate-800 hover:bg-slate-700 transition-colors cursor-pointer"
            >
          ...
          <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3.5 py-2 text-xs md:text-sm font-bold text-sky-300 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-xl transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3.5 py-2 text-xs md:text-sm font-bold bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-xl transition-all shadow-md active:scale-95 cursor-pointer"
            >
      ...
      <div className="flex-1 flex relative overflow-hidden min-h-0">
        ...
        <aside
          className={`fixed lg:static top-[4.5rem] bottom-0 left-0 lg:top-0 h-[calc(100vh-4.5rem)] lg:h-full w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 z-30 shrink-0 flex flex-col transition-transform duration-300 ease-in-out ${
            sidebarOpen
        ...
        >
          ...
          <div className="shrink-0 p-4 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-b from-slate-50 to-white dark:from-gray-800 dark:to-gray-800">
          ...
          {/* Navigation with Subtle Animated Indicator */}
          <nav className="flex-1 overflow-y-auto py-2.5 px-2 space-y-1 scrollbar-thin">
            {sections.map((sec) => {
              const Icon = sec.icon;
              ...
              return (
                <button
                  ...
                  className={`group relative w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer text-left ${
                    isActive
                      ? "bg-indigo-600 text-white shadow-xs"
                      : "text-slate-600 dark:text-gray-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-gray-700/60"
                  }`}
                >
                  {/* Subtle, compact left accent bar */}
                  {isActive && (
                    <span className="absolute left-1 top-1/2 -translate-y-1/2 w-0.5 h-3.5 bg-white/90 rounded-full transition-all duration-200" />
                  )}
                  <Icon
            ...
            >
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xs border border-gray-200 dark:border-gray-700 p-5 sm:p-6">
                  ...
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <h3 className="text-lg font-black text-gray-900 dark:text-white mt-1">
                        {result.cutoffData?.categoryCutoff !== null &&
                        result.cutoffData?.categoryCutoff !== undefined
                          ? result.cutoffData?.isCleared
                            ? result.cutoffData?.hasCategoryCutoffs
                ...
                <div className="pt-5">
                  ...
                  Object.keys(result.cutoffData.cutoffs).length > 1 ? (
                    <>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5">
                        {Object.entries(result.cutoffData.cutoffs).map(
                          ([cat, cutoffVal]) => {
                            const isUserCat =
                              (result.cutoffData?.userCategory || "UR") === cat;
                            const clearedThis =
                              (result.score ?? 0) >= cutoffVal;
                            return (
                              <div
                                key={cat}
                    ...
                    <div className="p-4 rounded-xl bg-slate-50 dark:bg-gray-700/30 border border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-sm font-extrabold text-gray-900 dark:text-white mt-0.5">
                          ...
                          <span className="text-indigo-600 dark:text-indigo-400 font-black text-base">
                            {result.cutoffData.categoryCutoff} marks
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {result.cutoffData?.isCleared
                            ? `Cleared (+${result.cutoffData?.margin})`
                            : `Below Cutoff (${result.cutoffData?.margin})`}
                        </span>
                      </div>
                    </div>
                  ) : null}
                ...
                <>
                      ...
                      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1 pt-0.5 -mx-1 px-1">
                        ...
                        {resultSections.map((sec) => {
                          const secCount = questions.filter(
                            (q) =>
                              (q.section || q.subject || "General") === sec,
                          ).length;
                  ...
                  <div className="space-y-3.5">
                    {getFilteredQuestions().map((q, idx) => {
                      ...
                      const isExpanded =
                        expandedSolutions[q.id || q._id || idx];
