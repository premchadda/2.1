...
export default function TestReview() {
        ...
        try {
          ...
          if (response.data?.data) {
            ...
            const questions = (rawData.questions || []).map((q, idx) => ({
              ...
              correctMarks:
                q.marks !== undefined && q.marks !== null && !isNaN(q.marks)
                  ? Number(q.marks)
                  : (rawData.marksPerQuestion ?? rawData.positiveMarks ?? 2),
              negativeMarks:
                q.negativeMarks !== undefined &&
                q.negativeMarks !== null &&
                !isNaN(q.negativeMarks)
                  ? Number(q.negativeMarks)
                  : (rawData.negativeMarks ?? 0.5),
            }));
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-gray-900 fixed inset-0 z-50 print:relative print:h-auto print:inset-auto print:bg-white print:text-black print:overflow-visible">
      {/* ═══ PRINTABLE DOCUMENT VIEW (Only rendered during window.print()) ═══ */}
      <div className="hidden print:block p-8 bg-white text-black font-sans w-full">
        <div className="border-b-2 border-black pb-4 mb-6">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-black">
                {testData?.testTitle || "Test Paper & Solutions"}
              </h1>
              <p className="text-xs text-gray-600 mt-1">
                Trstprep Comprehensive Answer Key & Solution Paper
              </p>
            </div>
            <div className="text-right text-xs">
              <div className="font-bold">
                Score: {testData?.score ?? 0} / {testData?.totalMarks ?? 0}
              </div>
              <div className="text-gray-600">
                Rank: #{testData?.rank ?? 1} /{" "}
                {testData?.totalParticipants ?? 1} | Accuracy:{" "}
                {Math.round(testData?.accuracy ?? 0)}%
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {questions.map((q, idx) => {
            ...
            return (
              <div
                key={q.id || idx}
                className="break-inside-avoid border-b border-gray-300 pb-5 mb-5"
              >
                <div className="flex justify-between items-center text-xs font-bold text-gray-700 mb-2">
                  ...
                  <span>
                    +{q.correctMarks ?? 2} / -{q.negativeMarks ?? 0} marks
                    {q.communityStats?.attempts > 0 &&
                      ` • ${q.communityStats.correctPercentage}% answered correctly`}
                  </span>
                </div>
                <div className="text-sm font-medium text-gray-900 mb-3">
                  <MathRenderer content={q.text || q.questionText || ""} />
                </div>
                <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                  {q.options?.map((opt, optIdx) => {
                    const isCorrect = Number(q.correctOption) === optIdx;
                    const isUserSelected =
                      Number(uAns?.selectedOption) === optIdx;
                    return (
                      <div
                        key={optIdx}
                        className={`p-2 rounded border ${
                          isCorrect
                            ? "border-green-600 bg-green-50 font-bold text-green-900"
                            : isUserSelected
                      ...
                      >
                        ...
                        {isUserSelected && !isCorrect && " ✗ (Your Selection)"}
                      </div>
                    );
                  })}
                </div>
                {q.explanation && (
                  <div className="bg-gray-50 border border-gray-200 rounded p-2.5 text-xs text-gray-800">
      ...
      <div className="flex flex-col flex-1 h-full overflow-hidden print:hidden">
        {/* ═══ TOP HEADER ═══ */}
        <header className="bg-slate-900 text-white border-b border-slate-800 h-16 flex items-center justify-between px-4 sm:px-6 shadow-md select-none z-30">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 px-3 py-1.5 rounded-xl font-bold text-xs shadow-xs transition cursor-pointer"
              title="Export full test paper with questions, answers, and solutions as PDF"
            >
        ...
        <div className="flex flex-1 overflow-hidden relative">
          ...
          <div className="flex-1 flex flex-col bg-slate-50 dark:bg-gray-900 overflow-y-auto relative w-full">
            ...
            <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-8 py-2.5 flex justify-between items-center text-xs sm:text-sm">
              ...
              <div className="flex items-center gap-2">
                {/* Crowd Accuracy Badge */}
                {currentQuestion.communityStats &&
                  Number(currentQuestion.communityStats.attempts) > 0 && (
                    <div
                      className="flex items-center gap-1.5 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60 px-2.5 py-1 rounded-lg text-xs font-bold shadow-2xs"
                      title={`Aggregated from ${currentQuestion.communityStats.attempts} student attempts`}
                    >
                      <Users className="w-3.5 h-3.5 text-indigo-500" />
                      <span>
                        🎯 {currentQuestion.communityStats.correctPercentage}%
                        got this right
                      </span>
                      {currentQuestion.communityStats.averageTimeSeconds && (
                        <span className="text-indigo-400 font-medium border-l border-indigo-200 dark:border-indigo-800 pl-1.5 ml-0.5">
                          Avg:{" "}
                          {currentQuestion.communityStats.averageTimeSeconds}s
                        </span>
                      )}
                    </div>
                  )}
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="bg-white dark:bg-gray-800 p-5 sm:p-6 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xs">
                  <div className="text-gray-900 dark:text-white text-base sm:text-lg leading-relaxed font-medium">
                    <MathRenderer
                      text={sanitizeHtml(currentQuestion.question)}
                    />
                  </div>
                </div>
