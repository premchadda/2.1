...
import { LayoutGrid, ChevronLeft, ChevronRight, X, CheckCircle, XCircle, ArrowLeft, LayoutDashboard, RotateCcw, Eye, HelpCircle, Sparkles, BookOpen } from 'lucide-react'
...
export default function TestReview() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-gray-900">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-bold text-gray-600 dark:text-gray-300">Loading Solution & Review...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-gray-900">
        <div className="text-center bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 max-w-md">
          <p className="text-rose-600 dark:text-rose-400 font-bold mb-4">{error || 'Test data not available.'}</p>
          <button onClick={() => navigate('/dashboard')} className="px-4 py-2 bg-indigo-600 text-white rounded-xl font-bold text-sm">
            Back to Dashboard
          </button>
        </div>
      </div>
  ...
  const getQuestionStatus = (index) => {
    ...
    if (!answer || answer.selectedOption === undefined || answer.selectedOption === null) return 'not-answered'
    ...
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-gray-900 fixed inset-0 z-50">
      ...
      <header className="bg-slate-900 text-white border-b border-slate-800 h-16 flex items-center justify-between px-4 sm:px-6 shadow-md select-none z-30">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            <RotateCcw className={`w-3.5 h-3.5 ${interactiveMode ? 'animate-spin-once' : ''}`} />
          ...
          <button
            onClick={() => {
              if (seriesId && testId) {
                navigate(`/${seriesId}/tests/${testId}/result`, { state: { attemptId } })
              } else {
                navigate(-1)
              }
            }}
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 py-2 flex flex-wrap items-center justify-between gap-2 shadow-xs">
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
          {[
            ...
            { id: 'correct', label: `Correct (${userAnswers.filter(a => a.isCorrect).length})`, color: 'text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800' },
            { id: 'incorrect', label: `Incorrect (${userAnswers.filter(a => !a.isCorrect && a.selectedOption !== null && a.selectedOption !== undefined).length})`, color: 'text-rose-700 dark:text-rose-300 bg-rose-50 dark:bg-rose-900/20 border-rose-200 dark:border-rose-800' },
            { id: 'unattempted', label: `Unattempted (${userAnswers.filter(a => a.selectedOption === null || a.selectedOption === undefined).length})`, color: 'text-slate-700 dark:text-gray-200 bg-slate-50 dark:bg-gray-900 border-slate-200 dark:border-gray-700' }
          ].map(filter => (
            <button
              key={filter.id}
        ...
        <div className="flex items-center gap-2">
          {userAnswers.some(a => !a.isCorrect && a.selectedOption !== null && a.selectedOption !== undefined) && (
            <button
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col bg-slate-50 dark:bg-gray-900 overflow-y-auto relative w-full">
          ...
          <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-8 py-2.5 flex justify-between items-center text-xs sm:text-sm">
            <div className="font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2">
              <span className="text-gray-400 dark:text-gray-500 uppercase text-[10px] tracking-wider">Section:</span>
              <span className="text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-md border border-indigo-100 dark:border-indigo-800/60 font-extrabold">{currentQuestion.section || 'General'}</span>
            </div>
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-gray-900 px-2.5 py-1 rounded-lg border border-gray-200 dark:border-gray-700 shadow-2xs text-xs">
              <span className="text-emerald-600 dark:text-emerald-400 font-bold">+{currentQuestion.correctMarks || 2} marks</span>
              <span className="text-gray-300 dark:text-gray-500">|</span>
              <span className="text-rose-500 font-bold">-{currentQuestion.negativeMarks || 0.5} neg</span>
            </div>
          </div>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xs">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Live Attempt Status</p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {isOriginalSkipped ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-gray-400 border border-slate-200 dark:border-gray-700">
                          <HelpCircle className="w-3.5 h-3.5" /> Unattempted during Test
                        </span>
                      ) : isOriginalCorrect ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-200 border border-emerald-200 dark:border-emerald-800">
                          <CheckCircle className="w-3.5 h-3.5" /> Answered Correctly
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-rose-100 dark:bg-rose-900/30 text-rose-800 dark:text-rose-200 border border-rose-200 dark:border-rose-800">
                          <XCircle className="w-3.5 h-3.5" /> Answered Incorrectly
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {interactiveMode && (
                  <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-200 px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-2xs">
                    ...
                    <span>Interactive Resolve Active — Click options to test yourself!</span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                {currentQuestion.options.map((option, index) => {
                  ...
                  if (!interactiveMode) {
                    ...
                    if (isCorrectOption) {
                      optionClass = 'border-emerald-500 bg-emerald-50/70 dark:bg-emerald-900/20 shadow-xs'
                      badgeText = '✓ Correct Answer'
                      badgeColor = 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-200 border-emerald-200 dark:border-emerald-800'
                    } else if (isInitialChoice && !isOriginalCorrect) {
                      optionClass = 'border-rose-500 bg-rose-50/70 dark:bg-rose-900/20 shadow-xs'
                      badgeText = '✗ Your Choice (Wrong)'
                      badgeColor = 'bg-rose-100 dark:bg-rose-900/30 text-rose-800 dark:text-rose-200 border-rose-200 dark:border-rose-800'
                    }
                  } else {
                    // Interactive Re-solve Mode
                    if (isCorrectOption && currentResolveChoice !== undefined) {
                      optionClass = 'border-emerald-500 bg-emerald-50/70 dark:bg-emerald-900/20'
                    } else if (isCurrentReSolve && !isCorrectOption) {
                      optionClass = 'border-rose-500 bg-rose-50/70 dark:bg-rose-900/20'
                    } else if (isCurrentReSolve && isCorrectOption) {
                      optionClass = 'border-emerald-500 bg-emerald-50/70 dark:bg-emerald-900/20'
                    }

                    if (isInitialChoice) {
                      badgeText = isOriginalCorrect ? 'Test Choice: ✓ Correct' : 'Test Choice: ✗ Incorrect'
                      badgeColor = isOriginalCorrect ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-200 border-emerald-200 dark:border-emerald-800' : 'bg-rose-100 dark:bg-rose-900/30 text-rose-800 dark:text-rose-200 border-rose-200 dark:border-rose-800'
                    }
                  }
                  ...
                  return (
                    <div
                      key={index}
                      onClick={() => interactiveMode && handleReSolveOption(index)}
                      className={`p-4 rounded-2xl border-2 ${optionClass} transition-all relative ${
                        interactiveMode ? 'cursor-pointer active:scale-[0.99]' : 'cursor-default'
                      }`}
                    >
                      <div className="flex items-start gap-3.5">
                        <span className={`flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm ${
                          isCorrectOption && (!interactiveMode || currentResolveChoice !== undefined)
                            ? 'bg-emerald-600 text-white'
                            : isCurrentReSolve && !isCorrectOption
                            ? 'bg-rose-600 text-white'
                            : isInitialChoice && !isOriginalCorrect && !interactiveMode
                            ? 'bg-rose-600 text-white'
                        ...
                        <div className="flex flex-col items-end gap-1 flex-shrink-0">
                          ...
                          {interactiveMode && isCurrentReSolve && (
                            <span className={`px-2 py-0.5 rounded-md border text-[10px] font-black uppercase tracking-wider ${
                              isCorrectOption ? 'bg-emerald-600 text-white border-emerald-700' : 'bg-rose-600 text-white border-rose-700'
                            }`}>
                              {isCorrectOption ? 'Now Correct ✓' : 'Now Wrong ✗'}
                            </span>
                          )}
                        </div>
                      </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div className="bg-white dark:bg-gray-800 p-3.5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xs">
                  ...
                  <span className="text-xs font-extrabold text-gray-800 dark:text-gray-200 capitalize mt-0.5 block">{currentQuestion.difficulty || 'Medium'}</span>
                </div>
                <div className="bg-white dark:bg-gray-800 p-3.5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xs">
                  <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block">Topic</span>
                  <span className="text-xs font-extrabold text-gray-800 dark:text-gray-200 truncate mt-0.5 block">{currentQuestion.topic || 'General'}</span>
                </div>
                <div className="bg-white dark:bg-gray-800 p-3.5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xs col-span-2 sm:col-span-1">
                  <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block">Time Spent in Test</span>
                  <span className="text-xs font-extrabold text-gray-800 dark:text-gray-200 mt-0.5 block">{userAnswer?.timeSpent || 0} seconds</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-3.5 bg-indigo-50/70 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800/60 rounded-2xl flex-wrap gap-2">
                <div className="flex items-center gap-2 text-xs font-bold text-indigo-950 dark:text-indigo-200">
                  ...
                  <span>Revise {currentQuestion.topic || currentQuestion.chapter || currentQuestion.subject || 'this concept'} in Study Notes</span>
                </div>
                <Link
                  to="/study"
        ...
        {/* ═══ PALETTE SIDEBAR ═══ */}
        <div className={`w-80 border-l border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-y-auto p-4 ${showPalette ? 'block' : 'hidden md:block'}`}>
          <div className="flex items-center justify-between mb-4 border-b border-gray-100 dark:border-gray-700 pb-3">
            <h3 className="font-extrabold text-sm text-gray-900 dark:text-white uppercase tracking-wider">Question Navigator</h3>
            <button onClick={() => setShowPalette(false)} className="md:hidden p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
