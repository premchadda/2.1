...
export default function LiveTests() {
  ...
  const [formatFilter, setFormatFilter] = useState("tests"); // 'tests' | 'quizzes'
  ...
  const examOptions = useMemo(() => {
    ...
    if (Array.isArray(serverCategories)) {
      serverCategories.forEach((cat) => {
        ...
        if (label) {
          const id = (cat.slug || label).toLowerCase().trim();
          if (!EXCLUDED.has(id)) {
            map.set(id, label);
          }
        }
  ...
  useEffect(() => {
    ...
    const handleTestStarted = (data) => {
      ...
      toast.success(
        `🚀 Live ${isQuizItem ? "Quiz" : "Test"} "${data?.title || "New Session"}" is now LIVE!`,
        {
          duration: 6000,
          icon: isQuizItem ? "⚡" : "🔥",
        },
  ...
  const isQuiz = useCallback((test) => {
    return (
      checkIsQuiz(test) || test.itemType === "quiz" || test.item_type === "quiz"
    );
  }, []);

  // Split and categorize tests & quizzes
  const { liveTests, upcomingTests, pastTests } = useMemo(() => {
    const now = new Date();
    const live = [];
    const upcoming = [];
    const past = [];
  ...
  // Count tests vs quizzes for the currently selected active tab
  ...
  }, [activeTab, liveTests, upcomingTests, pastTests, isQuiz]);

  // Filter tests based on active tab, format filter (tests vs quizzes), search, exam, and subject
  const displayedTests = useMemo(() => {
    ...
    return list.filter((test) => {
      // Format Filter (Live Tests vs Live Quizzes)
      ...
      if (searchQuery.trim()) {
        ...
        const titleMatch = String(test.title || "")
          .toLowerCase()
          .includes(q);
        const descMatch = String(test.description || "")
          .toLowerCase()
          .includes(q);
        const catMatch = String(test.category || test.categoryName || "")
          .toLowerCase()
          .includes(q);
        const subCatMatch = String(test.subCategory || test.sub_category || "")
          .toLowerCase()
          .includes(q);
        const examMatch = String(test.examName || test.exam || "")
          .toLowerCase()
          .includes(q);
        const subjectMatch = String(test.subject || test.subjectName || "")
          .toLowerCase()
          .includes(q);
        const tagMatch =
          Array.isArray(test.tags) &&
          test.tags.some((t) => String(t).toLowerCase().includes(q));
        if (
          !titleMatch &&
          !descMatch &&
          !catMatch &&
      ...
      if (selectedExam !== "all") {
        ...
        const matchExam =
          String(test.category || "")
            .toLowerCase()
            .includes(ex) ||
          String(test.categoryName || "")
            .toLowerCase()
            .includes(ex) ||
          String(test.exam || "")
            .toLowerCase()
            .includes(ex) ||
          String(test.examName || "")
            .toLowerCase()
            .includes(ex) ||
          String(test.title || "")
            .toLowerCase()
            .includes(ex) ||
          (Array.isArray(test.tags) &&
            test.tags.some((t) => String(t).toLowerCase().includes(ex)));
      ...
      if (selectedSubject !== "all") {
        ...
        const matchSub =
          String(test.subCategory || test.sub_category || "")
            .toLowerCase()
            .includes(sub) ||
          String(test.subject || test.subjectName || "")
            .toLowerCase()
            .includes(sub) ||
          String(test.sectionName || "")
            .toLowerCase()
            .includes(sub) ||
          String(test.title || "")
            .toLowerCase()
            .includes(sub) ||
          (Array.isArray(test.tags) &&
            test.tags.some((t) => String(t).toLowerCase().includes(sub)));
  ...
  const handleRegister = async (test) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error || "Unable to register at this moment.",
      );
    } finally {
  ...
  // Render Section based on active format filter (Live Tests vs Live Quizzes)
  const renderTwoColumnSection = (items, extraCardProps = {}) => {
    ...
    // If filtered for 'quizzes'
    if (formatFilter === "quizzes") {
      return (
        <div key="quizzes-view" className="space-y-4 animate-slide-up">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400">
                  10-15 min quick sprints for topic mastery
                </p>
              </div>
            </div>
          ...
          {quizItems.length > 0 ? (
            ...
            <div className="text-center py-10 px-4 rounded-2xl bg-slate-50/70 dark:bg-slate-800/40 border border-dashed border-slate-200 dark:border-slate-700/60">
              ...
              <p className="text-sm font-bold text-slate-700 dark:text-slate-300">
                No Speed Quizzes in this View
              </p>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Explore the live tests tab for full mock exams
              </p>
            </div>
          )}
        </div>
    ...
    // Default: 'tests' (Live Tests)
    return (
      <div key="tests-view" className="space-y-4 animate-slide-up">
        <div className="flex items-center justify-between pb-3 border-b border-slate-200/80 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400">
                Full exam pattern & multi-section battles
              </p>
            </div>
          </div>
        ...
        {testItems.length > 0 ? (
          ...
          <div className="text-center py-10 px-4 rounded-2xl bg-slate-50/70 dark:bg-slate-800/40 border border-dashed border-slate-200 dark:border-slate-700/60">
            ...
            <p className="text-sm font-bold text-slate-700 dark:text-slate-300">
              No Full Tests in this View
            </p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Explore the live quizzes tab for quick sprint battles
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200 pb-24">
      <Helmet>
        ...
        <meta
          name="description"
          content="Compete in real-time synchronized live tests and speed quizzes on Trstprep. Measure your All-India Rank, speed, accuracy, and percentile with thousands of students nationwide."
        />
        <meta
          property="og:title"
          content="Live Tests & Real-Time Quizzes Arena | Trstprep"
        />
        <meta
          property="og:description"
          content="Compete in real-time synchronized live tests and speed quizzes on Trstprep with instant leaderboards."
        />
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 sticky top-0 z-30 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Breadcrumb
            items={[{ label: "Home", path: "/" }, { label: "Live Arena" }]}
          />
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pt-2 -mx-1 px-1">
          {[
            ...
            return (
              ...
              >
                <Icon
                  className={`w-3.5 h-3.5 ${tab.isLive && isActive ? "animate-pulse text-rose-600" : tab.isLive ? "animate-pulse text-rose-300" : ""}`}
                />
                <span>{tab.label}</span>
                <span
          ...
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-2 sm:gap-3">
            ...
            <div className="relative w-full sm:w-full max-w-[260px] sm:w-[260px] grid grid-cols-2 p-1 bg-slate-100/90 dark:bg-slate-800/80 rounded-xl sm:rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shrink-0 select-none">
              ...
              >
                ...
                >
                  {formatCounts.quizzes}
                </span>
              </button>
            </div>

            {/* Single Row for Search Bar + Exams Dropdown + Subjects Dropdown on Mobile & Desktop */}
            <div className="flex items-center gap-1.5 sm:gap-2 flex-1 min-w-0">
              ...
              <div className="relative flex-1 min-w-0">
                ...
                <input
                  ...
                  placeholder="Search..."
                  className="w-full pl-7 pr-6 py-2 bg-slate-50 dark:bg-slate-800/70 border border-slate-200/90 dark:border-slate-700/80 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                />
                {searchQuery && (
                  <button
                    onClick={() => setSearchQuery("")}
                    className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-md"
              ...
              <select
                ...
                className="w-[85px] xs:w-[96px] sm:w-auto sm:min-w-[125px] px-2 sm:px-3 py-2 bg-slate-50 dark:bg-slate-800/70 border border-slate-200/90 dark:border-slate-700/80 rounded-xl sm:rounded-2xl text-[11px] sm:text-xs font-bold text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer transition-all truncate shrink-0"
                aria-label="Exams"
              >
                {examOptions.map((opt) => (
          ...
          <div className="space-y-6">
            {displayedTests.length > 0 ? (
              ...
              <EmptyState
                ...
                desc="There are no live tests or quizzes ongoing at this moment. Check the upcoming schedule to register early, or browse past tests to review solutions and rankings."
                onReset={() => {
                  setSearchQuery("");
                  setFormatFilter("tests");
                  setSelectedExam("all");
                  setSelectedSubject("all");
                }}
                onNavigate={() => setActiveTab("upcoming")}
                ctaText="View Upcoming Schedule"
              />
            )}
          </div>
        )}
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">
                    Battle Calendar & Upcoming Schedule
                  </h2>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Scheduled live tests and speed quizzes. Register in advance
                    for free slot reservation and start alerts.
                  </p>
                </div>
              </div>
            </div>
            ...
            {displayedTests.length > 0 ? (
              ...
              <EmptyState
                ...
                onNavigate={() => setActiveTab("past")}
                ctaText="View Past Tests"
              />
            )}
          </div>
        )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    View final ranks, topper scorecards, and review full
                    step-by-step solutions for concluded events
                  </p>
                </div>
              </div>
            </div>
