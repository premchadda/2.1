...
function TestSeries() {
  ...
  const { data: freeQuizzes = [] } = useQuery({
  ...
  const attemptCountBySeries = useMemo(() => {
    ...
    attemptRows.forEach((attempt) => {
      const seriesKeys = [attempt.seriesId, attempt.seriesSlug]
        .filter(Boolean)
        .map(String);
      const testKey = String(
        attempt.testId || attempt.testSlug || attempt.id || "",
      );
  ...
  const getSeriesAttemptCount = useCallback(
    (series) => {
  ...
  const filteredSeries = useMemo(() => {
    ...
    if (selectedCategory !== "All") {
      result = result.filter(
        (s) => (s.categoryName || s.category) === selectedCategory,
      );
    }
    ...
    if (freeOnly) {
      result = result.filter(
        (s) => (s.freeTests || 0) > 0 || s.isPro === false,
      );
    }
    ...
    if (hindiOnly) {
      result = result.filter((s) => {
        ...
        if (Array.isArray(langs))
          return langs.some((l) => String(l).toLowerCase().includes("hi"));
        if (typeof langs === "string")
          return langs.toLowerCase().includes("hi");
        return false;
      });
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
          ...
          <div className="space-y-6 sm:space-y-8 min-w-0">
            ...
            {user && enrolledSeries.length > 0 && (
              <section className="fade-in relative overflow-hidden bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl border border-blue-100 dark:border-indigo-900/40 shadow-sm">
                ...
                <div className="bg-gradient-to-r from-slate-800 via-slate-800 to-slate-700 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 px-4 sm:px-5 py-3.5 sm:py-4 text-white flex flex-wrap sm:flex-nowrap items-center justify-between gap-2.5 sm:gap-3 shadow-sm">
                  ...
                  <Link
                    to="/analysis"
                    className="text-xs sm:text-sm font-bold text-white bg-white/20 hover:bg-white/30 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl backdrop-blur-sm flex items-center gap-1 transition-all shrink-0 ml-auto sm:ml-0"
                  >
                ...
                <div className="bg-gradient-to-r from-slate-800 via-slate-800 to-slate-700 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 px-4 sm:px-5 py-3.5 sm:py-4 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
                  <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-slate-300 truncate">
                        High-yield mock tests & previous year papers
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {(liveTests.length > 0 || freeQuizzes.length > 0) && (
              <section
                className="fade-in relative overflow-hidden bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl border border-rose-100 dark:border-rose-900/30 shadow-sm"
                style={{ animationDelay: user ? "0.1s" : "0.2s" }}
              >
                {/* Full-width Section Header Banner (Responsive) */}
                <div className="bg-gradient-to-r from-red-600 via-rose-600 to-orange-600 px-4 sm:px-5 py-3.5 sm:py-4 text-white flex items-center justify-between gap-2.5 sm:gap-3 shadow-sm">
                  <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-rose-100 truncate">
                        Real-time competitive exam simulation
                      </p>
                    </div>
                  </div>
                  <Link
                    to="/live-tests"
                    className="text-xs sm:text-sm font-bold text-white bg-white/20 hover:bg-white/30 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl backdrop-blur-sm flex items-center gap-1 transition-all shrink-0"
                  >
                    <span>View All</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </Link>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-3.5">
                        ...
                        <h3 className="font-bold text-gray-800 dark:text-gray-200 text-xs sm:text-sm">
                          Live Tests
                        </h3>
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        {liveTests.length > 0 ? (
                          liveTests.map((test) => {
                            const series = seriesWithStats.find(
                              (s) =>
                                String(s._id) === String(test.seriesId) ||
                                String(s.id) === String(test.seriesId),
                            );
                            return (
                              <Link
                              ...
                              >
                                ...
                                <div className="flex items-center gap-3 text-[10px] text-gray-500 dark:text-gray-400 mb-3">
                                  ...
                                  <span className="flex items-center gap-1">
                                    ...
                                    {test.totalQuestions || 100} Qs
                                  </span>
                                </div>
                                <button
                                  onClick={() =>
                                    navigate(
                                      `/test/${series?.slug || series?._id || test.seriesId}/${test.slug || test._id || test.id}`,
                                    )
                                  }
                                  className="w-full py-1.5 sm:py-2 bg-red-500 hover:bg-red-600 text-white text-[10px] sm:text-xs font-bold rounded-lg transition shadow-sm"
                                >
                                  Register Now
                                </button>
                              </Link>
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  className="w-full py-1.5 sm:py-2 bg-blue-500 hover:bg-blue-600 text-white text-[10px] sm:text-xs font-bold rounded-lg transition shadow-sm"
                                >
                                  Start Quiz
                                </button>
                              </Link>
            ...
            {user && newSeriesForYou.length > 0 && (
              <section
                className="fade-in relative overflow-hidden bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl border border-indigo-100 dark:border-indigo-900/40 shadow-sm"
                  ...
                  <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-indigo-100 truncate">
                        Tailored series based on your interests
                      </p>
                    </div>
                  </div>
                  <Link
            ...
            <section
              className="fade-in relative overflow-hidden bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl border border-indigo-100 dark:border-indigo-900/30 shadow-sm"
              style={{ animationDelay: user ? "0.4s" : "0.3s" }}
            >
              {/* Full-width Section Header Banner (Responsive) */}
              <div className="bg-gradient-to-r from-slate-800 via-slate-800 to-slate-700 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 px-4 sm:px-5 py-3.5 sm:py-4 text-white flex items-center justify-between gap-2.5 sm:gap-3 shadow-sm">
                <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] sm:text-xs text-slate-300 truncate">
                      Targeted test series grouped by exam category
                    </p>
                  </div>
                </div>
                <span className="bg-white/20 text-white text-[10px] sm:text-xs font-bold px-2 sm:px-2.5 py-0.5 rounded-full whitespace-nowrap backdrop-blur-sm shrink-0">
              ...
              <div className="p-4 sm:p-5 md:p-6">
                <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                  {Object.entries(seriesByCategory).map(
                    ([category, series]) => {
                          ...
                          <div>
                            ...
                            <div className="flex items-center justify-between mb-3.5">
                              <div className="flex items-center gap-2.5 sm:gap-3">
                                <div
                                  className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-xl sm:text-2xl transition-all duration-300 group-hover:scale-110 group-hover:shadow-sm ${styles.bgLight.split(" ")[0]} ${styles.bgLight.split(" ")[1]}`}
                                >
              ...
              <div className="relative">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {attemptRows.length > 0 && (
                    <Link
                      to="/attempted-tests"
                      className="text-[10px] font-bold text-brand-start dark:text-indigo-400 hover:underline"
                    >
                      View All →
                    </Link>
                  )}
                    ...
                    {attemptRows.slice(0, 4).map((attempt, idx) => {
                      ...
                      const timeLabel =
                        dateObj && !isNaN(dateObj)
                          ...
                          : "Recently";
                      const accuracy =
                        attempt.accuracy !== null ? attempt.accuracy : null;
                      const accColor =
                        accuracy === null
                      ...
                      return (
                        ...
                        >
                          ...
                          {accuracy !== null && (
                            <span
                              className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${accColor}`}
                            >
                              {accuracy}%
                            </span>
                          )}
