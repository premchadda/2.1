...
function TestInstructions() {
  ...
  const getSectionNameInLanguage = (name) => {
    ...
    let n = name;
    n = n.replace(
      /General Intelligence & Reasoning|General Intelligence|Reasoning/gi,
      "सामान्य बुद्धिमत्ता और तर्कशक्ति",
    );
    n = n.replace(
      /General Awareness|General Knowledge|GK/gi,
      "सामान्य जागरूकता",
    );
    n = n.replace(
      /Quantitative Aptitude|Mathematics|Quant/gi,
      "मात्रात्मक योग्यता",
    );
    n = n.replace(
      /English Comprehension|English Language|English/gi,
      "अंग्रेजी समझ",
    );
    return n;
  };
  ...
  const attemptNo = useMemo(() => {
    ...
    if (
      test?.userAttemptCount !== undefined &&
      test?.userAttemptCount !== null
    ) {
  ...
  const displayQuestionCount = questionsLoading
    ? "..."
    : actualQuestionCount !== null
      ? actualQuestionCount
      : test?.totalQuestions;

  const hasSectionalTiming = Boolean(
  ...
  const effectiveSections = useMemo(() => {
    if (Array.isArray(test?.sections) && test.sections.length > 0) {
      return test.sections.map((s) => {
        const qCount =
          s.questionCount ??
          s.question_count ??
          Math.round((test.totalQuestions || 100) / test.sections.length);
        const tMarks =
          s.totalMarks ??
          s.total_marks ??
          (qCount
    ...
    const sectionNames = Array.isArray(test?.testSections)
      ? test.testSections
      : typeof test?.testSections === "string" && test.testSections.trim()
        ? test.testSections
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean)
    ...
    const totalM = Number(
      test?.totalMarks || totalQs * (test?.marksPerQuestion || 2),
    );
    ...
    return names.map((name, i) => ({
      ...
      questionCount:
        i === names.length - 1
          ? totalQs - qPerSec * (names.length - 1)
          : qPerSec,
      totalMarks:
        i === names.length - 1
      ...
      try {
        ...
        if (!currentTest && seriesId) {
          try {
            ...
            currentTest = seriesTests.find(
              (t) =>
                t._id === testId ||
                t.id === testId ||
                t.slug === testId ||
                String(t._id || t.id) === testId ||
                String(t.public_id || "") === testId ||
                String(t.public_id_uuid || "") === testId,
            );
          } catch {
        ...
        if (currentTest) {
          ...
          try {
            ...
            if (!controller.signal.aborted) {
              const currentSeries = allSeries.find(
                (s) =>
                  s._id === seriesId ||
                  s.id === seriesId ||
                  s.slug === seriesId ||
                  String(
                    s.examId || s.exam_id || s.category || "",
                  ).toLowerCase() === String(seriesId).toLowerCase() ||
  ...
  const handleStartTest = () => {
    if (isLocked) {
      toast.error(
        isHindi
          ? "इस टेस्ट के लिए प्रो पास आवश्यक है"
          : "Pro Pass required for this test",
      );
      navigate("/pass");
      return;
    }
    if (agreedToRules && !noQuestions) {
      ...
      toast.error(
        isHindi
          ? "कृपया पहले नियमों से सहमत हों"
          : "Please agree to the rules first",
      );
    }
  };
  ...
  const testType = useMemo(() => {
    ...
    if (tags.includes("pyq") || tags.includes("pyp"))
  ...
  const _generalInstructions = [
    {
      ...
      detail:
        "The timer will start as soon as you begin. Ensure you manage your time effectively across all sections.",
    },
    {
      icon: BookOpen,
      title: "Questions",
      ...
      detail:
        "Each question carries equal marks. Read each question carefully before selecting your answer.",
    },
    {
      icon: BarChart3,
    ...
    },
    {
      ...
      detail:
        "Move between questions using the sidebar or next/previous buttons. Flag questions for review.",
    },
    {
      icon: Timer,
      title: "Auto Submit",
      ...
      detail:
        "The test will be automatically submitted when time expires. Save your answers regularly.",
    },
    {
      icon: Shield,
      title: "Security",
      ...
      detail:
        "Tab switching is monitored. Multiple violations may result in test cancellation.",
    },
  ];
        ...
        <div className="flex flex-col items-center justify-center p-8 max-w-md w-full animate-fade-in text-center space-y-4">
          ...
          <div>
            <h3 className="text-base font-bold text-gray-900 dark:text-gray-100 tracking-wide">
              {isHindi
                ? "परीक्षण निर्देश लोड हो रहे हैं..."
                : "Loading Test Instructions..."}
            </h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {isHindi
                ? "नियम, अवधि और परीक्षा निर्देश तैयार किए जा रहे हैं"
                : "Preparing test rules, duration & exam instructions"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-tcs-surface">
        <div className="text-center max-w-md">
          ...
          <p className="text-tcs-text-secondary mb-6">
            {isHindi
              ? "आप जिस परीक्षण की तलाश कर रहे हैं वह मौजूद नहीं है या हटा दिया गया है।"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between py-2.5 sm:h-14 gap-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="hidden sm:block">
                <Breadcrumb
                  items={[
                    {
                      label: isHindi ? "परीक्षण श्रृंखला" : "Test Series",
                      path: "/test-series",
                    },
                    {
                      label: series?.title || (isHindi ? "श्रृंखला" : "Series"),
            ...
            <div className="flex items-center gap-2.5 sm:ml-4 shrink-0">
              <div className="inline-flex items-center p-0.5 rounded-full border border-indigo-200/80 dark:border-indigo-800/60 bg-slate-100 dark:bg-slate-800 shadow-inner">
                ...
                >
                  <Globe
                    className={`w-3 h-3 ${!isHindi ? "text-indigo-200" : "text-slate-400"}`}
                  />
                  <span>English</span>
                </button>
                <button
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                  <h1 className="text-lg sm:text-xl font-black text-white tracking-tight drop-shadow-xs">
                    {test.title}
                  </h1>
                ...
                <div className="bg-gradient-to-r from-indigo-50 to-slate-50 px-4 py-2.5 border-b border-tcs-border flex items-center justify-between flex-wrap gap-2">
                  ...
                  {sectionalTimerEnabled ? (
                    ...
                    <span className="text-xs font-medium px-2.5 py-0.5 rounded-full bg-amber-100/80 text-amber-800 border border-amber-300 flex items-center gap-1">
                      ...
                      {isHindi
                        ? `अनुभाग टाइमर: निष्क्रिय (साझा ${test?.duration || 60} मिनट टाइमर)`
                        : `Sectional Timer: Disabled (Shared ${test?.duration || 60} Min Timer)`}
                    </span>
                  )}
                </div>
                  ...
                  <div className="p-2.5 rounded-lg bg-white border border-tcs-border flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="font-bold text-tcs-text-primary text-xs sm:text-sm">
                        {test.duration || 60} {isHindi ? "मिनट" : "Mins"}
                      </div>
                    </div>
                  </div>
                  <div className="p-2.5 rounded-lg bg-white border border-tcs-border flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="font-bold text-tcs-text-primary text-xs sm:text-sm">
                        {displayQuestionCount} {isHindi ? "प्रश्न" : "Qs"}
                      </div>
                    </div>
                  </div>
                  <div className="p-2.5 rounded-lg bg-white border border-tcs-border flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-tcs-text-secondary text-[10px] uppercase font-semibold">
                        {isHindi ? "कुल अंक" : "Total Marks"}
                      </div>
                      <div className="font-bold text-tcs-text-primary text-xs sm:text-sm">
                        {test.totalMarks ||
                          (displayQuestionCount !== "..."
                            ? Number(displayQuestionCount) * 2
                            : 200)}{" "}
                        {isHindi ? "अंक" : "Marks"}
                      </div>
                    </div>
              ...
              <section className="bg-amber-50/50 rounded-xl border border-amber-200/80 overflow-hidden shadow-2xs">
                <div className="px-4 py-2.5 bg-amber-100/60 border-b border-amber-200/60 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xs sm:text-sm font-bold text-amber-900">
                      ...
                      ({rules.length} {isHindi ? "नियम" : "Rules"})
                    </h2>
                  </div>
                ...
                <div className="p-3 space-y-2.5">
                  <ul className="space-y-2">
                    {(showRules ? rules : rules.slice(0, 2)).map(
                      (rule, index) => (
                        <li
                          key={index}
                          className="flex items-start gap-2.5 text-xs sm:text-sm text-amber-900/90 leading-relaxed"
                        >
                      ...
                      <span>
                        {showRules
                          ? isHindi
                            ...
                            : "Show Less"
                          : isHindi
                            ? `+ ${rules.length - 2} और नियम दिखाएं`
                            : `+ Show ${rules.length - 2} More Rules`}
                      </span>
                      <ChevronDown
                        className={`w-3.5 h-3.5 transition-transform duration-200 ${showRules ? "rotate-180" : ""}`}
                      />
                    </button>
                  )}
                </div>
            ...
            <div className="lg:col-span-1 p-4 sm:p-5 border-l border-tcs-border bg-tcs-surface/30 flex flex-col justify-between space-y-3">
              ...
              <div className="bg-white p-3.5 rounded-xl border border-tcs-border shadow-2xs text-xs space-y-2">
                ...
                <div className="pt-2 border-t border-gray-100 text-xs text-tcs-text-muted">
                  ...
                  <Link
                    to="/contact"
                    className="text-tcs-primary hover:underline font-semibold"
                  >
                    {isHindi
              ...
              <div className="hidden lg:block bg-white p-4 rounded-xl border border-indigo-200 shadow-md space-y-3">
                ...
                {isLocked && (
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-2 text-amber-800">
                    ...
                    <p className="text-xs font-medium">
                      {isHindi
                        ? "यह टेस्ट प्रो पास के अंतर्गत आता है।"
                        : "This test requires an active Pro Pass."}
                    </p>
                  </div>
                )}
                ...
                {isLocked ? (
                  ...
                  >
                    ...
                    {isHindi
                      ? "प्रो पास प्राप्त करें"
                      : "Get Pro Pass to Start"}
                  </button>
                ) : noQuestions ? (
                  <button
                    disabled
                  ...
                  >
                    {user?.attemptedTestIds?.includes(
