      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-lg border border-gray-100 p-2 mb-6">
          <div className="flex flex-row items-center gap-2">
            ...
            <div className="relative flex-1 min-w-0">
              ...
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-0.5">
            ...
            {/* Results Count */}
            <span className="text-xs text-gray-500 flex-shrink-0 hidden md:inline">{filteredPapers.length} found</span>

            {/* Clear Button */}
            {(examFilter !== 'all' || yearFilter !== 'all' || searchTerm) && (
              <button 
                onClick={() => { setExamFilter('all'); setYearFilter('all'); setSearchTerm(''); }}
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">Curating Papers...</p>
          </div>
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {(paper.subjects || ['Quantitative', 'Reasoning', 'English']).slice(0, 3).map((sub, i) => (
                      <span key={i} className="text-[10px] font-bold text-gray-400 border border-gray-100 px-2 py-0.5 rounded-md">
                        {sub}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center justify-between py-4 border-t border-gray-50">
                    <div className="flex items-center gap-1.5 text-gray-400">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-tighter">{paper.questions || 100} Questions</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-gray-400">
                      <Calendar className="w-3.5 h-3.5" />
                      <span className="text-[10px] font-black uppercase tracking-tighter">Official</span>
                    </div>
                  </div>
          ...
          <div className="text-center py-24 bg-white rounded-[3rem] border border-dashed border-gray-200">
            ...
            <h3 className="text-xl font-black text-gray-900 mb-2">No Papers Found</h3>
            <p className="text-gray-500 font-medium max-w-xs mx-auto mb-8 text-sm">We couldn't find any papers matching your current filters. Try resetting them.</p>
            <button 
              onClick={() => {
                setExamFilter('all')
                setYearFilter('all')
        ...
        {/* Global Stats Footer */}
        <div className="mt-20 bg-gray-900 rounded-[3rem] p-10 md:p-16 text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 rounded-full blur-[100px]" />
          <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              <p className="text-3xl md:text-5xl font-black mb-1">{platformStats.mockTests}</p>
              <p className="text-[10px] md:text-xs font-black text-indigo-300 uppercase tracking-[0.2em]">Live Papers</p>
            </div>
             <div>
              <p className="text-3xl md:text-5xl font-black mb-1">{platformStats.examsCovered}</p>
              <p className="text-[10px] md:text-xs font-black text-indigo-300 uppercase tracking-[0.2em]">Exams Covered</p>
            </div>
             <div>
              <p className="text-3xl md:text-5xl font-black mb-1">5+</p>
              <p className="text-[10px] md:text-xs font-black text-indigo-300 uppercase tracking-[0.2em]">Years Depth</p>
            </div>
             <div>
              <p className="text-3xl md:text-5xl font-black mb-1">FREE</p>
              <p className="text-[10px] md:text-xs font-black text-indigo-300 uppercase tracking-[0.2em]">View Access</p>
            </div>
          </div>
        </div>
      </div>
