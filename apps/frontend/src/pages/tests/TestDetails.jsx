...
function TestDetails() {
  ...
  const allStages = useMemo(() => {
    return (directStages && directStages.length > 0) ? directStages : (hookStages || [])
  ...
  const seriesStages = useMemo(() => {
    ...
    let rawStages = series?.stages ?? series?.stageIds ?? series?.stage_ids ?? []
    if (typeof rawStages === 'string') {
      try {
        rawStages = JSON.parse(rawStages)
      } catch {
        rawStages = rawStages.replace(/[{}]/g, '').split(',').map(s => s.trim()).filter(Boolean)
      }
    }
    ...
    if (stageIdSet.size > 0) {
      const matched = allStages.filter(s =>
    ...
    if (Array.isArray(tests) && tests.length > 0) {
      ...
      tests.forEach(t => {
        ...
        if (Array.isArray(t.stageIds)) t.stageIds.forEach(id => presentStageIds.add(String(id)))
        if (Array.isArray(t.stage_ids)) t.stage_ids.forEach(id => presentStageIds.add(String(id)))
      })
    ...
    // 3. Fallback: filter by exam subcategory / category
    if (series) {
      const examIdentifier = String(series.subcategory || series.sub_category || series.category || '').toLowerCase()
      const linked = allStages.filter(s => {
        if (Array.isArray(s.examIds)) {
          return s.examIds.some(eid => String(eid).toLowerCase() === examIdentifier || examIdentifier.includes(String(eid).toLowerCase()))
        }
        return false
      })
    ...
    return allStages.filter(s => s.isActive !== false && s.is_active !== false)
  ...
  const stageOptions = useMemo(() => {
    ...
    const stagesToUse = (seriesStages && seriesStages.length > 0) ? seriesStages : allStages
  ...
  const visibleTests = useMemo(() => {
    return tests.filter(t => {
      ...
      if (status === 'draft' || status === 'archived' || status === 'disabled' || status === 'inactive') return false;
      if (checkIsArchivedLive(t)) return false; // Fully archived live test (> 7 days past expiry)
      return status === 'published' || status === 'active' || status === '' || t.isActive !== false;
    });
  }, [tests]);
  ...
  const stageFilteredTests = useMemo(() => {
    ...
    const stage = allStages.find(s => s.slug === activeStage || String(s.id) === activeStage || String(s._id) === activeStage);
    ...
    return visibleTests.filter(test => {
      ...
      const testStageIds = (Array.isArray(test.stageIds) ? test.stageIds : Array.isArray(test.stage_ids) ? test.stage_ids : []).map(String);
      ...
      const testSubCategory = String(test.subCategory || test.subcategory || '').toLowerCase();
      const testTags = (Array.isArray(test.tags) ? test.tags : []).map(t => String(t).toLowerCase());

      // Direct stage ID matching
      if (testStageId && (testStageId === targetId || testStageId === targetDbId)) return true;
      if (testStageIds.includes(targetId) || testStageIds.includes(targetDbId)) return true;
      if (testTier && (testTier === stageName || testTier === stageSlug)) return true;

      // Stage semantic matching (Tier 1 / Tier 2 / CBT 1 / CBT 2)
      if (stageSlug.includes('tier-1') || stageSlug.includes('tier-i') || stageName.includes('tier 1') || stageName.includes('tier i')) {
        if (testTags.includes('tier-1') || testTags.includes('tier-i') || testTags.includes('tier1')) return true;
        if (/\btier[- ]?(1|i)\b/i.test(testTitle) || /\btier[- ]?(1|i)\b/i.test(testCategory) || /\btier[- ]?(1|i)\b/i.test(testSubCategory)) return true;
      }

      if (stageSlug.includes('tier-2') || stageSlug.includes('tier-ii') || stageName.includes('tier 2') || stageName.includes('tier ii')) {
        if (testTags.includes('tier-2') || testTags.includes('tier-ii') || testTags.includes('tier2')) return true;
        if (/\btier[- ]?(2|ii)\b/i.test(testTitle) || /\btier[- ]?(2|ii)\b/i.test(testCategory) || /\btier[- ]?(2|ii)\b/i.test(testSubCategory)) return true;
      }

      if (stageSlug.includes('cbt-1') || stageName.includes('cbt 1') || stageName.includes('cbt-1')) {
        if (testTags.includes('cbt-1') || testTags.includes('cbt1')) return true;
        if (/\bcbt[- ]?1\b/i.test(testTitle)) return true;
      }

      if (stageSlug.includes('cbt-2') || stageName.includes('cbt 2') || stageName.includes('cbt-2')) {
        if (testTags.includes('cbt-2') || testTags.includes('cbt2')) return true;
  ...
  const totalPermanentTests = permanentTests.length || Number(series?.total_tests ?? series?.totalTests ?? 0)
  ...
  const computedCategories = useMemo(() => {
    ...
    stageFilteredTests.forEach(test => {
      ...
      const matchedCats = allTestCategories.filter(c => {
        ...
        if (testCategoryIds.has(catIdStr) || testCategoryIds.has(catSlugStr) || testCategoryIds.has(catNameStr)) {
          return true;
        }
        ...
        for (const tid of testCategoryIds) {
          if (/\b(19\d\d|20\d\d)\b/.test(tid)) {
            if (catNameStr === tid || catSlugStr === tid || catNameStr.includes(tid) || catSlugStr.includes(tid)) {
              return true;
            }
          }
        }
  ...
  const availableThirdCategories = useMemo(() => {
    if (!activeMainCategory || !computedCategories[activeMainCategory]) return [];
  ...
  const availableFourthCategories = useMemo(() => {
    ...
    if (activeThirdCategory !== 'all') {
      ...
      for (const sub of subChildren) {
        if (Array.isArray(sub.children)) {
          ...
          if (found) { targetThird = found; break; }
        }
      }
    ...
    const targetSubList = activeSubCategory !== 'all'
    ...
    return aggregated;
  }, [computedCategories, activeMainCategory, activeSubCategory, activeThirdCategory]);
  ...
  const testBelongsToActiveCategory = useCallback((test) => {
    ...
    return Array.from(testCategoryIds).some(id => {
      ...
      if (/\b(19\d\d|20\d\d)\b/.test(id)) {
        return activeAllowedCategoryIds.some(allowed => allowed.includes(id));
      }
      return false;
    });
  ...
  const categoryResumeTest = useMemo(() => {
    if (!user || incompleteAttempts.length === 0 || !series || tests.length === 0) return null;
    ...
    const completedTestIds = new Set(
      attemptRows
        .filter(a => String(a.status).toLowerCase() === 'completed' || a.isCompleted || a.is_completed)
        .map(a => String(a.testId || a.test_id))
    );

    const seriesIncomplete = incompleteAttempts.filter(attempt => {
      const matchSeries = String(attempt.seriesId) === currentSeriesId ||
        (attempt.seriesSlug && attempt.seriesSlug === currentSeriesSlug);
      const isPausedOrInProgress = attempt.status === 'PAUSED' ||
    ...
    for (const attempt of seriesIncomplete) {
      ...
      if (matchedTest && testBelongsToActiveCategory(matchedTest)) {
        return { ...attempt, testDetails: matchedTest, title: attempt.testTitle || matchedTest.title };
      }
    }
    return null;
  }, [user, incompleteAttempts, series, tests, testBelongsToActiveCategory, attemptRows]);
  ...
  const categoryNextTest = useMemo(() => {
    ...
    const seriesAttemptRows = attemptRows.filter(a =>
      String(a.seriesId) === currentSeriesId || (a.seriesSlug && a.seriesSlug === currentSeriesSlug)
    );
    ...
    return null;
  }, [user, attemptRows, series, tests, testBelongsToActiveCategory, categoryResumeTest]);
  ...
  useEffect(() => {
    if (Object.keys(computedCategories).length > 0) {
      ...
      rootKeys.forEach(k => { exclusiveCounts[k] = 0; });

      stageFilteredTests.forEach(test => {
        const rawCatId = test.testCategoryId ?? test.test_category_id ?? test.categoryId ?? test.category_id ?? '';
        let matchedCat = null;
        if (rawCatId) {
          const catIdStr = String(rawCatId).toLowerCase();
          matchedCat = allTestCategories.find(c =>
        ...
        let foundRoot = null;
        while (currentId) {
          const rk = rootKeys.find(k => String(computedCategories[k]?.id) === currentId || k === currentId);
          if (rk) { foundRoot = rk; break; }
          const ancestor = allTestCategories.find(c => String(c._id || c.id) === currentId);
          currentId = ancestor ? String(ancestor.parentId || ancestor.parent_id || '') : '';
        }
  ...
  }, [computedCategories, stageFilteredTests, allTestCategories, activeMainCategory, activeSubCategory, activeThirdCategory, activeFourthCategory]);
  ...
  useEffect(() => {
    ...
    const fetchMetadata = async () => {
      try {
        const [catsData, examsData, testCatsData, stagesRes] = await Promise.all([
          getExamCategories(),
          getExams(),
          getTestCategories(),
    ...
    const fetchData = async () => {
      try {
        ...
        if (isMyView || !lookupId || lookupId === 'my') {
          ...
          if (examSlug) {
            ...
            matched = allSeries.find(s =>
              (String(s.slug || '').toLowerCase() === slugLower ||
               String(s.subcategory || '').toLowerCase() === slugLower ||
          ...
          if (!matched && user) {
            matched = allSeries.find(s => isSeriesEnrolled(user, s, [s._id, s.id]))
          }
        ...
        if (seriesData) {
          ...
          if (Array.isArray(testsData)) {
            testsData.forEach(test => {
              const tid = String(test.id ?? test._id ?? test.public_id ?? test.slug ?? Math.random());
              if (!seenIds.has(tid)) {
                seenIds.add(tid);
        ...
        const related = allSeries
          .filter(s => s._id !== targetSid && s.id !== targetSid && s.slug !== targetSid)
  ...
  useEffect(() => {
    ...
    if (user && isEnrolled) {
      ...
      if (location.pathname !== expectedPath && !location.pathname.endsWith('/my')) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-transparent">
        <Helmet>
          <title>{series?.title ? `${series.title} | Trstprep` : 'Test Series | Trstprep'}</title>
          <meta name="description" content="View test details, enroll, and start practicing on Trstprep." />
        </Helmet>
        <div className="flex flex-col items-center justify-center p-8 max-w-md w-full animate-fade-in text-center space-y-4">
          ...
          <div>
            <h3 className="text-base font-bold text-gray-900 dark:text-gray-100 tracking-wide">Loading Test Series...</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Preparing tests, categories & performance insights</p>
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center px-4">
        <Helmet>
          ...
          <meta name="description" content="The requested test series was not found." />
        </Helmet>
        <div className="text-center">
          <div className="text-6xl mb-4">📦</div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Test Series Not Found</h1>
          <p className="text-gray-600 dark:text-gray-300 mb-2">The test series you're looking for doesn't exist in our system.</p>
          <p className="text-gray-500 dark:text-gray-400 mb-6">Please check the URL or browse our available test series.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <Helmet>
        ...
        <meta name="description" content={series?.description || 'View test details, enroll, and start practicing on Trstprep.'} />
        <meta property="og:title" content={`${series?.title || 'Test Details'} | Trstprep`} />
        <meta property="og:description" content={series?.description || 'View test details, enroll, and start practicing on Trstprep.'} />
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600 dark:text-indigo-200/80 mb-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold text-slate-900 dark:text-white">{totalPermanentTests}</span> Tests
                </div>
                {liveTestsCount > 0 && (
                  <div className="flex items-center gap-1.5">
                    <Radio className="w-4 h-4 text-rose-500 animate-pulse" />
                    <span className="font-bold text-slate-900 dark:text-white">{liveTestsCount}</span> Live Tests
                  </div>
                )}
                <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold text-slate-900 dark:text-white">{series.rating || 4.8}</span>
                </div>
                <span className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-200 border border-emerald-200 dark:bg-emerald-500/20 dark:text-emerald-300 dark:border-emerald-500/30 px-2.5 py-0.5 rounded-full text-xs font-semibold">
                  {permanentFreeTestsCount || series.free_tests || series.freeTests || 0} Free Tests
                </span>
              </div>

              {/* Description */}
              {series.description && (
                <p className="text-slate-700 dark:text-indigo-100/90 text-sm mb-4 leading-relaxed font-medium">{series.description}</p>
              )}
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {seriesStages.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1 sm:gap-2">
                      <span className="text-[10px] sm:text-xs font-bold text-slate-550 dark:text-indigo-200/60 whitespace-nowrap">Covers:</span>
                  ...
                  {categoryCounts.length > 0 && (
                    <div className="flex flex-wrap items-center gap-2">
                      {categoryCounts.map((cat, i) => (
                        <div key={i} className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-100/70 border border-slate-200 dark:bg-white/5 dark:border-white/10 rounded-lg">
                          <span className="text-[10px] font-black text-slate-550 dark:text-indigo-200/60 uppercase tracking-widest">{cat.name}</span>
                          <span className="bg-slate-200 text-slate-700 dark:bg-white/10 dark:text-white px-1.5 py-0.5 rounded-md font-black text-[11px]">{cat.count}</span>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            {user && (categoryResumeTest || categoryNextTest) && (
              <section className="fade-in mb-6">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <h2 className="text-xl font-bold text-gray-900 dark:text-white">Up Next</h2>
                </div>
                      ...
                      <div className="px-3.5 py-2.5">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide bg-amber-500 text-white">
                            ...
                            {categoryResumeTest.status === 'PAUSED' ? 'PAUSED' : 'IN PROGRESS'}
                          </span>
                        ...
                        <div className="flex justify-between items-start gap-2.5">
                          ...
                          <Link
                            to={(() => {
                              const resumeTestId = categoryResumeTest.public_id_uuid || categoryResumeTest.public_id || categoryResumeTest.testDetails?.public_id || categoryResumeTest.testId || categoryResumeTest.testDetails?.id || categoryResumeTest.testDetails?._id || categoryResumeTest.slug
                              const resumeSlug = categoryResumeTest.seriesSlug || series?.slug
                              const attemptNo = categoryResumeTest.attemptNo || categoryResumeTest.attemptNumber || 1
                              return resumeSlug
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-xs text-gray-500 dark:text-gray-400">
                          <span className="flex items-center gap-1">
                            ...
                            {categoryResumeTest.answeredQuestions || 0}/{categoryResumeTest.totalQuestions || 0} Qs Answered
                          </span>
                          <span className="text-gray-300 dark:text-gray-500">|</span>
                          <span className="flex items-center gap-1">
                            <span className="text-sm">📄</span>
                            {categoryResumeTest.testDetails?.totalMarks || categoryResumeTest.testDetails?.marks || 200} Marks
                          </span>
                          <span className="text-gray-300 dark:text-gray-500">|</span>
                          {categoryResumeTest.remainingTimeSeconds > 0 ? (
                            <span className="flex items-center gap-1">
                              <span className="text-sm">🕒</span>
                              {Math.round(categoryResumeTest.remainingTimeSeconds / 60)} Mins Left
                            </span>
                          ) : (
                            <span className="flex items-center gap-1">
                              <span className="text-sm">🕒</span>
                              {categoryResumeTest.testDetails?.duration || 60} Mins Total
                            </span>
                          )}
                        </div>
                        ...
                        <div className="mt-3">
                          ...
                          <div className="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${categoryResumeTest.progressPct || 0}%` }}
                            />
                          </div>
                        </div>
                      </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-none md:rounded-xl border border-gray-100 dark:border-gray-700 p-2 md:p-4">
              ...
              <div className="mt-4">
                {paginatedTests.length > 0 ? (
                  <>
                    <div key={`${animateKey}-${currentPage}`} className="space-y-3 animate-fadeIn">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {startPage > 1 && (
                              <>
                                ...
                                {startPage > 2 && (
                                  <span className="text-gray-400 dark:text-gray-500 text-xs px-0.5">...</span>
                                )}
                              </>
                            )}
                            ...
                            {endPage < totalTestPages && (
                              <>
                                ...
                                <button
                                  type="button"
                                  onClick={() => setCurrentPage(totalTestPages)}
                                  className="w-7 h-7 md:w-8 md:h-8 rounded-lg text-xs font-bold transition-all cursor-pointer bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"
                                >
                                  {totalTestPages}
                                </button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {userStats?.totalTests ?? userStats?.testCount ?? 0} tests attempted
                        </p>
                      </div>
                ...
                <div className="space-y-2">
                  {rankingsLoading ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">No performances in this series yet</p>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 overflow-hidden">
              <div className="p-4 border-b border-gray-100 dark:border-gray-700">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-gray-900 dark:text-white">Suggested Series</h3>
                </div>
                <p className="text-gray-500 dark:text-gray-400 text-xs mt-1">Based on your preparation</p>
              </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {suggested.rating && (
                            <div className="flex items-center gap-1 mt-1">
                              ...
                              <span className="text-xs font-medium text-gray-600 dark:text-gray-300">{suggested.rating}</span>
                            </div>
                          )}
                        </div>
            ...
            <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl shadow-card p-5 text-white">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-green-300">{permanentFreeTestsCount || series.freeTests || 0}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-indigo-100">Total Questions</span>
                  <span className="font-bold">{permanentTests.reduce((acc, t) => acc + (t.totalQuestions || t.questions || 0), 0)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-indigo-100">Total Marks</span>
                  <span className="font-bold">{permanentTests.reduce((acc, t) => acc + (t.totalMarks || t.marks || 100), 0)}</span>
                </div>
              </div>
            </div>
          </div>
