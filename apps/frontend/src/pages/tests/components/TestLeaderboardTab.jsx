...
export default function TestLeaderboardTab({
    ...
    const fetchLeaderboard = async () => {
      try {
        ...
        const normalized = (rawList || []).map((entry, idx) => ({
          ...
          name:
            entry.name || entry.userName || `Student #${entry.rank || idx + 1}`,
          score: parseFloat(entry.score ?? 0),
          percentile: entry.percentile
            ? parseFloat(entry.percentile).toFixed(1)
            : "90.0",
          accuracy: entry.accuracy ?? 85,
          ...
          setLeaderboardData([
            {
              ...
              percentile: result.percentile
                ? parseFloat(result.percentile).toFixed(1)
                : "100.0",
              accuracy: result.accuracy ? Math.round(result.accuracy) : 90,
              isCurrentUser: true,
              isPro: isProUser,
            },
          ]);
  ...
  const maxScore =
    result?.maxScore ||
    result?.totalMarks ||
    (result?.totalQuestions ? result.totalQuestions * 2 : 200);

  const userRank =
    result?.rank || leaderboardData.find((r) => r.isCurrentUser)?.rank || 1;
