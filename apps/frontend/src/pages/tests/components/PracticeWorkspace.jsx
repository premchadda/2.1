...
import {
  CheckCircle, XCircle, Lightbulb, Bookmark, MessageSquare,
  Sparkles, Layers, BookOpen, ThumbsUp, Plus, ArrowRight, Play, Eye
} from 'lucide-react'
...
export default function PracticeWorkspace({ session, onComplete }) {
  ...
  const handleCheckAnswer = async () => {
    ...
    try {
      const res = await practiceAPI.checkAnswer(session.id, currentIdx, { selectedOption: selectedOption })
      setCheckResult(res)
      setIsChecked(true)
    } catch {
  ...
  const handleUpvoteApproach = async (approachId) => {
    try {
      ...
      setApproaches(prev => prev.map(a => a.id === approachId ? { ...a, upvotes: (a.upvotes || 0) + 1 } : a))
    ...
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-6 shadow-xs flex flex-wrap items-center justify-between gap-4">
        <div className="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
          ...
          <span>→</span>
          <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded font-mono">{question.difficulty || 'Medium'}</span>
        </div>

        <div className="flex items-center gap-4 text-xs font-bold">
          <span className="text-slate-600">Question {currentIdx + 1} / {session?.totalQuestions || 10}</span>
          <span className="text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full">Accuracy: 80%</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-xs mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            <Bookmark className="w-3.5 h-3.5 mr-1 text-amber-500" /> Save to Knowledge Vault
          </button>
        </div>
        ...
        <div className="space-y-3 mb-8">
          {question.options?.map((opt, i) => {
            ...
            let style = 'border-slate-200 bg-white hover:border-indigo-300 hover:bg-slate-50'
            ...
            if (isChecked) {
              ...
              style = 'border-indigo-600 bg-indigo-50/60 text-indigo-900 font-semibold ring-1 ring-indigo-500'
            }
            ...
            return (
              ...
              >
                ...
                <div className="text-sm">
                  <MathRenderer content={typeof opt === 'object' ? opt.text : opt} />
                </div>
              </button>
            )
          })}
        ...
        {!isChecked ? (
          ...
          <div className={`p-4 rounded-xl border flex items-center justify-between mb-2 ${
            checkResult?.isCorrect ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : 'bg-rose-50 border-rose-200 text-rose-900'
          }`}>
            <div className="flex items-center gap-2">
              {checkResult?.isCorrect ? <CheckCircle className="w-5 h-5 text-emerald-600" /> : <XCircle className="w-5 h-5 text-rose-600" />}
              <span className="font-bold text-sm">{checkResult?.isCorrect ? 'Correct Answer!' : 'Incorrect'}</span>
            </div>
            <span className="text-xs font-medium">Correct Choice: Option {checkResult?.correctAnswer}</span>
          </div>
        )}
      </div>
      ...
      {isChecked && (
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-indigo-900 to-slate-900 rounded-2xl p-5 text-white shadow-md">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-bold text-sm flex items-center text-indigo-200">
                <Sparkles className="w-4 h-4 mr-2 text-amber-400" /> Interactive AI Tutor
              </h4>
              <span className="text-[10px] uppercase font-mono tracking-wider text-slate-400">Contextual Learning</span>
            </div>

            <div className="flex flex-wrap gap-2 mb-3">
              <button onClick={() => handleAskAiTutor('hint')} className="px-3 py-1.5 rounded-lg bg-indigo-800/60 hover:bg-indigo-700 text-xs font-semibold text-indigo-100 border border-indigo-700/50 transition">
                💡 Give me a hint
              </button>
              <button onClick={() => handleAskAiTutor('explain_simply')} className="px-3 py-1.5 rounded-lg bg-indigo-800/60 hover:bg-indigo-700 text-xs font-semibold text-indigo-100 border border-indigo-700/50 transition">
                🧒 Explain simply
              </button>
              <button onClick={() => handleAskAiTutor('another_method')} className="px-3 py-1.5 rounded-lg bg-indigo-800/60 hover:bg-indigo-700 text-xs font-semibold text-indigo-100 border border-indigo-700/50 transition">
                ⚡ Show another method
              </button>
              <button onClick={() => handleAskAiTutor('why_wrong')} className="px-3 py-1.5 rounded-lg bg-indigo-800/60 hover:bg-indigo-700 text-xs font-semibold text-indigo-100 border border-indigo-700/50 transition">
                🔍 Why is my answer wrong?
              </button>
            </div>

            {aiLoading && <div className="text-xs text-indigo-300 animate-pulse">AI Tutor is generating guidance...</div>}
            {aiTutorResponse && (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <h3 className="font-bold text-slate-900 text-base flex items-center">
                <BookOpen className="w-5 h-5 text-indigo-600 mr-2" /> Explanation System
              </h3>
                ...
                {['text', 'visual', 'video', 'formula'].map((tab) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold capitalize transition ${
                      activeExplTab === tab ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                    }`}
                  >
                    {tab}
                  </button>
            ...
            {activeExplTab === 'text' && (
              <div className="space-y-4 text-sm text-slate-700 leading-relaxed">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <h5 className="font-bold text-slate-900 text-xs uppercase tracking-wider mb-1 text-indigo-600">Step-by-Step Solution</h5>
                  <MathRenderer content={explanations?.text?.stepByStep || question.explanation || 'Detailed step-by-step solution.'} />
                </div>
                {explanations?.text?.shortcut && (
                  <div className="bg-amber-50 p-4 rounded-xl border border-amber-200/60">
                    <h5 className="font-bold text-amber-900 text-xs uppercase tracking-wider mb-1">⚡ Exam Shortcut</h5>
              ...
              <div className="py-4 text-center">
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: sanitizeHtml(explanations?.visual?.svgContent || '<p>Visual diagram placeholder</p>'),
                  }}
                />
                <p className="text-xs text-slate-500 mt-2">Diagram schema representation of percentage change.</p>
              </div>
            )}
            ...
            {activeExplTab === 'video' && (
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-500">Short video explanation (180s duration)</p>
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <h3 className="font-bold text-slate-900 text-base flex items-center">
                <MessageSquare className="w-5 h-5 text-indigo-600 mr-2" /> 💬 Discussion — {approaches.length} Approaches
              </h3>
              <button
            ...
            <div className="space-y-3">
              {approaches.map((appr) => (
                <div key={appr.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    >
                      <ThumbsUp className="w-3 h-3 mr-1 text-indigo-500" /> {appr.upvotes}
                    </button>
                  </div>
                  <p className="text-xs text-slate-800 leading-relaxed font-medium mb-1">{appr.content}</p>
                  <div className="text-[10px] text-slate-400 font-semibold">— Shared by {appr.authorName}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs">
            <h3 className="font-bold text-slate-900 text-base mb-3 flex items-center">
              <Layers className="w-5 h-5 text-indigo-600 mr-2" /> Want to master this concept?
            </h3>
            <p className="text-xs text-slate-500 mb-4">Practice level-by-level similar questions without losing context.</p>

            <div className="space-y-2">
              {similarQs.map((sim, i) => (
                <div key={sim.id} className="p-3 rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-between">
                  <div className="text-xs font-medium text-slate-800 truncate max-w-lg">
                    Level {i + 1}: <MathRenderer content={sim.questionText || sim.question_text} />
                  </div>
