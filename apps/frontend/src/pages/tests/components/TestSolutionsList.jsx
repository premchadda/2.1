              ...
              <div className="max-w-5xl mx-auto px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 space-y-2">
                ...
                <div className="flex items-center justify-between gap-2 min-w-0">
                      ...
                      <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 font-semibold truncate">
                        ...
                        {solutionSectionFilter !== "all" && (
                          <span className="text-indigo-600 dark:text-indigo-400">
                            {" "}
                            • {questionsInActiveSection.length} in{" "}
                            {solutionSectionFilter}
                          </span>
                        )}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <div
                      className="flex items-center gap-1 p-0.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800"
                      role="group"
                      aria-label="Solution language"
                      title="Solution language"
                    >
                      <Globe
                        className="w-3.5 h-3.5 ml-1.5 text-indigo-500 dark:text-indigo-400"
                        aria-hidden="true"
                      />
                      {["en", "hi"].map((lang) => (
                        <button
                        ...
                        <div className="flex items-center justify-between pt-1 text-xs">
                          ...
                          <div className="flex items-center gap-3">
                            {onReportQuestion && (
                              <button
                                type="button"
                                onClick={() => onReportQuestion(q.id || q._id)}
                                disabled={reportedQuestions?.has(q.id || q._id)}
                                className={`inline-flex items-center gap-1 text-[11px] font-bold transition-colors ${
                                  reportedQuestions?.has(q.id || q._id)
                                    ? "text-emerald-600 dark:text-emerald-400 cursor-default"
                                    : "text-gray-400 hover:text-rose-500 dark:hover:text-rose-400 cursor-pointer"
                                }`}
                                title={
                                  reportedQuestions?.has(q.id || q._id)
                                    ? "Reported for review"
...
TestSolutionsList.propTypes = {
  ...
  onReportQuestion: PropTypes.func,
  reportedQuestions: PropTypes.any,
};
