...
export default function PracticeSessionCanvas({ session, onExit, onComplete }) {
  ...
  const isLearningMode =
    session?.mode === "learn" || session?.mode === "adaptive";
      ...
      <main className="max-w-4xl mx-auto w-full px-4 py-6 flex-1 flex flex-col gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200/80 dark:border-slate-700/80 p-6 md:p-8 shadow-sm space-y-6">
          ...
          <div className="text-slate-900 dark:text-white font-medium text-sm md:text-base leading-relaxed">
            <MathRenderer
              content={sanitizeHtml(currentQ.questionText || currentQ.question)}
            />
          </div>
          ...
          <div className="space-y-3 pt-2">
            {(currentQ.options || []).map((optText, idx) => {
              ...
              if (submission && isLearningMode) {
                ...
                optionStyle =
                  "bg-indigo-50/80 dark:bg-indigo-950/50 border-indigo-600 text-indigo-950 dark:text-indigo-100 font-semibold shadow-xs";
              }
              ...
              return (
                <div
                  ...
                  className={`group relative flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer ${optionStyle} ${
                    isStruck
                      ? "opacity-30 line-through cursor-not-allowed bg-slate-100 dark:bg-slate-900"
                      : ""
                  }`}
                >
                  ...
                  <button
                    ...
                    title={
                      isStruck ? "Un-strike option" : "Cross out wrong option"
                    }
            ...
            >
              ...
              {submission.explanation && (
                <div className="text-xs md:text-sm leading-relaxed space-y-1">
                  ...
                  <MathRenderer
                    content={sanitizeHtml(submission.explanation)}
                  />
                </div>
              )}
            </div>
      ...
      {aiDrawerOpen &&
        ...
        createPortal(
          <div
            role="dialog"
            aria-modal="true"
            aria-label="Socratic AI Study Tutor"
            className="fixed inset-0 z-[99999] flex justify-end bg-slate-900/60 backdrop-blur-sm animate-fade-in"
            onClick={() => setAiDrawerOpen(false)}
          >
            <div
              className="w-full max-w-md bg-white dark:bg-slate-800 h-full p-6 shadow-2xl flex flex-col justify-between border-l border-slate-200 dark:border-slate-700 animate-slide-in"
