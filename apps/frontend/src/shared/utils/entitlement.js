...
export const resolveTestCtaState = ({
  ...
  if (
    test.isUpdating ||
    (test.totalQuestions === 0 && !isLive && !isUpcoming)
  ) {
    return {
  ...
  // 2. Expired Live Test / Result Available (Contest has ended for all users)
  if (isExpired) {
    if (targetResultUrl && (isAttempted || user)) {
      return {
        actionState: "RESULT",
        label: "View Result",
        to: targetResultUrl,
  ...
  // 4. Unauthenticated User Flow (for active/upcoming/regular mock tests)
  if (!user) {
  ...
  // 5. Pro Pass Required for Free User
