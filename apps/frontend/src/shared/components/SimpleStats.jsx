...
export default function SimpleStats({
  ...
  if (error)
    return (
      <div className={`p-4 ${colors.bg} rounded-lg border ${colors.border}`}>
        <p className={`text-sm ${colors.iconText}`}>{error}</p>
      </div>
    );
  if (!stats)
    return (
      <div className={`p-4 ${colors.bg} rounded-lg border ${colors.border}`}>
        <p className={`text-sm ${colors.iconText}`}>No data available</p>
      </div>
    );
    ...
    <div className={`p-4 ${colors.bg} rounded-lg border ${colors.border}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard
          ...
          value={stats.activeLearners || stats.users || 0}
          colors={colors}
        />
        <StatCard
          label="Mock Tests"
          value={stats.mockTests || stats.tests || 0}
          colors={colors}
        />
        <StatCard
          label="Practice Questions"
          value={stats.practiceQuestions || stats.questions || 0}
          colors={colors}
        />
        <StatCard
          label="Test Series"
