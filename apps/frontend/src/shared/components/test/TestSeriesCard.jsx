...
function TestSeriesCard({ series, user, showProgress = false, onEnroll, showCategories = true }) {
  ...
  const isSeriesPro = Boolean(isPro === true || series.is_pro === true || (freeTests === 0 && totalTests > 0));
  const requiresPro = (isSeriesPro && !hasFreeTests) || (freeTests === 0 && totalTests > 0);
  ...
  const stageText = Array.isArray(stageNames) && stageNames.length > 0
    ? stageNames.filter(Boolean).join(', ')
    : (stageName || '');

  return (
    <div className="test-series-card flex-shrink-0 w-[310px] sm:w-[320px] h-full flex flex-col self-stretch">
      <Card variant="default" padding="p-0" hover className="h-full flex flex-col justify-between overflow-hidden group">
        <Link to={targetUrl} className="flex-1 flex flex-col p-4">
          ...
          <div className="flex items-center justify-between gap-2 mb-3">
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <div className="text-xl shrink-0">{series.icon || getCategoryEmoji(categoryName || category)}</div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              {isSeriesPro ? (
                <Badge variant="pro" size="xs" className="flex items-center gap-1"><Crown className="w-3 h-3" />PRO</Badge>
              ) : (
                <Badge variant="success" size="xs" className="font-bold">FREE</Badge>
              )}
              <span className="text-xs text-gray-500 dark:text-gray-400 font-medium ml-0.5">👥 {formatUserCount(displayUserCount)}</span>
            </div>
          </div>

          {/* Test Series Title with Stage Name in Brackets */}
          <div className="min-h-[2.75rem] mb-2.5 flex items-start">
            <h3 className="font-bold text-gray-900 dark:text-white text-sm leading-snug line-clamp-2 group-hover:text-brand-start dark:group-hover:text-indigo-400 transition-colors">
              {title} {stageText ? <span className="text-xs font-medium text-gray-500 dark:text-gray-400">({stageText})</span> : null}
            </h3>
          </div>
          ...
          <div className="flex items-center justify-between text-xs border-t border-b border-gray-100 dark:border-gray-700/80 py-2 mb-3">
            ...
            <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{freeTests || 0} Free</span>
            <span className="text-cyan-500 dark:text-cyan-400 font-medium">{languages?.join(', ') || 'Eng, Hin'}</span>
          </div>
          ...
          {showCategories && (
            <div className="space-y-1.5 mb-2 flex-1 min-h-[5.5rem] flex flex-col justify-start">
              {mainCategories.map((row, i) => (
                <div key={`${row.label}-${i}`} className="flex items-center justify-between text-xs py-1 px-2.5 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <span className="text-gray-700 dark:text-gray-300 truncate mr-2 font-medium">{row.label}</span>
                  <span className="font-bold text-gray-900 dark:text-white">{row.count}</span>
                </div>
              ))}
              {extraCategories.length > 0 && !expanded && (
                <button type="button" onClick={handleExpandClick} className="text-[11px] text-brand-start dark:text-indigo-400 font-semibold w-full text-left px-2 hover:underline">
                  +{extraCategories.length} more
                </button>
              )}
              {expanded && extraCategories.map((row, i) => (
                <div key={`${row.label}-${i}`} className="flex items-center justify-between text-xs py-1 px-2.5 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <span className="text-gray-700 dark:text-gray-300 truncate mr-2 font-medium">{row.label}</span>
                  <span className="font-bold text-gray-900 dark:text-white">{row.count}</span>
                </div>
        ...
        <div className="mt-auto px-4 pb-4 pt-1 flex items-center gap-2 border-t border-gray-50 dark:border-gray-700/40">
          {showProgress && progressPercentage > 0 && (
            <ProgressRing percentage={progressPercentage} size={28} strokeWidth={3}>
              <span className="text-[8px] font-bold">{progressPercentage}%</span>
            </ProgressRing>
          )}
