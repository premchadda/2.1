    ...
    <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide md:grid md:grid-cols-5 md:gap-3">
      {cards.map((card) => {
        ...
        return (
          ...
          >
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-md ${colors.iconBg} shrink-0`}
            >
              <Icon className={`w-4 h-4 ${colors.iconText}`} />
            </div>
            <div className="flex flex-col min-w-0">
              <span
                className={`text-base font-bold leading-tight ${colors.value}`}
              >
                {card.value}
              </span>
              <span
                className="text-[10px] leading-tight text-gray-500 dark:text-gray-400 truncate"
                title={card.label}
              >
                {card.label}
              </span>
