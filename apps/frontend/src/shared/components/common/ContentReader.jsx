...
export default function ContentReader({ isOpen, onClose, contentData }) {
  ...
  const checkBookmarkStatus = async () => {
    ...
    try {
      const { data } = await apiClient.get(`/api/bookmarks/check/${bookmarkItemType}/${contentData.id}`)
          ...
          <div className="p-8 md:p-12">
            ...
            {contentData?.allowComments && (
              <div className="mt-8 bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                <p className="text-gray-500 text-center py-8">Join the discussion</p>
                <div className="mt-4">
                  <a href={`/discussions/${contentData.id}`} className="inline-flex items-center px-4 py-2 bg-brand-start text-white rounded-lg hover:bg-brand-end transition-colors">
                    View Discussion
                  </a>
                </div>
              </div>
