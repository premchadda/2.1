...
export default function OnboardingWizard() {
  ...
  const handleFinish = async () => {
    const prefs = {
      ...
      selectedCategory: selectedCategory
        ? {
            ...
            name: selectedCategory.name || selectedCategory.label,
          }
        : null,
      selectedExam: selectedExam
        ? {
            id: selectedExam.id || selectedExam._id,
            name:
              selectedExam.fullName || selectedExam.name || selectedExam.title,
          }
        : null,
      selectedSubjects,
      dailyGoal,
    ...
    try {
      const examName =
        prefs.selectedExam?.name || prefs.selectedCategory?.name || "";
      if (examName) {
    ...
    toast.success("You're all set! Welcome to Trstprep.", {
      icon: "🎉",
      duration: 4000,
    });
    setIsOpen(false);
  ...
  const steps = [
    {
      ...
      icon: GraduationCap,
      desc: "Pick your target exam so we can personalize recommendations.",
    },
    {
      title: "Which subjects interest you?",
      icon: BookOpen,
      desc: "Select the subjects you want to focus on.",
    },
    {
      title: "Set your daily goal",
      icon: Target,
      desc: "How many questions do you want to practice each day?",
    },
  ];
  ...
  return createPortal(
    ...
    >
      <div
        className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] max-h-[90dvh] overflow-y-auto"
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-700">
          ...
          <button
            onClick={handleClose}
            className="w-8 h-8 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center text-gray-400 hover:text-gray-600"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 px-5 pt-4">
          {steps.map((_, i) => (
            <div
              key={i}
              className={`h-1.5 rounded-full transition-all ${i === step ? "flex-1 bg-indigo-600" : i < step ? "flex-1 bg-indigo-300" : "w-1.5 bg-gray-200 dark:bg-gray-600"}`}
            />
          ))}
        </div>
              ...
              <div>
                ...
                {filteredExams.length === 0 ? (
                  <p className="text-sm text-gray-400 py-4 text-center">
                    No exams found for this category. You can skip this step.
                  </p>
                ) : (
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {filteredExams.map((exam) => {
                      const isSelected =
                        selectedExam &&
                        String(selectedExam.id || selectedExam._id) ===
                          String(exam.id || exam._id);
                      return (
                        <button
                        ...
                        >
                          <span className="text-sm font-semibold text-gray-900 dark:text-white text-left">
                            {exam.fullName || exam.name || exam.title || "Exam"}
                          </span>
                          {isSelected && (
              ...
              {subjectOptions.map((subj) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-semibold text-left ${isSelected ? "text-indigo-700 dark:text-indigo-300" : "text-gray-700 dark:text-gray-300"}`}
                    >
                      {subj.label}
                    </span>
                    {isSelected && (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-left flex-1">
                      <p
                        className={`text-sm font-bold ${isSelected ? "text-indigo-700 dark:text-indigo-300" : "text-gray-900 dark:text-white"}`}
                      >
                        {opt.label}
                      </p>
          ...
          <div className="flex items-center gap-2">
            {step > 0 && (
              <button
                ...
                className="flex items-center gap-1 px-4 py-2 rounded-xl text-sm font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
              >
            ...
            {step < steps.length - 1 ? (
              <button
                onClick={() => setStep((s) => s + 1)}
                disabled={!canProceed}
                className="flex items-center gap-1 px-5 py-2 rounded-xl text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-30 disabled:cursor-not-allowed transition"
              >
              ...
              <button
                ...
                disabled={!canProceed}
                className="flex items-center gap-1 px-5 py-2 rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-30 disabled:cursor-not-allowed transition"
              >
