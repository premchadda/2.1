...
import { X, MessageSquare, Send, Trash2, Edit3, Clock, User } from 'lucide-react'
...
function CommentItem({ comment, currentUser, onEdit, onDelete }) {
  ...
  const [editDraft, setEditDraft] = useState(comment.text || comment.content || '')
  const editRef = useRef(null)
  const isOwn = currentUser && (comment.userId === currentUser.id || comment.userId === currentUser._id || comment.user?._id === currentUser.id || comment.user?._id === currentUser._id)
  ...
  const userName = comment.user?.name || comment.user?.fullName || comment.userName || 'Anonymous'
  const userAvatar = comment.user?.avatar || comment.user?.avatarUrl || comment.userAvatar
    ...
    <div className="group flex gap-2.5 py-3">
      ...
      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center shrink-0 border-2 border-white dark:border-gray-800 shadow-sm">
        {userAvatar ? (
          ...
          <span className="text-[10px] font-black text-white">{getInitials(userName)}</span>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="text-xs font-bold text-gray-900 dark:text-white truncate">{userName}</span>
        ...
        {editing ? (
          <div>
            ...
            <div className="flex gap-2 mt-1.5">
              <button onClick={handleSave} className="px-2 py-0.5 rounded bg-indigo-500 text-white text-[11px] font-bold hover:bg-indigo-600 transition-colors">
                Save
              </button>
              <button onClick={() => { setEditing(false); setEditDraft(comment.text || comment.content || '') }} className="px-2 py-0.5 rounded bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-300 text-[11px] font-bold hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors">
                Cancel
              </button>
            </div>
          </div>
...
function QuestionDiscussions({
  ...
  const handleEdit = async (commentId, newText) => {
    try {
      ...
      setComments(prev =>
        prev.map(c => (c.id === commentId || c._id === commentId) ? { ...c, text: newText, updatedAt: Date.now() } : c)
      )
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900 dark:text-white">Discussions</h2>
        ...
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 shrink-0">
          <div className="flex gap-2">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center shrink-0">
              {currentUser?.avatar ? (
                <img src={currentUser.avatar} alt={currentUser?.name || 'User avatar'} className="w-full h-full rounded-full object-cover" />
              ) : (
        ...
        {/* Comments List */}
        <div ref={listRef} className="flex-1 overflow-y-auto px-4 divide-y divide-gray-100 dark:divide-gray-700/50">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <button onClick={fetchComments} className="mt-2 text-xs text-indigo-500 hover:underline font-medium">
                Retry
              </button>
            </div>
          ) : comments.length === 0 ? (
