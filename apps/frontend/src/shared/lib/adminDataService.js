// Admin-only data service
// IMPORTANT: This module exposes privileged backend endpoints. It should
// ONLY be imported by the admin SPA. The user-facing frontend must not
// bundle these methods or import from this file.
//
// If you need to call an admin endpoint from the user app, route the
// request through the backend's authenticated proxy instead.
...
export const adminAPI = {
  ...
  // Public Leaderboard (read-only, but listed under admin namespace for grouping)
  getLeaderboard: (seriesId) => apiClient.get(`/api/leaderboards?testId=${seriesId}`),

  // Bookmarks (admin namespace)
