...
export const categorizeTestsList = (tests = []) => {
  ...
  tests.forEach((t) => {
    ...
    const sub = String(t.testCategoryName || t.test_category_name || t.subCategory || t.subcategory || t.sub_category || '')
...
export const getSeriesTestStats = (series = {}, tests = []) => {
  ...
  const baseTestCounts = toCountObject(series.testCounts || series.test_counts || {})

  // 2. Counts from individual active loaded tests
  const loadedCounts = activeSeriesTests.length > 0 ? categorizeTestsList(activeSeriesTests) : {}

  const testCounts = activeSeriesTests.length > 0 ? loadedCounts : baseTestCounts
  ...
  const sumCategoryCounts = Object.values(testCounts).reduce((sum, c) => sum + (Number(c) || 0), 0)
  const rawTotal = Number(series.totalTests ?? series.total_tests ?? 0)
  const totalTests = activeSeriesTests.length > 0 ? activeSeriesTests.length : (rawTotal > 0 ? rawTotal : sumCategoryCounts)
