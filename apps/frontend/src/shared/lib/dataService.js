...
import { mapTestSeriesToFrontend, mapTestToFrontend, mapQuestionToFrontend } from '../types/index.js'
import { apiClient, ValidationError, AuthenticationError } from './apiClient.js'
...
export { DataError, NetworkError, ValidationError, AuthenticationError, NotFoundError, apiClient, fetchFromAPI, api, isCancel } from './apiClient.js'
...
export { pypAPI, getPypCategories, getPypCategoryExams, getPypExamPapers, getPypExamInsights } from './pypAPI.js'
...
class DataService {
  ...
  async search(query, type = 'all', options = {}) {
    ...
    return this.fetchWithCache(key, async () => {
      const response = await apiClient.get('/api/search', { params: { q: query, type }, signal })
      return response.data?.data || response.data || []
    }, {
      ttl: 60000,
      ...cacheOptions
