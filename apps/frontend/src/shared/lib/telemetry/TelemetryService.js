...
class TelemetryService {
  ...
  async flush() {
    ...
    try {
      ...
      console.error('[TelemetryService] Failed to flush events, putting back in queue:', err);
  ...
  scheduleRetry() {
    ...
    const delay = Math.min(30000, 2000 * Math.pow(2, this.retryCount)) + Math.random() * 1000;
   ...
   */
  flushSync() {
    const offlineEventsCount = this.offlineQueue ? this.offlineQueue.length() : 0;
    if (this.queue.length === 0 && offlineEventsCount === 0) return;

    const events = [...(this.offlineQueue ? this.offlineQueue.dequeueAll() : []), ...this.queue];
    this.queue = [];
  ...
  async flushOfflineEvents() {
    if (!this.isRunning || !this.offlineQueue || !navigator.onLine || this.isFlushing) return;
    ...
    try {
      ...
      // Update OfflineQueue
      if (remainder.length > 0) {
        localStorage.setItem(this.offlineQueue.storageKey, JSON.stringify(remainder));
      } else {
  ...
  async sendHeartbeat() {
    ...
    try {
      const res = await apiClient.post(`/api/attempt/${this.attemptId}/heartbeat`, heartbeatData);
      const { attemptStatus, serverTime } = res.data || {};
      
      if (attemptStatus && attemptStatus !== 'active') {
        console.warn(`[TelemetryService] Heartbeat reported attempt status: ${attemptStatus}. Stopping telemetry.`);
        if (this.onViolation) {
  ...
  getMetrics() {
    ...
    const offlineDropped = this.offlineQueue ? this.offlineQueue.getDroppedCount() : 0;
    
    let oldestEventTime = null;
    const allEvents = [...(this.offlineQueue ? this.offlineQueue.getAll() : []), ...this.queue];
  ...
  handleContextMenu(e) {
    this.lastActivity = Date.now();
    this.logEvent('context_menu', { clientX: e.clientX, clientY: e.clientY }, 'low');
