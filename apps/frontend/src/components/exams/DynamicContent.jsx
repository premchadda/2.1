...
export default function DynamicContent({ yearlyData, updates }) {
  ...
  const importantDates = [
    { label: 'Notification Date', date: yearlyData?.notificationDate, icon: Bell },
    { label: 'Application Start', date: yearlyData?.applicationStart, icon: Calendar },
    { label: 'Application End', date: yearlyData?.applicationEnd, icon: Calendar },
    { label: 'Exam Date (Start)', date: yearlyData?.examDateStart, icon: Calendar },
        ...
        <div className="space-y-3">
          {importantDates.map((item, index) => (
            <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
              ...
              <span className={`font-medium ${item.date ? 'text-gray-900' : 'text-gray-400'}`}>
      ...
      {yearlyData?.vacancies > 0 && (
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-green-600 font-medium">Total Vacancies</p>
              <p className="text-3xl font-bold text-green-700">{yearlyData.vacancies.toLocaleString()}</p>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          {yearlyData?.officialWebsite && (
            ...
            >
              <span className="text-gray-700 group-hover:text-indigo-700">Official Website</span>
          ...
          {yearlyData?.brochureUrl && (
            <a
              ...
              className="flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 transition-colors group"
            >
              <span className="text-gray-700 group-hover:text-indigo-700">Official Brochure</span>
              <ExternalLink className="w-4 h-4 text-gray-400 group-hover:text-indigo-600" />
            </a>
          )}
          {(!yearlyData?.officialWebsite && !yearlyData?.brochureUrl) && (
            <p className="text-gray-500 text-sm">No official links available for this year.</p>
          )}
        </div>
      </div>
