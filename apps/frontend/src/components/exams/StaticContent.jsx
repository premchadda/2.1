  ...
  const sections = [
    {
      ...
      title: 'Exam Overview',
      icon: BookOpen,
      content: exam?.description || yearlyData?.overview || 'No overview available.'
    },
    {
      id: 'eligibility',
        ...
        <div className="space-y-3">
          {yearlyData.eligibility.education && (
            <div>
              ...
              <p className="text-gray-600">{yearlyData.eligibility.education}</p>
            </div>
          )}
          ...
          {yearlyData.eligibility.nationality && (
            <div>
              ...
              <p className="text-gray-600">{yearlyData.eligibility.nationality}</p>
            </div>
          )}
          {yearlyData.eligibility.experience && (
            <div>
              <span className="font-semibold text-gray-900">Experience:</span>
              <p className="text-gray-600">{yearlyData.eligibility.experience}</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          {yearlyData.examPattern.tiers.map((tier, index) => (
            <div key={index} className="bg-gray-50 rounded-lg p-4">
              <h4 className="font-semibold text-gray-900 mb-2">{tier.name || `Tier ${tier.tier}`}</h4>
  ...
  return (
    <div className="space-y-6">
      {sections.map(section => (
        <section key={section.id} id={section.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
