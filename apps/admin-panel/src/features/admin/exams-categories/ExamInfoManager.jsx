...
import { Plus, Edit, Trash2, X, Save, FileText, ExternalLink, BookOpen, Clock, Zap, Layers, ChevronRight, Check, User, Calendar, Award } from 'lucide-react'
...
// Step configuration
// NOTE: `timeline`, `shortcuts`, and `layers` steps were removed because they
// had no form content — clicking them rendered a blank panel. Re-add when the
// forms for those steps are implemented.
const STEPS = [
...
export default function ExamInfoManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        response = await apiClient.put(`/admin/exam-info/${editingId}`, examData)
      } else {
  ...
  const getCategoryInfo = (categoryId) => {
    return categories.find(c => c.categoryId === categoryId || c.id === categoryId)
  }

  const examsByCategory = categories.reduce((acc, cat) => {
    acc[cat.categoryId || cat.id] = examInfoList.filter(
      exam => exam.categoryId === cat.categoryId || exam.categoryId === cat.id
    ).sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0))
    return acc
  }, {})

  const filteredExams = activeTab 
    ? examInfoList.filter(exam => exam.categoryId === activeTab).sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0))
    : []
  ...
  const isStepComplete = (stepId) => {
    switch (stepId) {
      case 'basic': return formData.title && formData.examId && formData.categoryId
      case 'details': return formData.description || formData.fullName
      case 'eligibility': return formData.eligibility || formData.ageLimit
      case 'dates': return formData.notification
      case 'process': return formData.syllabus
  ...
  const hasChanges = () => {
    return formData.title || formData.examId || formData.description || 
           formData.eligibility || formData.ageLimit || formData.syllabus
  }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Exam Manager</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Manage exam information with detailed configuration</p>
        </div>
        <button
          onClick={() => {
          ...
          {tabs.map((tab) => (
            ...
            >
              ...
              <span className={`ml-1 px-2 py-0.5 text-xs font-bold rounded-full ${
                activeTab === tab.id ? 'bg-indigo-100 text-indigo-700 dark:text-indigo-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'
              }`}>
                {tab.count}
              </span>
            </button>
        ...
        <div className="p-4">
          {filteredExams.length === 0 ? (
            <div className="text-center py-12 bg-gradient-to-b from-gray-50 to-white rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700">
              ...
              <h3 className="text-gray-900 dark:text-white font-semibold mb-1">No exams yet</h3>
              <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">Add your first exam to get started</p>
              <button
                onClick={() => {
                  setFormData(prev => ({ ...prev, categoryId: activeTab }))
                  setCurrentStep('basic')
                  ...
                  <div className="flex items-center gap-4 p-4">
                    ...
                    <div className="flex-shrink-0">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                        ...
                        <span className="text-2xl">{getCategoryInfo(exam.categoryId)?.icon || '📋'}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-mono text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-1.5 py-0.5 rounded">#{exam.id}</span>
                        <h3 className="font-bold text-gray-900 dark:text-white truncate">{exam.title}</h3>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{exam.fullName || exam.description || 'No description'}</p>
                      <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-400 dark:text-gray-500">
                        <span className="font-mono bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">{exam.examId}</span>
                        {exam.seriesId && <span className="text-purple-600 dark:text-purple-400">Series: {exam.seriesId}</span>}
                        {exam.displayOrder !== undefined && <span>Order: {exam.displayOrder}</span>}
                        {exam.year && <span className="text-amber-600 dark:text-amber-400">{exam.year}</span>}
                      </div>
                    </div>
                    ...
                    <div className="hidden lg:flex items-center gap-4">
                      <div className="text-center px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg">
                        <div className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{exam.eligibility ? '✓' : '-'}</div>
                        <div className="text-[9px] text-indigo-400 dark:text-indigo-500">Eligibility</div>
                      </div>
                      <div className="text-center px-3 py-1.5 bg-purple-50 rounded-lg">
                        <div className="text-sm font-bold text-purple-600 dark:text-purple-400">{exam.ageLimit ? '✓' : '-'}</div>
                        <div className="text-[9px] text-purple-400 dark:text-purple-500">Age Limit</div>
                      </div>
                      <div className="text-center px-3 py-1.5 bg-pink-50 dark:bg-pink-900/20 rounded-lg">
                        <div className="text-sm font-bold text-pink-600 dark:text-pink-400">{exam.syllabus ? '✓' : '-'}</div>
                        <div className="text-[9px] text-pink-400 dark:text-pink-500">Syllabus</div>
                      </div>
<div className="text-center px-3 py-1.5 bg-amber-50 dark:bg-amber-900/20 rounded-lg flex items-center gap-1">
                <div className="text-sm font-bold text-amber-600 dark:text-amber-400">{exam.notification ? '✓' : '-'}</div>
                {exam.notification && <ExternalLink className="w-3 h-3 text-amber-400 dark:text-amber-500" />}
                <div className="text-[9px] text-amber-400 dark:text-amber-500">Dates</div>
              </div>
                    </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden shadow-2xl flex">
            {/* Side Navigation */}
            <div className="w-48 bg-gradient-to-b from-indigo-600 to-purple-700 p-4 flex flex-col">
              <div className="mb-6">
                ...
                {editingId && (
                  <p className="text-xs text-white/50 mt-1">Record: {editingId}</p>
                )}
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                        isActive ? 'bg-indigo-600 text-white' : isComplete ? 'bg-white dark:bg-gray-800/30' : 'bg-white dark:bg-gray-800/10'
                      }`}>
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col">
              <div className="flex-1 overflow-y-auto p-6">
                ...
                {currentStep === 'basic' && (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Exam ID *</label>
                        <input
                          type="text"
                      ...
                      <div>
                        ...
                        >
                          ...
                          {categories.map(cat => (
                            <option key={cat.id} value={cat.categoryId || cat.id}>
                              {cat.icon} {cat.label}
                            </option>
                          ))}
                        </select>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Test Series ID</label>
                        <input
                          type="text"
                ...
                {currentStep === 'details' && (
                  <div className="space-y-4">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Exam Details</h3>
                    <div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notification Details</label>
                      <textarea
                ...
                {currentStep === 'eligibility' && (
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Educational Qualification</label>
                      <textarea
                ...
                {currentStep === 'dates' && (
                  <div className="space-y-4">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Important Dates</h3>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">All Important Dates</label>
                      <textarea
                        value={formData.notification}
                        onChange={(e) => setFormData({ ...formData, notification: e.target.value })}
                        rows={8}
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 bg-amber-50 dark:bg-amber-900/20 p-2 rounded-lg">Tip: Enter each date on a new line</p>
                  </div>
                )}
                ...
                {currentStep === 'process' && (
                  <div className="space-y-4">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Selection Process & Syllabus</h3>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Syllabus / Exam Pattern</label>
                      <textarea
              ...
              {/* Footer */}
              <div className="flex items-center justify-between px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-100">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 dark:bg-gray-700 text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    ← Back
                  </button>
                  ...
                  <button
                    type="submit"
                    className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:shadow-lg font-medium transition-all text-sm ml-2"
                  >
