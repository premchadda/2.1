...
import {
  Shield, Search, Filter, Download, RefreshCw, Eye, ChevronLeft, ChevronRight,
  Calendar, User, FileText, AlertCircle, X, Clock, Database, Layers
} from 'lucide-react'
...
export default function AuditTrailManager() {
  ...
  const fetchData = useCallback(async (signal) => {
    try {
      ...
      const [logsRes, statsRes] = await Promise.allSettled([
        adminAPI.apiClient.get(`/admin/audit-logs?${params.toString()}`, { signal }),
      ...
      if (logsRes.status === 'fulfilled') {
        ...
        const fetchedLogs = Array.isArray(payload?.data) ? payload.data : Array.isArray(payload) ? payload : []
        setLogs(fetchedLogs)
        setPagination(payload?.pagination || { page: currentPage, limit: 50, total: fetchedLogs.length, totalPages: 1 })
      } else {
      ...
      if (statsRes.status === 'fulfilled') {
        const statsPayload = statsRes.value?.data?.data || statsRes.value?.data || null
        setStats(statsPayload)
      }

      if (logsRes.status === 'rejected' && statsRes.status === 'rejected') {
        setError('Unable to reach the audit logs service. Please check your admin privileges or backend connection.')
      }
    } catch (err) {
      if (signal?.aborted) return
  ...
  const handleExport = () => {
    ...
    const rows = [
      ['ID', 'Action', 'Table/Resource', 'Record ID', 'User', 'Email', 'IP Address', 'Timestamp'],
      ...logs.map(log => [
        log.id ?? '',
        log.action ?? '',
    ...
    exportToCSV(`audit_trail_${new Date().toISOString().split('T')[0]}_page_${currentPage}`, rows)
    toast.success('Audit trail exported successfully')
  }

  const formatAction = (action) => {
    if (!action) return 'Action'
    return String(action).replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
  }
  ...
  if (loading && logs.length === 0) {
    return (
      <div className="p-12 flex flex-col items-center justify-center min-h-96 text-center space-y-4">
        ...
        <p className="text-sm font-bold text-gray-500 dark:text-gray-400">Loading audit trail records...</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Tamper-evident operational audit trail capturing all administrative mutations and data changes.
          </p>
        </div>
      ...
      {error && (
        <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 flex items-start gap-3">
          ...
          <div className="text-xs space-y-1">
            <h4 className="font-extrabold text-rose-900 dark:text-rose-200">Audit Logs Error</h4>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-gray-400 text-xs font-bold uppercase tracking-wider">Total Actions</span>
          ...
          <div className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mt-2">
            {formatNumber(stats?.summary?.total_logs ?? pagination.total ?? logs.length)}
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-gray-400 text-xs font-bold uppercase tracking-wider">Entities Tracked</span>
          ...
          <p className="text-xs text-gray-500 mt-1">Database tables & endpoints</p>
        </div>

        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-gray-400 text-xs font-bold uppercase tracking-wider">Unique Actions</span>
          ...
          <p className="text-xs text-gray-500 mt-1">CRUD & auth operation types</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        {logs.length === 0 ? (
          <div className="p-16 text-center text-gray-400 space-y-3">
            ...
            <p className="font-bold text-base text-gray-700 dark:text-gray-300">No audit log records found</p>
            <p className="text-xs text-gray-400">Try adjusting your search query or action filter.</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-xs">
                <thead className="bg-gray-50/80 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700 text-gray-500 dark:text-gray-400 uppercase tracking-wider font-bold sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left">Administrator / User</th>
                  ...
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-gray-50/70 dark:hover:bg-gray-700/30 transition-colors">
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-bold border ${getActionColor(log.action)}`}>
            ...
            <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between flex-wrap gap-2">
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Showing <span className="font-bold text-gray-900 dark:text-white">{logs.length}</span> of{' '}
                <span className="font-bold text-gray-900 dark:text-white">{formatNumber(pagination.total || logs.length)}</span> records
              </p>
              <div className="flex items-center gap-2">
                <button
      ...
      {/* Log Detail Modal */}
      {selectedLog && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4" onClick={() => setSelectedLog(null)}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] overflow-hidden border border-gray-100 dark:border-gray-700 animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900/30">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-extrabold text-sm text-gray-900 dark:text-white">Audit Event Details</h3>
              </div>
              <button onClick={() => setSelectedLog(null)} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl text-gray-400 hover:text-gray-600">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-5 overflow-y-auto max-h-[calc(85vh-130px)] space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3 bg-gray-50 dark:bg-gray-900/40 p-4 rounded-xl border border-gray-100 dark:border-gray-700/60">
                <div>
                  ...
                  <p className="font-bold text-gray-900 dark:text-white mt-0.5">{formatAction(selectedLog.action)}</p>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase">Table / Entity</span>
                  <p className="font-bold font-mono text-gray-900 dark:text-white mt-0.5">{selectedLog.table_name || selectedLog.tableName || selectedLog.entity_type || 'N/A'}</p>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase">Record ID</span>
                  <p className="font-bold font-mono text-gray-900 dark:text-white mt-0.5">{selectedLog.record_id || selectedLog.recordId || selectedLog.entity_id || 'N/A'}</p>
                </div>
                <div>
                  ...
                  <p className="font-bold text-gray-900 dark:text-white mt-0.5">
                    {selectedLog.timestamp || selectedLog.created_at ? new Date(selectedLog.timestamp || selectedLog.created_at).toLocaleString() : 'N/A'}
                  </p>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase">User</span>
                  <p className="font-bold text-gray-900 dark:text-white mt-0.5">{selectedLog.user_name || selectedLog.userName || 'System'}</p>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase">IP Address</span>
                  <p className="font-bold font-mono text-gray-900 dark:text-white mt-0.5">{selectedLog.ip_address || selectedLog.ipAddress || '127.0.0.1'}</p>
                </div>
              </div>

              {/* Old Data */}
              {(selectedLog.old_data || selectedLog.oldData || selectedLog.old_values) && (
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Previous State (Before)</span>
                  <pre className="p-3 bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/40 rounded-xl text-[11px] font-mono text-rose-800 dark:text-rose-300 overflow-x-auto">
                    {JSON.stringify(selectedLog.old_data || selectedLog.oldData || selectedLog.old_values, null, 2)}
                  </pre>
                </div>
              )}

              {/* New Data */}
              {(selectedLog.new_data || selectedLog.newData || selectedLog.new_values) && (
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">New State (After)</span>
                  <pre className="p-3 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/40 rounded-xl text-[11px] font-mono text-emerald-800 dark:text-emerald-300 overflow-x-auto">
                    {JSON.stringify(selectedLog.new_data || selectedLog.newData || selectedLog.new_values, null, 2)}
                  </pre>
                </div>
              )}
            </div>
