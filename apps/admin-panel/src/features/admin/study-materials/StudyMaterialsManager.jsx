...
const SUBJECT_PALETTE = ['#6366f1', '#8b5cf6', '#ec4899', '#f59e0b', '#10b981', '#06b6d4', '#3b82f6', '#ef4444']
...
const SubjectTreeNode = ({ subject, level = 0, onEdit, onDelete, onAddChild, expandedNodes, toggleExpand, allSubjects }) => {
        ...
        <div className="p-4 sm:p-5 flex flex-wrap sm:flex-nowrap items-center justify-between gap-4">
          <div className="flex items-center gap-3.5 min-w-0 flex-1">
            ...
            <div
              ...
              style={{
                backgroundColor: (subject.color || paletteColor(subject.name)) + '18',
                borderColor: (subject.color || paletteColor(subject.name)) + '30',
                borderWidth: '1px'
              }}
            >
              {subject.icon || '📚'}
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {hasChildren ? (
                  <span className="text-xs px-2.5 py-0.5 rounded-full font-semibold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/50 flex items-center gap-1">
                    ...
                    {children.length} {children.length === 1 ? 'sub-subject' : 'sub-subjects'}
                  </span>
                ) : (
...
function SubjectsPanel() {
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirmOnce({
      title: 'Delete Subject',
      message: 'Are you sure you want to delete this subject? ALL CHILD SUBJECTS WILL ALSO BE DELETED RECURSIVELY!',
      danger: true
    })
    if (!confirmed) return
    try {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-0.5">
            Root subjects and nested sub-subjects strictly ordered by Display Order
          </p>
        </div>
        <button
      ...
      <div className="space-y-3">
        {rootSubjects.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700">
            ...
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">No Subjects Found</h3>
            <p className="text-gray-500 dark:text-gray-400 mt-2">Add subjects to build your curriculum hierarchy</p>
          </div>
        ) : (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-700">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={resetForm} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition" aria-label="Close">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center pt-6">
                    <input
                      ...
                      id="isActive"
                    />
                    <label htmlFor="isActive" className="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
                      Active Status
                    </label>
                  </div>
                </div>
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight flex items-center gap-3">
              <div className="p-2.5 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 rounded-xl shadow-sm border border-indigo-100 dark:border-indigo-900/50">
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-md p-1.5 rounded-2xl border border-gray-200/80 dark:border-gray-700/80 shadow-sm flex items-center gap-2 overflow-x-auto">
          {tabs.map(tab => {
            const Icon = tab.icon
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2.5 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 whitespace-nowrap cursor-pointer ${
                  isActive
                    ? 'bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-600 text-white shadow-md shadow-indigo-500/25 scale-[1.01]'
                    : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700/60'
                }`}
              >
                <Icon className={`w-4 h-4 transition-transform duration-200 ${isActive ? 'text-white scale-110' : 'text-gray-400 dark:text-gray-500'}`} />
                <span>{tab.label}</span>
                {isActive && (
      ...
      <div className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200/70 dark:border-gray-700/70 shadow-sm min-h-[500px]">
