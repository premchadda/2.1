...
import {
  CreditCard, Search, RefreshCw, CheckCircle2, XCircle, Clock, ArrowLeft, ArrowRight, X
} from 'lucide-react'
...
export default function PaymentsManager() {
  ...
  const fetchTransactions = useCallback(async (pageToFetch, searchToUse, statusToUse) => {
    try {
      setLoading(true)
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Transaction monitoring & refunds</p>
        </div>
        <button
          onClick={() => { fetchStats(); fetchTransactions(page, search, statusFilter) }}
        ...
        {statCards.map((card) => {
          ...
          return (
            <div key={card.label} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-3">{card.label}</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">Try adjusting filters or search</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Gateway</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {transactions.map((txn) => {
                  ...
                  return (
                    <tr key={txn.id || txn._id || txn.gatewayPaymentId} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                      <td className="px-4 py-4">
                        <div className="flex flex-col">
                          <span className="font-medium text-gray-900 dark:text-white">{txn.userName || 'Unknown'}</span>
                          <span className="text-xs text-gray-500 dark:text-gray-400">{txn.userEmail || `User #${txn.userId ?? '—'}`}</span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <span className="font-semibold text-gray-900 dark:text-white">{formatCurrency(txn.amount, txn.currency)}</span>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm text-gray-700 dark:text-gray-300">{txn.gateway || '—'}</span>
                          {txn.gatewayPaymentId && (
                            <span className="text-xs text-gray-400 dark:text-gray-500 font-mono">{txn.gatewayPaymentId}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${STATUS_STYLES[txn.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'}`}>
        ...
        {!loading && transactions.length > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-700">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Showing <span className="font-medium">{transactions.length}</span> of <span className="font-medium">{total}</span> transactions
            </p>
    ...
    <div className="p-3 sm:p-4 md:p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setRefundTarget(null)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded">
                  <X className="w-5 h-5" />
                </button>
              </div>

              <p className="text-gray-600 dark:text-gray-400 mb-4">
                Are you sure you want to refund this payment? This action will mark the payment as refunded and cannot be undone from this panel.
              </p>

              <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 space-y-2 mb-6">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">User</span>
                  <span className="font-medium text-gray-900 dark:text-white">{refundTarget.userName || 'Unknown'}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Amount</span>
                  <span className="font-semibold text-gray-900 dark:text-white">{formatCurrency(refundTarget.amount, refundTarget.currency)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Gateway</span>
                  <span className="text-gray-700 dark:text-gray-300">{refundTarget.gateway || '—'}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Reference</span>
                  <span className="text-gray-700 dark:text-gray-300 font-mono text-xs">{refundTarget.gatewayPaymentId || '—'}</span>
                </div>
              </div>
