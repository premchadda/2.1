...
import {
  Megaphone, Plus, Search, Calendar, Users, Gift, Tag, AlertCircle,
  ChevronLeft, ChevronRight, Percent, RefreshCw, Edit2, Trash2, ToggleLeft, ToggleRight
} from 'lucide-react'
                            ...
                            <div 
                              className={`h-full rounded-full transition-all ${
                                getUsagePercent(promotion.usedCount, promotion.usageLimit) >= 100 
                                  ? 'bg-red-500' 
                                  : getUsagePercent(promotion.usedCount, promotion.usageLimit) >= 75 
                                    ? 'bg-yellow-500' 
                                    : 'bg-green-500'
                              }`}
                              style={{ width: `${getUsagePercent(promotion.usedCount, promotion.usageLimit)}%` }}
                            />
                          </div>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-400 mb-6">
              Are you sure you want to delete "{showDeleteModal.title}"? This will deactivate the promo code immediately.
            </p>
