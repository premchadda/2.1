...
export default function CouponsManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCoupon) {
        await apiClient.put(`/admin/coupons/${editingCoupon._id}`, formData)
        toast.success('Coupon updated successfully')
      } else {
        await apiClient.post('/admin/coupons', { ...formData, usageCount: 0 })
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCoupons.map(coupon => {
          ...
          return (
            <div key={coupon._id} className={`bg-white rounded-lg shadow-sm border overflow-hidden ${
              !coupon.isActive || expired || exhausted ? 'opacity-60' : ''
            }`}>
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {coupon.applicablePlans?.map(plan => (
                    <span key={plan} className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full">
                ...
                <div className="flex items-center justify-between pt-3 border-t">
                  <div className="flex gap-2">
                    {expired && (
                      <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">Expired</span>
                    )}
                    {exhausted && (
                      <span className="px-2 py-1 bg-red-100 text-red-600 text-xs rounded-full">Exhausted</span>
                    )}
                    {!coupon.isActive && (
                      <span className="px-2 py-1 bg-yellow-100 text-yellow-600 text-xs rounded-full">Inactive</span>
                    )}
        ...
        {filteredCoupons.length === 0 && (
          <div className="col-span-full p-8 text-center text-gray-500 bg-white rounded-lg border">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {APPLICABLE_PLANS.map(plan => (
                      <label key={plan.value} className="flex items-center gap-2 px-3 py-2 border rounded-lg cursor-pointer hover:bg-gray-50">
                        <input
                          type="checkbox"
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {getRootCategoryNames().map(cat => (
                      <label key={cat} className="flex items-center gap-2 px-3 py-2 border rounded-lg cursor-pointer hover:bg-gray-50">
                        <input
                          type="checkbox"
