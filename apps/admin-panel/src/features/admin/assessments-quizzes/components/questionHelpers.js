...
export const getSeriesId = (series) => series?._id ?? series?.id ?? series?.public_id ?? null
export const getTestId = (test) => test?._id ?? test?.id ?? test?.public_id ?? null
export const getQuestionId = (question) => question?._id ?? question?.id ?? question?.public_id ?? null
export const getTestSeriesIdFromTest = (test = {}) =>
  test.testSeriesId ?? test.test_series_id ?? test.seriesId ?? test.series_id ?? null
export const getTestIdFromQuestion = (question = {}) => question.testId ?? question.test_id ?? question.testid ?? null
...
export const getSeriesExamCategoryId = (series = {}) => series.category ?? series.category_id ?? series.examCategoryId ?? series.exam_category_id ?? null
export const getStageIdFromTest = (test = {}) => test.stageId ?? test.stage_id ?? test.tierId ?? test.tier_id ?? null
export const getSectionId = (section = {}) => section._id ?? section.id ?? null
export const getSectionName = (section = {}) => section.name || section.title || section.label || ''
...
export const recordMatchesTestCategory = (record, refs) => valueMatchesRefs(getTestCategoryValues(record), refs)
