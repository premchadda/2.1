...
function SystemHealthMonitor() {
  ...
  const refreshHealth = useCallback(async (showToast = false) => {
    ...
    try {
      ...
      const memoryUsage = heapTotal > 0 ? Math.min(100, Math.round((heapUsed / heapTotal) * 100)) : 0
      ...
      const alerts = []
      if (memoryUsage > 80) {
        alerts.push({ id: 1, severity: 'error', message: 'High memory usage detected (>80%)', time: new Date().toLocaleTimeString(), resolved: false })
      } else if (memoryUsage > 60) {
        alerts.push({ id: 2, severity: 'warning', message: 'Memory usage above 60%', time: new Date().toLocaleTimeString(), resolved: false })
      }
      if (cpuUsage > 80) {
        alerts.push({ id: 3, severity: 'error', message: 'High CPU usage detected (>80%)', time: new Date().toLocaleTimeString(), resolved: false })
      }
      if (diskUsage > 80) {
        alerts.push({ id: 4, severity: 'warning', message: 'Disk usage above 80%', time: new Date().toLocaleTimeString(), resolved: false })
      }
      if (data.database !== 'connected') {
        alerts.push({ id: 5, severity: 'error', message: 'Database connection lost!', time: new Date().toLocaleTimeString(), resolved: false })
      }
      ...
      setHealth({
        ...
        services: [
          { name: 'API Server', status: 'operational', latency: `${latency}ms`, uptime: formatUptime(data.uptime) },
          {
            name: 'Database',
          ...
          { name: 'Static Assets', status: 'operational', latency: 'n/a', uptime: 'n/a' }
        ],
        resources: {
          cpu: { usage: cpuUsage, trend: cpuUsage > 60 ? 'up' : 'stable' },
          memory: { usage: memoryUsage, trend: memoryUsage > 75 ? 'up' : 'stable' },
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{health.responseTime != null ? `${health.responseTime}ms` : 'N/A'}</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between mb-4">
            ...
            <p className="text-3xl font-bold text-gray-900 dark:text-white">{health.requestsPerMin != null ? `${health.requestsPerMin}/min` : 'N/A'}</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{health.alerts.length}</p>
              ...
              <div key={key}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-gray-900 dark:text-white">{data.usage}%</span>
                    {data.trend === 'up' ? (
                ...
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full transition-all ${
                      data.usage > 80 ? 'bg-red-500' : data.usage > 60 ? 'bg-amber-500' : 'bg-green-500'
                    }`}
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div className="divide-y divide-gray-200 dark:divide-gray-700">
          {health.alerts.map((alert) => (
            <div key={alert.id} className={`p-4 border-l-4 ${getAlertColor(alert.severity)}`}>
