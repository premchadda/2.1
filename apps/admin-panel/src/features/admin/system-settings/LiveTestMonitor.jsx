...
const StatusBadge = ({ status }) => (
  <span
    className={`text-xs px-2.5 py-0.5 rounded-full font-medium ${statusColors[status] || "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300"}`}
  >
    {status || "unknown"}
  </span>
);
  ...
  useEffect(() => {
    ...
    const unsubPresence = on("live_test:presence", (data) => {
      ...
      setParticipants((prev) => {
        ...
        if (isNew) {
          pushActivity(
            "join",
            `User #${data.userId || "Guest"} joined Test #${data.testId || "Unknown"}`,
          );
        }
        return {
          ...prev,
    ...
    const unsubLeft = on("live_test:participant_left", (data) => {
      ...
      setParticipants((prev) => {
        ...
        if (existing) {
          pushActivity(
            "leave",
            `User #${existing.userId || "Guest"} disconnected from Test #${existing.testId || ""}`,
          );
        }
        const next = { ...prev };
    ...
    const unsubSubmitted = on("live-test:attempt_submitted", (data) => {
      if (data?.testId) {
        pushActivity(
          "submit",
          `Candidate submitted attempt for Test #${data.testId}`,
        );
      }
    });
    ...
    emit("admin:live-tests:subscribe", (response) => {
      if (response?.success) {
        ...
        toast.error(
          response?.message || "Failed to subscribe to live-test monitoring",
        );
      }
    });
    ...
    return () => {
      ...
      unsubSubmitted?.();
      try {
        emit("admin:live-tests:unsubscribe");
      } catch {
    ...
    return entries
      .filter((e) => {
        ...
        if (searchQuery.trim()) {
          ...
          const matchAttempt = String(e.attemptId || "")
            .toLowerCase()
            .includes(q);
          const matchUser = String(e.userId || "")
            .toLowerCase()
            .includes(q);
          const matchTest = String(e.testId || "")
            .toLowerCase()
            .includes(q);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-2">
            <Radio className="w-6 h-6 text-cyan-600 animate-pulse" /> Live Test
            Monitor
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Real-time candidate telemetry and presence tracking via WebSocket
            stream.
          </p>
        </div>
        <div className="flex items-center gap-2.5 flex-wrap">
          <span
            className={`text-xs px-3 py-1.5 rounded-xl font-bold flex items-center gap-1.5 border shadow-sm ${wsConnected ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-800" : "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/30 dark:text-rose-400 dark:border-rose-800"}`}
          >
            {wsConnected ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
            ...
            <div className="p-4 border-b border-gray-100 dark:border-gray-700/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gray-50/50 dark:bg-gray-900/30">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  aria-label="Search by User ID, Attempt ID, or Test ID"
                  value={searchQuery}
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 pt-3 pr-1">
              {activityLog.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center p-4 text-gray-400">
                  ...
                  <p className="text-xs">
                    Listening for candidate presence events...
                  </p>
                </div>
              ) : (
                activityLog.map((item) => (
                  ...
                  >
                    <span
                      className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${item.type === "join" ? "bg-emerald-500" : item.type === "submit" ? "bg-blue-500" : item.type === "leave" ? "bg-amber-500" : "bg-indigo-500"}`}
                    />
