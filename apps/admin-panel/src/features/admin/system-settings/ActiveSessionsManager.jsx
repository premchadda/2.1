...
} from "../../../shared/hooks/useAdminQueries";
...
const DeviceBadge = ({ type }) => {
  ...
  return (
    <span
      className={`text-xs px-2 py-0.5 rounded-full font-medium ${colors[type] || colors.Desktop}`}
    >
      {type || "Desktop"}
    </span>
  );
...
export default function ActiveSessionsManager() {
  ...
  const handleRevokeAll = async (userId, userName) => {
    const confirmed = await confirm({
      title: "Confirm",
      message: `Revoke ALL sessions for ${userName || "this user"}? They will be fully logged out.`,
    });
    if (!confirmed) return;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          <div
            key={label}
            className={`${bg} p-4 rounded-xl border border-white dark:border-gray-700 shadow-sm`}
          >
      ...
      {loading && sessions.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl border shadow-sm p-6 sm:p-8 flex flex-col items-center">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border shadow-sm text-center py-10">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {sessions.map((session) => {
              ...
              const browser =
                (di.browser && di.browser.toLowerCase() !== "unknown"
                  ? di.browser
                  : null) ||
                (session.browser && session.browser.toLowerCase() !== "unknown"
                  ? session.browser
                  : null) ||
                "Unknown Browser";
              const os =
                (di.os && di.os.toLowerCase() !== "unknown" ? di.os : null) ||
                  ...
                  <div className="px-4 py-3 flex items-center gap-4">
                    ...
                    <div
                      className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${
                        devType === "Mobile" || devType === "mobile"
                          ? "bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400"
                          : devType === "Tablet" || devType === "tablet"
                  ...
                  {isExp && (
                    <div className="px-4 pb-4 pt-1 bg-gray-50/60 dark:bg-gray-700/30 border-t border-dashed border-gray-200 dark:border-gray-600">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                        ...
                        <InfoCell
                          label="Country"
                          value={session.country || "Unknown Country"}
                        />
                        <InfoCell
                          label="Login Time"
                          value={fmtDate(
                        ...
                        {(session.userAgent || session.user_agent) && (
                          <div className="col-span-2 md:col-span-4">
                            <p className="text-gray-400 mb-0.5 uppercase tracking-wide font-semibold">
                              User Agent
                            </p>
                        ...
                        (session.ipAddress || session.ip_address) !==
                          ...
                          <div className="mt-2 flex items-center gap-2">
                            ...
                            <p className="text-xs text-amber-700 dark:text-amber-400">
                              If this IP looks suspicious, revoke the session
                              immediately.
                            </p>
                            <button
                              onClick={() => handleRevokeAll(userId, userName)}
...
function InfoCell({ label, value, mono = false }) {
  return (
    <div>
      <p className="text-gray-400 uppercase tracking-wide font-semibold text-[10px] mb-0.5">
        {label}
      </p>
      <p
        className={`text-gray-700 dark:text-gray-300 truncate ${mono ? "font-mono text-[10px]" : ""}`}
      >
        {value || "—"}
      </p>
    </div>
