...
const SearchBox = ({
  ...
  return (
    ...
    >
      ...
      <input
        type="text"
        aria-label={props["aria-label"] || placeholder || "Search"}
        placeholder={placeholder}
        value={value}
        onChange={onChange}
        className={`w-full pl-10 pr-10 md:pr-4 py-2.5 md:py-4 rounded-xl md:rounded-2xl border-0 focus:ring-4 focus:ring-white/20 text-gray-800 text-sm md:text-lg outline-none transition-all ${inputClass} ${showAction ? "pr-10 md:pr-10" : ""}`}
        {...props}
      />
