...
export function AuthProvider({ children }) {
    ...
    const checkAuth = async (attempt = 0) => {
      ...
      try {
        ...
        const isAuthError =
          ...
          err?.status === 401;
        const status =
          err?.response?.status || err?.status || (isAuthError ? 401 : null);
        const transient =
  ...
  const hasProPass = () =>
    !user?.isProUser
      ? false
      : user?.proPassExpiry
        ? new Date(user.proPassExpiry) > new Date()
        : true;
  ...
  useEffect(() => {
    ...
    const cleanupAdmin = on("admin:stats_update", () =>
      getQueryClient()?.invalidateQueries({ queryKey: ["admin", "stats"] }),
    );
    const cleanupLiveAttempts = on("live-test:attempt_submitted", () =>
