...
import { ValidationError, AuthenticationError, DataError } from '@trstprep/shared-config'
...
function extractListPayload(response, endpoint) {
  ...
  if (list === null) {
    throw new DataError(`Malformed response from ${endpoint}: expected a data array`, 'MALFORMED_RESPONSE', raw)
  }
  return list
}
...
class DataService {
  ...
  async fetchWithCache(key, fetchFn, options = {}) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        lastError = error
        if (import.meta.env.DEV) console.error(`Fetch attempt ${attempt} failed for ${key}:`, error)
  ...
  async search(query, type = 'all', options = {}) {
    ...
    return this.fetchWithCache(key, async () => {
      const response = await apiClient.get(ENDPOINTS.SEARCH, { params: { q: query, type } })
  ...
  async getTestsBySeriesId(seriesId, options = {}) {
    ...
    return this.fetchWithCache(key, async () => {
      ...
      const data = extractListPayload(response, ENDPOINTS.TESTS_BY_SERIES(seriesId))
      return data.map(mapTestToFrontend)
    }, options)
  }
  ...
  async getTestById(id, options = {}) {
    ...
    return allTests.find(t =>
      t._id === id || t.id === id || t._id === idStr || t.id === idStr ||
      t.slug === id || t._id === idNum || t.id === idNum ||
      String(t._id) === idStr || String(t.id) === idStr
    ) || null
  }
  ...
  async getQuestionsByTestId(testId, options = {}) {
    ...
    return this.fetchWithCache(key, async () => {
      ...
      const data = extractListPayload(response, ENDPOINTS.QUESTIONS_BY_TEST(testId))
      return data.map(mapQuestionToFrontend)
    }, options)
  }
  ...
  async handleMutation(mutationFn, affectedEndpoints = []) {
    try {
      ...
      affectedEndpoints.forEach(endpoint => this.clearCacheForEndpoint(endpoint))
