...
export const settingsSchema = z.object({
  ...
  security: z.object({
    ...
    ipWhitelist:        z.array(z.string().regex(/^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$|^(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}$/, 'Invalid IP address format')).optional(),
  ...
  payment: z.object({
    ...
    currency:           z.string().length(3, 'Currency must be a 3-letter code').regex(/^[A-Z]{3}$/, 'Must be a valid 3-letter ISO currency code'),
