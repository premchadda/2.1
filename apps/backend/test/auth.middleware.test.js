...
jest.unstable_mockModule(
  "../src/infrastructure/database/postgres-helpers.js",
  () => ({
    pool: {
      query: (...args) => {
        if (
          global.dbHelpers &&
          global.dbHelpers.pool &&
          typeof global.dbHelpers.pool.query === "function"
        ) {
    ...
    dbHelpers: {
      findById: (...args) => {
        if (
          global.dbHelpers &&
          typeof global.dbHelpers.findById === "function"
        ) {
  ...
  describe("isUserAdminRequest", () => {
    ...
    it("should return false for unauthenticated admin origin without verified user", async () => {
      const { isUserAdminRequest } =
        await import("../src/middleware/auth.middleware.js");
      expect(
        isUserAdminRequest({ headers: { origin: "http://localhost:3002" } }),
      ).toBe(false);
    });
    ...
    it("should return false for regular user or empty request", async () => {
      ...
      expect(
        isUserAdminRequest({ user: { role: "user", isAdmin: false } }),
      ).toBe(false);
    });
  });
});
