...
import { parseCSVBuffer, parseJSONBuffer, parseSpreadsheetBuffer, extractQuestionsFromParsedJSON } from "../../services/import/enhancedImporter.js";
...
const syncTestStats = async (testId) => {
  ...
  try {
    ...
    if (statsResult.rows.length > 0) {
      ...
      logger.info(`[Stats Sync] Test ${testId} updated: ${q_count} questions, ${total_marks} marks`);
    } else {
      await pool.query(
...
const syncTestQuestionsJunction = async (testId, questionId, payload = {}) => {
  ...
  try {
    ...
    logger.warn(`[Questions] Error syncing test_questions junction for test ${testId} question ${questionId}: ${err.message}`);
  }
};
...
const VALID_QUESTION_FIELDS = new Set([
  ...
  "explanation", "explanationHi", "explanation_hi", "hint",
  "options", "optionsHi", "options_hi",
...
const normalizeQuestionPayloadForDb = (data = {}, index = 0) => {
  ...
  const rawQNum = payload.question_number ?? payload.questionNumber ?? payload.q_order ?? payload.id;
  const qNum = Number(rawQNum);
  payload.question_number = Number.isFinite(qNum) && qNum > 0 ? Math.floor(qNum) : index + 1;
  ...
  } else if (typeof rawCorrOpt === "string" && (rawCorrOpt.includes(",") || rawCorrOpt.includes("-") || isNaN(Number(rawCorrOpt)))) {
    payload.correct_option = rawCorrOpt;
  } else if (rawCorrOpt !== undefined && rawCorrOpt !== null) {
    const corrOpt = Number(rawCorrOpt);
    payload.correct_option = Number.isFinite(corrOpt) && corrOpt >= 0 ? corrOpt : 0;
  }
...
router.get("/questions/stats", responseCache("admin-questions-stats", 60), async (req, res) => {
  try {
...
router.get("/questions", async (req, res) => {
  try {
    ...
    const isPracticeFilter = req.query.isPractice ?? (req.query.category === "practice" ? "true" : undefined);
    ...
    if (reqTestId) {
      ...
      if (!/^\d+$/.test(String(reqTestId))) {
        const { rows } = await pool.query('SELECT id FROM tests WHERE public_id = $1 LIMIT 1', [reqTestId]);
        if (rows[0]) numericTestId = rows[0].id;
      }
      whereClauses.push(`(q.test_id = $${paramIdx} OR EXISTS (SELECT 1 FROM test_questions tq WHERE tq.test_id = $${paramIdx} AND tq.question_id = q.id))`);
      params.push(numericTestId);
      paramIdx++;
    }

    const reqSeriesId = req.query.testSeriesId || req.query.seriesId || req.query.test_series_id || req.query.series_id;
    if (reqSeriesId) {
      let numericSeriesId = reqSeriesId;
      if (!/^\d+$/.test(String(reqSeriesId))) {
        const { rows } = await pool.query('SELECT id FROM test_series WHERE public_id = $1 LIMIT 1', [reqSeriesId]);
        if (rows[0]) numericSeriesId = rows[0].id;
      }
      whereClauses.push(`(q.series_id = $${paramIdx} OR q.test_series_id = $${paramIdx})`);
      params.push(numericSeriesId);
      paramIdx++;
    }

    if (search) {
      whereClauses.push(`(q.question_text ILIKE $${paramIdx} OR q.explanation ILIKE $${paramIdx})`);
      params.push(`%${search}%`);
      paramIdx++;
    }
    ...
    const orderByClause = reqTestId ? 'COALESCE(q.question_number, q.id) ASC' : 'q.id DESC';
...
router.delete("/questions/bulk", async (req, res) => {
  try {
    ...
    if (ids.length > 200) {
      return res.status(400).json({ success: false, message: 'Cannot delete more than 200 questions at once' })
    }
...
router.get("/questions/export", async (req, res) => {
  try {
    ...
    if (!res.headersSent) res.status(500).json({ success: false, message: sanitizeErrorMessage(error) });
    else { logger.error("Export questions error:", error); res.end(); }
  }
});
...
router.post("/questions/:id/predict-difficulty", async (req, res) => {
  try {
    ...
    if (req.query.apply === 'true' || req.body?.apply) {
      await dbHelpers.updateById('questions', req.params.id, { difficulty: prediction.level });
...
router.post("/questions/bulk/predict-difficulty", async (req, res) => {
  try {
    ...
    if (ids.length > 200) {
      return res.status(400).json({ success: false, message: "Cannot process more than 200 questions at once" });
    }
    const results = [];
    for (const id of ids) {
      try {
        const question = await dbHelpers.findById('questions', id);
        if (!question) { results.push({ id, error: 'not found' }); continue; }
        const prediction = await predictQuestionDifficulty(question);
        if (apply) await dbHelpers.updateById('questions', id, { difficulty: prediction.level });
...
router.post("/questions", async (req, res) => {
  try {
    ...
    if (payload.category && !payload.category_id) {
      ...
      if (!isNaN(parsedCatId) && String(parsedCatId) === String(payload.category).trim()) {
    ...
    if (testId) {
      await syncTestQuestionsJunction(testId, newQuestion.id || newQuestion._id, payload);
...
router.post("/questions/practice", async (req, res) => {
  try {
    ...
    logger.error("[Practice Questions] Error creating question:", error.message);
    res.status(500).json({ success: false, message: sanitizeErrorMessage(error) });
  }
});
...
router.put("/questions/:id", async (req, res) => {
  try {
    ...
    try {
      await questionBuilderService.createVersion(req.params.id, updated, req.user?.id);
    } catch (vErr) {
      logger.warn(`[Questions] Could not snapshot question version for ${req.params.id}: ${vErr.message}`);
    }

    const testId = updated.testId || updated.test_id;
    if (testId) {
      await syncTestQuestionsJunction(testId, updated.id || req.params.id, payload);
      await syncTestStats(testId);
    }
...
router.delete("/questions/:id", async (req, res) => {
  try {
    ...
    if (testId) {
      try {
        await pool.query('DELETE FROM test_questions WHERE test_id = $1 AND question_id = $2', [testId, req.params.id]);
      } catch (jErr) {
        logger.warn(`[Questions] Error removing junction row for question ${req.params.id}: ${jErr.message}`);
      }
      await syncTestStats(testId);
    }
...
router.post("/questions/bulk", bulkQuestionUpload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const fileInfo = req.file ? { originalname: req.file.originalname, mimetype: req.file.mimetype, size: req.file.size } : null;
    ...
    if (req.file?.buffer) {
      ...
      } else if (extension === ".json" || req.file.mimetype?.includes("json")) {
      ...
      try {
        ...
        logger.warn('[Questions Bulk Debug] Failed to parse stringified req.body.questions JSON:', parseErr.message);
      }
    ...
    if (!Array.isArray(normalizedRows) || normalizedRows.length === 0) {
      ...
      logger.warn(`[Questions Bulk Debug] No valid question rows found: ${JSON.stringify(debugInfo)}`);
    ...
    logger.info(`[Questions Bulk Debug] Found ${normalizedRows.length} question rows to process`);

    const config = {
      testId: req.body.testId || null,
      category: req.body.category || null,
      categoryId: req.body.categoryId || req.body.category_id || null,
      isPractice: req.body.isPractice === 'true' || req.body.isPractice === true || req.body.category === 'practice',
    ...
    const mappedWithValidation = (await Promise.all(
      normalizedRows.map(async (row, index) => {
        ...
        if (!payload || !payload.questionText || payload.options.filter(Boolean).length < 2) {
          questionSkipDetails.push({ row: index + 1, reason: "Missing question text or fewer than 2 options", sample: row });
          return null;
        }
    ...
    if (validTestIds.size > 0) {
      ...
      if (numericTestIds.length > 0) {
        const existingTests = await pool.query("SELECT id FROM tests WHERE id = ANY($1::int[])", [numericTestIds]);
        const existingTestIds = new Set(existingTests.rows.map((r) => Number(r.id)));
    ...
    const mapped = mappedWithValidation.map(({ payload }) => {
      ...
      if (currentTestId && invalidTestIds.includes(String(currentTestId))) {
        questionSkipDetails.push({ row: payload.rowIndex, reason: `Invalid testId: ${currentTestId}` });
        payload.testId = null;
    ...
    if (mapped.length === 0) {
      logger.warn('[Questions Bulk Debug] All rows failed validation', { skipDetails: questionSkipDetails });
    ...
    for (let i = 0; i < mapped.length; i += CHUNK_SIZE) {
      ...
      const inserted = await dbHelpers.insertMany("questions", chunk.map((q, idx) => normalizeQuestionPayloadForDb(q, i + idx)));
    ...
    try {
      await dbHelpers.insertOne('import_logs', {
        ...
        metadata: { started_at: startTime, completed_at: new Date().toISOString() }
      });
    } catch (logErr) {
