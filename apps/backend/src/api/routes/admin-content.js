...
router.get("/subjects-list", asyncHandler(async (req, res) => {
  ...
  const materials = await dbHelpers.find("studyMaterials", { isActive: true }, limit, offset);
...
router.post("/study-materials", asyncHandler(async (req, res) => {
  const { title, name, slug, description, order, display_order, displayOrder, isActive, is_active } = req.body;
  const materialTitle = (title || name || "").trim();
  if (!materialTitle) {
    return res.status(400).json({ success: false, message: "Title or name is required" });
  }
  const materialSlug = (slug || materialTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')).trim();
...
router.put("/study-materials/:id", asyncHandler(async (req, res) => {
...
router.put("/study-materials/:id/restore", asyncHandler(async (req, res) => {
...
router.get("/study-materials/:id", asyncHandler(async (req, res) => {
  const material = await dbHelpers.findById("studyMaterials", req.params.id);
  if (!material) {
    return res
      .status(404)
...
router.get("/chapters", asyncHandler(async (req, res) => {
  const smId = req.query.studyMaterialId || req.query.study_material_id || req.query.subjectId || req.query.subject_id;
...
router.get("/topics", responseCache("admin-topics", 60), asyncHandler(async (req, res) => {
  ...
  topics.sort((a, b) => (a.orderIndex || a.order_index || a.order || 0) - (b.orderIndex || b.order_index || b.order || 0));
  res.json({ success: true, data: topics });
}));

router.post("/chapters", asyncHandler(async (req, res) => {
  const { studyMaterialId, study_material_id, subjectId, subject_id, title, name, slug, orderIndex, order } = req.body;
  const smId = studyMaterialId || study_material_id || subjectId || subject_id;
  const chapterName = (name || title || "").trim();

  if (!smId || !chapterName || !slug) {
    return res.status(400).json({
      success: false,
      message: "studyMaterialId (or subjectId), title (or name), and slug are required",
    });
  }
...
router.get("/subject-videos", asyncHandler(async (req, res) => {
...
router.post("/subject-videos", asyncHandler(async (req, res) => {
  const body = req.body;
...
router.put("/subject-videos/:id", asyncHandler(async (req, res) => {
  const updated = await dbHelpers.updateById(
    "subjectVideos",
    req.params.id,
    req.body,
...
router.delete("/subject-videos/:id", asyncHandler(async (req, res) => {
...
router.put("/subject-videos/:id/reorder", asyncHandler(async (req, res) => {
...
router.get("/subject-pdfs", asyncHandler(async (req, res) => {
  const { studyMaterialId, chapterId } = req.query;
  const query = { isActive: true };
  if (studyMaterialId) query.studyMaterialId = studyMaterialId;
  if (chapterId) query.chapterId = chapterId;
...
router.post("/subject-pdfs", asyncHandler(async (req, res) => {
  const body = req.body;
  const studyMaterialId = body.studyMaterialId || body.study_material_id;
  const chapterId       = body.chapterId       || body.chapter_id       || null;
  const topicId         = body.topicId         || body.topic_id         || null;
...
router.put("/subject-pdfs/:id", asyncHandler(async (req, res) => {
  const updated = await dbHelpers.updateById(
    "subjectPdfs",
    req.params.id,
    req.body,
...
router.delete("/subject-pdfs/:id", asyncHandler(async (req, res) => {
...
router.put("/subject-pdfs/:id/reorder", asyncHandler(async (req, res) => {
  const { order } = req.body;
...
router.get("/topic-tests", asyncHandler(async (req, res) => {
  const { studyMaterialId, chapterId } = req.query;
  const query = { isActive: true };
  if (studyMaterialId) query.studyMaterialId = studyMaterialId;
  if (chapterId) query.chapterId = chapterId;
...
router.post("/topic-tests", asyncHandler(async (req, res) => {
  const body = req.body;
  const studyMaterialId = body.studyMaterialId || body.study_material_id;
...
router.put("/topic-tests/:id", asyncHandler(async (req, res) => {
  const updated = await dbHelpers.updateById("topicTests", req.params.id, {
    ...req.body,
    updatedAt: new Date().toISOString(),
  });
  if (!updated) {
    return res.status(404).json({ success: false, message: "Topic test not found" });
  }
  res.json({ success: true, data: updated });
}));

router.delete("/topic-tests/:id", asyncHandler(async (req, res) => {
...
router.put("/topic-tests/:id/reorder", asyncHandler(async (req, res) => {
  const { order } = req.body;
  const updated = await dbHelpers.updateById("topicTests", req.params.id, {
    displayOrder: order,
  });
