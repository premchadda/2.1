...
router.get("/", optionalAuth, async (req, res) => {
  try {
    const groups = await dbHelpers.find("communityGroups", { isActive: true });
    const posts = await dbHelpers.find("communityPosts", { isActive: true });
    res.json({
      success: true,
      data: {
    ...
    logger.error("[community] overview failed:", error?.message || error);
    res.status(500).json({
      success: false,
...
router.post("/groups", protect, async (req, res) => {
  try {
    const { name, description, tags, isPublic, maxMembers, category } =
      req.body;
...
router.put("/groups/:id", protect, async (req, res) => {
  try {
    const { name, description, tags, isPublic, maxMembers, category, rules } =
      req.body;
    ...
    const isAdmin =
      group.admins?.some((id) => idsMatch(id, req.user.id)) || req.user.isAdmin;
    if (!isAdmin) {
      return res
        .status(403)
        .json({ success: false, message: "Not authorized to edit this group" });
    }
...
router.post("/groups/:id/leave", protect, async (req, res) => {
  try {
    ...
    const isLeavingAdmin = admins.some(
      (id) => String(id) === String(req.user.id),
    );
    if (isLeavingAdmin && admins.length <= 1 && updatedMembers.length > 0) {
      return res.status(400).json({
        success: false,
        message:
          "Cannot leave as you are the only admin. Transfer admin role first.",
      });
    }

    await dbHelpers.updateById("communityGroups", req.params.id, {
      members: updatedMembers,
      admins: isLeavingAdmin
        ? admins.filter((id) => String(id) !== String(req.user.id))
        : admins,
    ...
    if (group.isPublic === false) {
      if (!req.user) {
        return res
          ...
          .json({
            success: false,
            message: "Login required to view private group posts",
          });
      }
      ...
      if (!isMember) {
        return res
          .status(403)
          .json({ success: false, message: "Join the group to view posts" });
      }
    }
...
router.post("/groups/:id/posts", protect, async (req, res) => {
  try {
    ...
    const isMember = group.members?.some(
      (mid) => String(mid) === String(req.user.id),
    );
    if (!isMember) {
      return res
        .status(403)
        .json({ success: false, message: "Join the group to post" });
    }
  ...
  try {
    ...
    if (existingVote) {
      if (existingVote.voteType === voteType) {
        return res
          .status(400)
          .json({ success: false, message: `Already ${voteType}d` });
      }
      // Change vote
      await dbHelpers.updateById("communityVotes", existingVote.id, {
        voteType,
      });
    } else {
    ...
    const allVotes = await dbHelpers.find("communityVotes", {
      postId: req.params.postId,
    });
