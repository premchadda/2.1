...
const sanitizeDoubtInput = (data) => {
  const sanitized = {};
  if (data.title)
    sanitized.title = sanitizeInput(String(data.title).substring(0, 200));
  if (data.description)
    sanitized.description = sanitizeInput(
      String(data.description).substring(0, 5000),
    );
  if (data.category)
    sanitized.category = sanitizeInput(String(data.category).substring(0, 50));
  if (data.tags && Array.isArray(data.tags)) {
    sanitized.tags = data.tags
      .slice(0, 10)
      .map((tag) => sanitizeInput(String(tag).substring(0, 50)));
  }
  return sanitized;
};
  ...
  try {
    ...
    const doubtsWithCounts = await Promise.all(
      doubts.map(async (doubt) => {
        const replies = await dbHelpers.find("doubtReplies", {
...
router.post("/", protect, contentCreationLimiter, async (req, res) => {
  try {
    ...
    const sanitizedData = sanitizeDoubtInput({
      title,
      description,
      category,
      tags,
...
router.put("/:id/reply/:replyId/accept", protect, async (req, res) => {
  try {
    ...
    const allReplies = await dbHelpers.find("doubtReplies", {
      doubtId: doubt._id || doubt.id,
    });
    for (const reply of allReplies) {
      if (reply.isAccepted) {
        await dbHelpers.updateById("doubtReplies", reply._id || reply.id, {
          isAccepted: false,
        });
      }
    }
    ...
    if (
      !replyToAccept ||
      !idsMatch(replyToAccept.doubtId, doubt._id || doubt.id)
    ) {
      return res
        .status(404)
        .json({ success: false, message: "Reply not found for this doubt" });
    }
