...
router.get("/curriculum/orphans", asyncHandler(async (req, res) => {
...
router.post("/topics", asyncHandler(async (req, res) => {
  const { name, title, slug, chapterId, chapter_id, orderIndex, order, isActive, is_active } = req.body;
  const topicName = (name || title || "").trim();
  if (!topicName) {
    return res.status(400).json({ success: false, message: "Topic name is required" });
  }
  const chId = chapterId || chapter_id;
  const topicSlug = (slug || topicName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')).trim();
  const payload = {
    name: topicName,
    slug: topicSlug,
...
router.put("/topics/:id", asyncHandler(async (req, res) => {
  const { name, title, slug, chapterId, chapter_id, orderIndex, order, isActive, is_active } = req.body;
  const payload = {};
  if (name !== undefined || title !== undefined) payload.name = (name || title || "").trim();
  ...
  if (orderIndex !== undefined || order !== undefined) payload.order_index = Number(orderIndex ?? order ?? 0);
  if (isActive !== undefined || is_active !== undefined) payload.is_active = Boolean(isActive ?? is_active);
  payload.updated_at = new Date().toISOString();

  const updated = await dbHelpers.updateById("topics", req.params.id, payload);
  if (!updated) {
    return res
      .status(404)
...
router.delete("/subjects/:id", asyncHandler(async (req, res) => {
  const deleted = await dbHelpers.softDelete("subjects", req.params.id, req.user.id);
...
router.get("/subject-parts", asyncHandler(async (req, res) => {
  try {
    const parts = await dbHelpers.find("subjectParts", { isActive: true });
    res.json({ success: true, data: parts || [] });
  } catch (err) {
    logger.warn("[Curriculum] subject_parts table query failed, returning empty list:", err.message);
    res.json({ success: true, data: [] });
  }
}));

router.post("/subject-parts", asyncHandler(async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Subject parts storage unavailable" });
  }
}));

router.put("/subject-parts/:id", asyncHandler(async (req, res) => {
  try {
    const updated = await dbHelpers.updateById("subjectParts", req.params.id, {
      ...req.body,
      updatedAt: new Date().toISOString(),
    });
...
router.delete("/subject-parts/:id", asyncHandler(async (req, res) => {
  try {
    const deleted = await dbHelpers.softDelete("subjectParts", req.params.id, req.user.id);
...
router.put("/units/:id", asyncHandler(async (req, res) => {
  ...
  if (!updated) return res.status(404).json({ success: false, message: "Unit not found" });
  res.json({ success: true, data: updated });
}));

router.delete("/units/:id", asyncHandler(async (req, res) => {
  const deleted = await dbHelpers.softDelete("units", req.params.id, req.user.id);
  if (!deleted) return res.status(404).json({ success: false, message: "Unit not found" });
  res.json({ success: true, message: "Unit moved to trash" });
}));

// ===== SUBTOPICS MANAGEMENT =====
router.get("/subtopics", asyncHandler(async (req, res) => {
  const subtopics = await dbHelpers.find("subtopics", { isActive: true });
  res.json({ success: true, data: subtopics });
}));

router.post("/subtopics", asyncHandler(async (req, res) => {
...
router.put("/subtopics/:id", asyncHandler(async (req, res) => {
  const updated = await dbHelpers.updateById("subtopics", req.params.id, {
    ...req.body,
    updatedAt: new Date().toISOString(),
  });
  if (!updated) return res.status(404).json({ success: false, message: "Subtopic not found" });
  res.json({ success: true, data: updated });
}));

router.delete("/subtopics/:id", asyncHandler(async (req, res) => {
  const deleted = await dbHelpers.softDelete("subtopics", req.params.id, req.user.id);
  if (!deleted) return res.status(404).json({ success: false, message: "Subtopic not found" });
  res.json({ success: true, message: "Subtopic moved to trash" });
}));

// ===== PASSAGES MANAGEMENT =====
router.get("/passages", asyncHandler(async (req, res) => {
  try {
