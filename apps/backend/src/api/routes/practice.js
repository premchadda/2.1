 ...
 */
async function pickPracticeQuestionIds({ subjectId, chapterId, topicId, difficulty, mode, count, userId, testId }) {
...
async function bumpStreak(userId) {
  ...
  const r = await pool.query(`SELECT user_id, current_streak, longest_streak, last_practice_date, total_sessions, total_questions, total_correct FROM practice_streaks WHERE user_id = $1`, [userId])
...
router.get('/chapters/:chapterId/topics', protect, async (req, res) => {
  try {
    ...
    const PRACTICE_SETS = [
      { id: 'quick', label: 'Quick Practice', description: '10 questions · Mixed', count: 10, difficulty: 'mixed', icon: '⚡' },
      { id: 'easy', label: 'Easy Set', description: '15 easy questions', count: 15, difficulty: 'easy', icon: '🟢' },
      { id: 'medium', label: 'Medium Set', description: '15 medium questions', count: 15, difficulty: 'medium', icon: '🟡' },
      { id: 'hard', label: 'Hard Set', description: '10 hard questions', count: 10, difficulty: 'hard', icon: '🔴' },
      { id: 'full', label: 'Full Practice', description: '25 questions · All levels', count: 25, difficulty: 'mixed', icon: '🎯' },
    ]
...
router.post('/sessions', protect, async (req, res) => {
  try {
    const userId = req.user.id
    const { examId, subjectId, chapterId, topicId, mode = 'learn', difficulty = 'mixed', targetCount = 20, timeLimitSec, testId } = req.body

    // Cap targetCount
    const count = Math.min(Math.max(parseInt(targetCount, 10) || 20, 1), 200)

    const questionIds = await pickPracticeQuestionIds({
      subjectId, chapterId, topicId, difficulty, mode, count, userId, testId,
    })
...
router.get('/sessions/:id', protect, async (req, res) => {
  try {
    const r = await pool.query(`SELECT id, user_id, exam_id, subject_id, chapter_id, topic_id, mode, difficulty, target_count, time_limit_sec, questions_json, current_index, correct_count, wrong_count, skipped_count, started_at, last_active_at, completed_at, is_active, is_deleted, deleted_at, deleted_by FROM practice_sessions WHERE id = $1 AND user_id = $2`, [req.params.id, req.user.id])
...
router.post('/sessions/:id/complete', protect, async (req, res) => {
  try {
    ...
    if (session.topicId) {
      await recordPracticeAnalytics(userId, session.id, { topic: session.topicId, subject: session.subjectId })
    }
...
router.post('/sessions/:id/questions/:idx/check', protect, async (req, res) => {
  try {
    ...
    let correctDelta = 0, wrongDelta = 0, skippedDelta = 0
...
router.post('/sessions/:id/questions/:idx/skip', protect, async (req, res) => {
  try {
    ...
    let correctDelta = 0, wrongDelta = 0, skippedDelta = 1
    const had = prev.rows[0]
    if (had) {
      if (had.is_skipped) {
        skippedDelta = 0
...
router.get('/mistakes', protect, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: safe, total: total.rows[0]?.c || 0, page, limit })
  } catch (err) {
    res.status(500).json({ success: false, error: sanitizeErrorMessage(err) })
  }
})
 ...
 */
router.get('/dashboard', protect, responseCache("practice-dashboard", 60), async (req, res) => {
  try {
    const userId = req.user.id

    // Run all independent queries in parallel
    const todayStr = new Date().toISOString().slice(0, 10)
    const [streakR, todayR, activeR, mistakesR, bookmarksR, masteryR, weakR] = await Promise.all([
      pool.query(`SELECT user_id, current_streak, longest_streak, last_practice_date, total_sessions, total_questions, total_correct FROM practice_streaks WHERE user_id = $1`, [userId]),
      pool.query(`
      ...
      pool.query(`
        ...
        ORDER BY started_at DESC LIMIT 1
      `, [userId]),
      pool.query(`SELECT COUNT(DISTINCT question_id)::int AS c FROM practice_answers WHERE user_id = $1 AND is_correct = false`, [userId]),
      pool.query(`SELECT COUNT(*)::int AS c FROM question_bookmarks WHERE user_id = $1`, [userId]),
      pool.query(`
...
router.get('/questions', protect, async (req, res) => {
  try {
    ...
    const result = await executePaginatedQuery(dbHelpers, 'questions',
      ['id', 'question_text', 'options', 'explanation', 'subject', 'topic', 'difficulty', 'language'],
...
router.get('/fundamentals/categories', protect, async (req, res) => {
  try {
    ...
    const categories = [
      { id: 'tables', title: 'Tables (1–30)', description: 'Master multiplication speed and mental calculation', icon: '🔢' },
      { id: 'squares', title: 'Squares (1–50)', description: 'Fast recall of square values', icon: '²' },
      { id: 'cubes', title: 'Cubes (1–30)', description: 'Fast recall of cube values', icon: '³' },
      { id: 'roots', title: 'Square & Cube Roots', description: 'Recognize perfect roots & estimations', icon: '√' },
      { id: 'fractions', title: 'Fractions ↔ % ↔ Decimals', description: 'Convert common fraction primitives', icon: '½' },
      { id: 'ratios', title: 'Ratio Primitives (a:b:c)', description: 'Equivalent ratios and quick scaling', icon: '⚖️' },
      { id: 'triplets', title: 'Mathematical Triplets', description: 'Pythagorean & common algebraic triplets', icon: '🔺' },
      { id: 'mental_math', title: 'Mental Shortcuts (×5, ×11, ÷25)', description: 'Speed calculation techniques', icon: '⚡' },
    ].map(cat => ({
      ...cat,
      mastery: masteryMap[cat.id] || { level: 1, score: 0, bestSpeedMs: 0, totalAttempts: 0 }
    }))
  ...
  try {
    ...
    for (let i = 0; i < parseInt(count, 10); i++) {
      if (category === 'tables') {
        ...
        questions.push({ id: `tb_${i}`, prompt: `${a} × ${b} = ?`, answer: String(a * b), options: [String(a * b), String(a * b + b), String(a * b - a), String((a + 1) * b)] })
      } else if (category === 'squares') {
        const n = Math.floor(Math.random() * 40) + 11
        questions.push({ id: `sq_${i}`, prompt: `${n}² = ?`, answer: String(n * n), options: [String(n * n), String((n+1)*(n+1)), String(n*n - 10), String((n-1)*(n-1))] })
      } else if (category === 'cubes') {
        const n = Math.floor(Math.random() * 20) + 5
        questions.push({ id: `cb_${i}`, prompt: `${n}³ = ?`, answer: String(n * n * n), options: [String(n * n * n), String((n+1)*(n+1)*(n+1)), String(n*n*n - 20)] })
      } else if (category === 'roots') {
        const n = Math.floor(Math.random() * 30) + 5
        const sq = n * n
        questions.push({ id: `rt_${i}`, prompt: `√${sq} = ?`, answer: String(n), options: [String(n), String(n + 2), String(n - 1), String(n + 5)] })
      } else if (category === 'fractions') {
        const pairs = [
          { f: '1/2', p: '50%' }, { f: '1/3', p: '33.33%' }, { f: '1/4', p: '25%' },
          { f: '1/5', p: '20%' }, { f: '1/6', p: '16.66%' }, { f: '1/8', p: '12.5%' },
          { f: '1/10', p: '10%' }, { f: '3/8', p: '37.5%' }, { f: '5/8', p: '62.5%' }
        ]
        const choice = pairs[Math.floor(Math.random() * pairs.length)]
        questions.push({ id: `fr_${i}`, prompt: `Convert ${choice.f} to Percentage`, answer: choice.p, options: [choice.p, '40%', '15%', '30%'] })
      } else {
        const a = Math.floor(Math.random() * 15) + 3
        const b = Math.floor(Math.random() * 10) + 2
        questions.push({ id: `gen_${i}`, prompt: `${a} + ${b} × 5 = ?`, answer: String(a + b * 5), options: [String(a + b * 5), String((a + b) * 5), String(a * 5 + b)] })
      }
    }
...
router.post('/fundamentals/submit', protect, async (req, res) => {
  try {
    ...
    const avgSpeedMs = totalQuestions > 0 ? Math.round(durationMs / totalQuestions) : 0
    ...
    res.json({ success: true, message: 'Fundamental drill recorded', level, accuracy })
  } catch (err) {
...
router.get('/questions/:id/explanations', protect, async (req, res) => {
  try {
    const { id } = req.params
    const qR = await pool.query(`SELECT id, question_text, explanation, options, correct_option FROM questions WHERE id = $1`, [id])
    if (!qR.rows.length) return res.status(404).json({ success: false, error: 'Question not found' })

    const q = qR.rows[0]

    const v2R = await pool.query(`SELECT explanation_text, explanation_visual, explanation_video, explanation_formula FROM question_explanations_v2 WHERE question_id = $1`, [id])
    const v2 = v2R.rows[0] || {}

    const textExplanation = v2.explanation_text || {
      concept: 'Key Concept & Definition',
      stepByStep: q.explanation || 'Step 1: Identify given variables. Step 2: Apply main identity or formula. Step 3: Calculate final answer.',
      shortcut: 'Exam Shortcut: Use option elimination or percentage ratio scaling to save 30 seconds.',
      commonMistake: 'Common Mistake: Forgetting to adjust for percentage change direction.'
    }

    const visualExplanation = v2.explanation_visual || {
      diagramUrl: null,
      svgContent: '<svg width="200" height="100"><rect width="200" height="100" fill="#f3f4f6"/><text x="20" y="55" font-family="sans-serif" font-size="14" fill="#374151">Visual Diagram Schema</text></svg>',
      animationSteps: ['Initial state representation', 'Transformation step', 'Final state']
    }

    const videoExplanation = v2.explanation_video || {
      videoUrl: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
      durationSeconds: 180,
      timestamps: [{ time: '0:30', title: 'Problem Setup' }, { time: '1:15', title: 'Shortcut Trick' }],
      transcript: 'In this question we look at successive percentage change using standard ratio method...'
    }

    const formulaExplanation = v2.explanation_formula || [
      { name: 'Percentage Change', formulaLatex: '\\text{Change}\\% = \\frac{\\text{Final} - \\text{Initial}}{\\text{Initial}} \\times 100', description: 'Standard formula for percentage increase or decrease' },
      { name: 'Successive Change', formulaLatex: 'a + b + \\frac{ab}{100}', description: 'Quick formula for two consecutive percentage changes' }
    ]
...
router.post('/questions/:id/approaches', protect, async (req, res) => {
  try {
    const { id } = req.params
    const { approachType = 'fastest', title, content, timeComplexity } = req.body

    const userR = await pool.query(`SELECT name FROM users WHERE id = $1`, [req.user.id])
    ...
    const r = await pool.query(`
      ...
      RETURNING *
    `, [id, req.user.id, authorName, approachType, title || 'Community Solution', content, timeComplexity || 'Standard'])
 ...
 */
router.post('/questions/:id/approaches/:approachId/upvote', protect, async (req, res) => {
  try {
...
router.get('/questions/:id/similar', protect, async (req, res) => {
  try {
    const { id } = req.params
    const qR = await pool.query(`SELECT subject_id, topic_id, difficulty FROM questions WHERE id = $1`, [id])
    const q = qR.rows[0] || {}
    ...
    res.json({ success: true, data: simR.rows.map(row => dbHelpers.toCamel(row)) })
  } catch (err) {
...
router.post('/vault/save', protect, async (req, res) => {
  try {
    const { questionId, saveReason = 'needs_revision', collectionName = 'Default', userNotes } = req.body
...
router.get('/vault/items', protect, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: r.rows.map(row => dbHelpers.toCamel(row)) })
  } catch (err) {
...
router.post('/ai/tutor', protect, async (req, res) => {
  try {
    ...
    const qR = await pool.query(`SELECT question_text, explanation, options FROM questions WHERE id = $1`, [questionId])
    const q = qR.rows[0]

    let responseText = ''
    if (promptType === 'hint') {
      responseText = '💡 **Hint**: Look at the relation between initial and final values. Express the successive change as a single ratio.'
    } else if (promptType === 'explain_simply') {
      responseText = '🧒 **Simple Explanation**: Imagine you have ₹100. If you increase it by 10%, it becomes ₹110. If you then decrease it by 10%, 10% of 110 is ₹11, so it becomes ₹99! That is a net loss of 1%.'
    } else if (promptType === 'another_method') {
      responseText = '⚡ **Alternative Method**: Use the formula $a + b + \\frac{ab}{100}$. Here $a = +10$, $b = -10$, so $10 - 10 - \\frac{100}{100} = -1\\%$.'
    } else {
