  ...
  try {
    ...
    if (searchType === "tests" || searchType === "all") {
      try {
        ...
        results.tests = testsRes.rows.map((row) =>
          toPublicTestDTO(dbHelpers.toCamel(row)),
        );
      } catch (err) {
    ...
    if (searchType === "series" || searchType === "all") {
      try {
        const seriesRes = await pool.query(
          `
          SELECT id, slug, title, category, description, image, thumbnail, icon, total_tests, free_tests, active_users, users_count, rating, tags, test_types, is_pro, price, difficulty, is_active, is_pinned
          FROM test_series 
          WHERE is_active = true 
    ...
    if (searchType === "study" || searchType === "all") {
      ...
      results.studyMaterials = materialsRes.rows.map((row) =>
