...
function validateSectionInput(body, isUpdate = false) {
  const errors = []
  const { name, difficulty, time_limit, marks_per_question, negative_marks } = body
  ...
  if (time_limit !== undefined && time_limit !== null) {
    ...
    if (isNaN(tl) || tl < 0) errors.push('time_limit must be a non-negative number')
  }

  if (marks_per_question !== undefined && marks_per_question !== null) {
    const mpq = Number(marks_per_question)
    if (isNaN(mpq) || mpq < 0) errors.push('marks_per_question must be a non-negative number')
  }

  if (negative_marks !== undefined && negative_marks !== null) {
    const nm = Number(negative_marks)
    if (isNaN(nm) || nm < 0) errors.push('negative_marks must be a non-negative number')
...
router.get('/batch', protect, admin, responseCache("admin-sections-batch", 60), async (req, res) => {
  try {
    ...
    const resolveId = async (table, value) => {
      if (!value) return null
      if (/^\d+$/.test(String(value))) return Number(value)
      const { rows } = await pool.query(`SELECT id FROM "${table}" WHERE public_id = $1 LIMIT 1`, [value])
      return rows[0]?.id || null
    }

    let resolvedTestSeriesId = testSeriesId ? await resolveId('test_series', testSeriesId) : null
    ...
    const batchData = await Promise.allSettled([
      ...
      examCategories: results[4].status === 'fulfilled' ? results[4].value : [],
...
router.get('/', protect, admin, responseCache("admin-sections", 60), async (req, res) => {
  try {
    ...
    if (testSeriesId && stageId) {
      where.push(`(d.test_series_id = ${pushParam(testSeriesId)} AND (d.stage_id = ${pushParam(stageId)} OR d.stage_id IS NULL))`)
    ...
    if (scope === 'templates') {
      where.push('d.test_id IS NULL AND d.test_series_id IS NULL AND d.stage_id IS NULL')
    } else if (scope === 'linking') {
      where.push('d.test_id IS NULL AND (d.test_series_id IS NOT NULL OR d.stage_id IS NOT NULL)')
    } else if (onlyLinked === 'true') {
      where.push('d.test_series_id IS NOT NULL OR d.stage_id IS NOT NULL OR d.test_id IS NOT NULL')
    }
...
router.post('/preset', protect, admin, async (req, res) => {
  try {
    ...
    if (!testId && !(testSeriesId && stageId)) {
      return res.status(400).json({ success: false, message: 'Either testId or (testSeriesId + stageId) is required' })
    }
    ...
    for (const section of presetSections) {
      ...
      if (existing) {
        if (testId && String(existing.test_id) === String(testId)) {
          results.skipped.push({ id: existing.id, name: canonicalName, alias: examAlias, reason: 'already linked' })
          continue
        }
        if (testId) {
          await pool.query(
          ...
          results.linked.push({ id: existing.id, name: canonicalName, alias: examAlias })
        } else {
          results.skipped.push({ id: existing.id, name: canonicalName, alias: examAlias, reason: 'already in scope' })
        }
      } else {
        ...
        `, [
          canonicalName, examAlias,
          testId || null, testSeriesId || null, stageId || null,
          section.description || '', section.duration || 15, section.passing_marks || 0,
          true, section.display_order || 0, section.marks_per_question ?? 2, section.negative_marks ?? 0.5,
          section.time_limit ?? 900, false, section.instructions || '', 'medium',
          false, false, section.expected_questions ?? 0, section.total_marks ?? 0,
          section.exam_stage || null, section.paper || null, section.session || null,
          section.section_code || null, section.is_qualifying || false
        ])
        results.created.push({ id: inserted[0].id, name: canonicalName, alias: examAlias })
      }
    }
...
router.post('/seed-templates', protect, admin, async (req, res) => {
  try {
    const TEMPLATES = [
      ...
      { name: 'English Language & Comprehension', display_order: 3, expected_questions: 25, total_marks: 50, marks_per_question: 2, negative_marks: 0.5, time_limit: 900 },
    ...
    if (toInsert.length === 0) {
      return res.json({ success: true, data: { created: 0, message: 'All templates already exist' } })
    }
...
router.post('/aliases', protect, admin, async (req, res) => {
  try {
    ...
    if (!canonical_name?.trim() || !alias_name?.trim()) {
      return res.status(400).json({ success: false, message: 'canonical_name and alias_name are required' })
    }
    const { rows } = await pool.query(
...
router.post('/', protect, admin, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      ...
      RETURNING *
    `, [
      name, category_id || null, test_id || null, test_series_id || null, stage_id || null,
      description || null,
      duration || 60, passing_marks || 0,
      ...
      is_qualifying || false, exam_alias || null
    ])
...
router.put('/:id', protect, admin, async (req, res) => {
  try {
    ...
    `, [
      ...
      instructions, difficulty, shuffle_questions, shuffle_options,
      expected_questions, total_marks, exam_stage, paper, session,
      section_code, is_qualifying, id, exam_alias || null
    ])
...
router.delete('/:id', protect, admin, async (req, res) => {
  try {
    ...
    const { rows: sectionRows } = await pool.query('SELECT id, name FROM test_sections WHERE id = $1', [id])
