...
router.get("/admin/list", async (req, res) => {
  try {
    ...
    const dbQuery = {};
    if (seriesId) dbQuery.seriesId = Number(seriesId) || seriesId;
    if (userId) dbQuery.userId = Number(userId) || userId;
    if (status) dbQuery.status = status;

    // Fetch enrollments bounded to 500 records max
    const enrollments = await dbHelpers.find("enrollments", dbQuery, 500);

    // Efficiently fetch only the users and series referenced by these enrollments
    const userIds = [
      ...new Set(enrollments.map((e) => e.userId).filter(Boolean)),
    ];
    const seriesIds = [
      ...new Set(enrollments.map((e) => e.seriesId).filter(Boolean)),
    ];

    const [userRows, seriesRows] = await Promise.all([
      userIds.length > 0
        ? pool
            .query(
              `SELECT id, name, email, phone, is_pro_user AS "isProUser", pro_pass_expiry AS "proPassExpiry", created_at AS "createdAt"
    ...
    const seriesMap = new Map(seriesRows.map((s) => [String(s.id), s]));
    ...
    for (const enrollment of enrollments) {
      ...
      const series = seriesMap.get(String(enrollment.seriesId));
      ...
      enrichedEnrollments.push({
        ...
        seriesName:
          series?.title || series?.name || `Series #${enrollment.seriesId}`,
        enrolledAt:
    ...
    if (search) {
      ...
      enrichedEnrollments = enrichedEnrollments.filter(
        (e) =>
    ...
    enrichedEnrollments.sort(
      (a, b) => new Date(b.enrolledAt || 0) - new Date(a.enrolledAt || 0),
    );

    const offset = (pageNum - 1) * limitNum;
    const paginated = enrichedEnrollments.slice(offset, offset + limitNum);
    ...
    res.json({
      ...
      pagination: {
        ...
        limit: limitNum,
        total: enrichedEnrollments.length,
        totalPages: Math.ceil(enrichedEnrollments.length / limitNum),
      },
    });
  } catch (error) {
    res
...
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    const allEnrollments = await dbHelpers.find("enrollments", {
      userId,
      isActive: true,
    });
    const allSeries = await dbHelpers.find("testSeries", { isActive: true });
    ...
    const userEnrollments = allEnrollments.map((enrollment) => {
      const series = allSeries.find(
        (s) => String(s.id || s._id) === String(enrollment.seriesId),
      );
      return {
        id: enrollment.id,
        seriesId: enrollment.seriesId,
  ...
  try {
    ...
    if (seriesId) {
      ...
      enrollments.forEach((e) => {
        if (e.seriesId) {
          seriesEnrollments[e.seriesId] =
            (seriesEnrollments[e.seriesId] || 0) + 1;
        }
      });
