...
const normalizeQuizPayload = (body = {}) => {
  const payload = {};
  const set = (col, val) => {
    if (payload[col] === undefined && val !== undefined && val !== null) payload[col] = val;
  };
  ...
  set("total_questions", body.totalQuestions ?? body.total_questions ?? body.questionCount ?? body.question_count);
  set("passing_score", body.passingMarks ?? body.passing_marks ?? body.passingScore ?? body.passing_score);
  ...
  set("tags", body.tags);
  const questionIds = body.questionIds ?? body.question_ids ?? body.questionIdList;
...
router.post("/quizzes/:id/duplicate", asyncHandler(async (req, res) => {
...
router.post("/subjects", asyncHandler(async (req, res) => {
  const { name, title, slug, icon, description, isActive, is_active } = req.body;
  const subjectName = (name || title || "").trim();
  if (!subjectName) {
    return res.status(400).json({ success: false, message: "Subject name is required" });
  }
  const subjectSlug = (slug || subjectName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')).trim();
  const payload = {
    name: subjectName,
    slug: subjectSlug,
    icon: icon || null,
...
router.put("/subjects/:id", asyncHandler(async (req, res) => {
  ...
  const updated = await dbHelpers.updateById("subjects", req.params.id, {
    ...rest,
    parent_id: parentId === "" || parentId === undefined || parentId === null ? null : Number(parentId),
    is_active: isActive,
    updatedAt: new Date().toISOString(),
  });
  if (!updated) {
...
router.get("/promotions", asyncHandler(async (req, res) => {
  ...
  if (search) {
    ...
    promotions = promotions.filter(p => (p.title || p.name || '').toLowerCase().includes(q) || (p.code || '').toLowerCase().includes(q));
  }
...
router.post("/quizzes/bulk", asyncHandler(async (req, res) => {
  const items = Array.isArray(req.body) ? req.body : req.body.quizzes || (req.body.items ? req.body.items : [req.body]);
  const inserted = [];
  const skipped = [];
  ...
  res.status(201).json({ success: true, count: results.length, data: results, skipped: skipped.length, skipDetails: skipped.length > 0 ? skipped : undefined });
}));

router.post("/ai/generate-questions", asyncHandler(async (req, res) => {
