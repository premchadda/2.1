...
function parseImportData(req) {
  ...
  if (
    req.body &&
    (Array.isArray(req.body.questions) || Array.isArray(req.body))
  ) {
    return {
  ...
  throw new Error(
    "No import data provided. Send a JSON file or { questions: [...] } in the body.",
  );
}
...
router.get("/import/history", async (req, res) => {
  try {
    ...
    const countResult = await pool.query(
      "SELECT COUNT(*)::int AS c FROM import_logs",
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const results = await universalImport(
        req.file.buffer,
