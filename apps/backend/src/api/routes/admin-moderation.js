...
router.get("/stats", responseCache("admin-mod-stats", 60), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load moderation stats" });
  }
});
...
router.get(
  "/doubts",
  responseCache("admin-mod-doubts", 30),
  async (req, res) => {
    try {
      ...
      if (search) {
        conditions.push(
          `(d.title ILIKE $${i} OR d.description ILIKE $${i} OR u.name ILIKE $${i})`,
        );
        values.push(`%${search}%`);
        i++;
      }
      ...
      const userJoinClause = cols.has("user_id")
        ...
        : cols.has("userId")
          ...
          : "";
      const orderByClause = cols.has("created_at")
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to load doubts" });
    }
  },
);
...
router.put("/doubts/:id/status", async (req, res) => {
  try {
    ...
    try {
      ...
      // Fallback to raw SQL
      await pool.query(
        "UPDATE doubts SET status = $1, updated_at = $2 WHERE id = $3",
...
router.delete("/doubts/:id", async (req, res) => {
  try {
    ...
    try {
      ...
      await pool.query("UPDATE doubts SET is_active = false WHERE id = $1", [
        id,
      ]);
    }
