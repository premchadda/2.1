...
function calculateProgress(definition, stats) {
  ...
  switch (req.type) {
    ...
    case "accuracy":
      return stats.totalAccuracy >= req.count
        ? 100
        : (stats.totalAccuracy / req.count) * 100;
    case "bookmarks":
...
// @route   GET /api/achievements/xp-summary
// @desc    Get candidate XP points, level progression, and streak freeze status
// @access  Protected
router.get("/xp-summary", async (req, res) => {
  try {
    const userId = req.user.id;
    const isPro = !!(req.user.is_pro_user || req.user.isPro);

    // Fetch user achievements for cumulative badge XP
    const userAchievements = await dbHelpers
    ...
    const badgeXp = userAchievements.length * 100;
    const attempts = await dbHelpers
      .find("attempts", { userId })
      .catch(() => []);
    const attemptXp = attempts.length * 50;

    const totalXp = (req.user.xp || 0) + badgeXp + attemptXp;
    const levelInfo = calculateUserLevel(totalXp);
    const freezeInfo = getStreakFreezeStatus(userId, isPro);
    ...
    res.json({
      ...
      data: {
        ...levelInfo,
        freeze: freezeInfo,
        badgesCount: userAchievements.length,
      },
    });
  } catch (error) {
...
// @route   POST /api/achievements/streak-freeze
// @desc    Apply streak freeze to preserve broken study streak
// @access  Protected
router.post("/streak-freeze", async (req, res) => {
  try {
    const userId = req.user.id;
    const isPro = !!(req.user.is_pro_user || req.user.isPro);
    const { currentStreak = 1 } = req.body;

    const result = applyStreakFreeze(userId, currentStreak, isPro);

    if (!result.frozen) {
      return res.status(400).json({
        success: false,
        message: "No streak freezes available for this month",
        data: result,
      });
    }
    ...
    res.json({
      success: true,
      message:
        "Streak freeze successfully activated! Your study streak is preserved.",
      data: result,
    });
  } catch (error) {
