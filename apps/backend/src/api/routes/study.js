...
async function getActiveStudyMaterials(dbHelpers) {
  const now = Date.now();
  if (
    _studyMaterialsCache &&
    now - _studyMaterialsCacheTime < STUDY_MATERIALS_CACHE_TTL
  ) {
...
function buildStudyMediaScope(
  ...
  if (subjectId != null) {
    ...
    if (
      ...
      (typeof subjectId === "string" && /^\d+$/.test(String(subjectId)))
    ) {
...
async function resolveSubjectMediaScope(dbHelpers, subject) {
  ...
  if (subjectId != null) {
    ...
    if (chapterIds.length) {
      const topics = await dbHelpers.find("topics", {
    ...
    const allTopics = await dbHelpers.find("topics", {
...
async function countMediaByScope(scope) {
  ...
  const where =
    conds.length > 0 ? `(${conds.join(" OR ")}) AND is_active = true` : "1=0";
...
const flattenHierarchyChapters = (parts = []) =>
  parts.flatMap((part) =>
    (part.units || []).flatMap((unit) => unit.chapters || []),
  );
...
async function findSubjectBySlugOrId(dbHelpers, slugOrId) {
  ...
  const sm = await findEntityByIdentifier(
    dbHelpers,
    "studyMaterials",
    slugOrId,
    {
...
function collectKeySet(record) {
  ...
  for (const v of [record.id, record._id]) {
    ...
    if (
      typeof v === "number" ||
      (typeof v === "string" && /^[0-9]+$/.test(String(v)))
    ) {
...
async function loadSubjectMediaBundle(dbHelpers, subject) {
  ...
  if (
    typeof smId === "number" ||
    (typeof smId === "string" && /^\d+$/.test(String(smId)))
  ) {
  ...
  const { chapterIds, topicIds } = await resolveSubjectMediaScope(
    dbHelpers,
    subject,
  );
  const linkResults = [
...
async function enrichLegacyChaptersWithMedia(dbHelpers, subject, chapters) {
  ...
  if (allVideos.length === 0 && allPdfs.length === 0 && allTests.length === 0)
    return chapters;
...
async function buildHierarchyFromParts(dbHelpers, parts, subject) {
  ...
  return Promise.all(
    sortedParts.map(async (part) => {
      const units = (
        await dbHelpers.find("units", {
          partId: part.id,
          isActive: true,
        })
      ).sort(sortByOrderAndId);

      const hierarchicalUnits = await Promise.all(
        units.map(async (unit) => {
          ...
          const hierarchicalChapters = await Promise.all(
            chapters.map(async (chapter) => {
              const chapterIdCandidates = [
                ...new Set(
                  [chapter.id, chapter._id].filter(
                    (v) => v != null && v !== "",
                  ),
                ),
              ];
              ...
              const videos = allVideos
                .filter((v) =>
                  assetBelongsToChapter(v, chapterKeySet, topicKeySet),
                )
                .sort(sortByOrderAndId)
                .map(mapVideoForClient);
              const pdfs = allPdfs
                .filter((p) =>
                  assetBelongsToChapter(p, chapterKeySet, topicKeySet),
                )
                .sort(sortByOrderAndId)
                .map(mapPdfForClient);
              const topicTests = allTests
                .filter((t) =>
                  assetBelongsToChapter(t, chapterKeySet, topicKeySet),
                )
                .sort(sortByOrderAndId);
...
function buildLegacyChaptersFromTopics(allTopics = []) {
  ...
  return chapters.map((chapter) => {
    ...
    const relatedTopics = chapterTopics.filter(
      (topic) => String(topic.parentTopicId) === String(chapterId),
    );
...
async function findInheritedParts(dbHelpers, subject) {
  ...
  for (const candidate of candidates) {
    ...
    const filteredParts = candidateParts.filter((part) => {
      ...
      return partKeywords.some(
        (keyword) => partName.includes(keyword) || keyword.includes(partName),
      );
    });
...
async function resolveSubjectContent(dbHelpers, subject) {
  ...
  const units = allUnits.filter(
    (u) => u.isDeleted !== true && u.is_deleted !== true,
  );
  ...
  const chapters = allChapters.filter(
    (c) => c.isDeleted !== true && c.is_deleted !== true,
  );

  const chapterIdCandidates = [
    ...new Set(
      chapters.flatMap((c) =>
        [c.id, c._id].filter((v) => v != null && v !== ""),
      ),
    ),
  ];
  const allTopics =
    chapterIdCandidates.length > 0
      ? await dbHelpers.find("topics", {
          chapterId: { $in: chapterIdCandidates },
          isActive: true,
        })
  ...
  const topics = allTopics.filter(
    (t) => t.isDeleted !== true && t.is_deleted !== true,
  );
  ...
  const enrichedChapters = chapters
    .map((chapter) => {
      const chapterTopics = topics
        .filter(
          (t) =>
            looseIdEquals(t.chapterId, chapter.id) ||
            looseIdEquals(t.chapterId, chapter._id),
        )
        .sort(sortByOrderAndId);
  ...
  const enrichedUnits = units
    .map((unit) => {
      const unitChapters = enrichedChapters.filter(
        (c) =>
          looseIdEquals(c.unitId, unit.id) || looseIdEquals(c.unitId, unit._id),
      );
      return {
        ...unit,
  ...
  const chaptersWithUnit = new Set(
    enrichedUnits.flatMap((u) => u.chapters).map((c) => c.id),
  );
  const orphanedChapters = enrichedChapters.filter(
    (c) => !chaptersWithUnit.has(c.id),
  );
...
function mapChapterSummary(chapter) {
  return {
    ...
    videos:
      chapter.videoCount || chapter.videos || chapter.videosList?.length || 0,
