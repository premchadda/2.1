...
router.get(
  "/stages",
  responseCache("admin-stages-list", 120),
...
router.get(
  "/stages/with-test-counts",
  responseCache("admin-stages-counts", 120),
  asyncHandler(async (req, res) => {
    const stages = await dbHelpers.find("stages");
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existingCategoryIds = Array.isArray(stage.categoryIds)
      ? stage.categoryIds
      : [];
    ...
    switch (operation) {
      case "add":
        newCategoryIds = [
          ...new Set([...existingCategoryIds, ...(categoryIds || [])]),
        ];
        break;
      case "remove":
        newCategoryIds = existingCategoryIds.filter(
