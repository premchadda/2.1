...
router.get('/rank-prediction', async (req, res) => {
  try {
    ...
    const totalStudents = req.query.totalStudents ? Number(req.query.totalStudents) : null
...
router.put('/revision-queue/:id/complete', async (req, res) => {
  try {
    ...
    const result = await learningService.completeRevisionItem(req.user.id, req.params.id, { isCorrect })
    if (!result.success) {
...
router.post('/daily-quiz/:quizId/submit', async (req, res) => {
  try {
    ...
    const result = await learningService.submitDailyQuiz(req.user.id, req.params.quizId, answers)
    if (!result.success) {
