...
router.put("/settings", asyncHandler(async (req, res) => {
  ...
  for (const key of ALLOWED_SETTINGS_KEYS) {
    if (req.body[key] !== undefined) {
      ...
      if (key === 'maintenanceMode' || key === 'allowRegistrations' || key === 'requireEmailVerification') {
...
router.post("/settings/test-email", asyncHandler(async (req, res) => {
  ...
  const fromName = process.env.SMTP_FROM_NAME || emailConfig.fromName || "Trstprep";
  ...
  try {
    ...
    return res.json({
      success: true,
      data: { message: "SMTP configuration looks valid (nodemailer not installed)" },
    });
  }
