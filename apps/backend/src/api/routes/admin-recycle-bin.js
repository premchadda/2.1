...
router.put("/:id/restore", protect, admin, async (req, res) => {
  try {
    ...
    if (!table || !TRASH_TABLES.includes(table)) {
      return res
        .status(400)
        .json({ success: false, message: "Missing or invalid table name" });
    }
...
const purgeAllTrash = async (_req, res) => {
  try {
    ...
    const results = await Promise.allSettled(
      TRASH_TABLES.map(async (table) => {
        if (!TRASH_TABLES.includes(table) || !/^[a-zA-Z0-9_]+$/.test(table)) {
          return 0;
        }
        try {
          const hasIsDeleted = await dbHelpers.columnExists(
            table,
            "is_deleted",
          );
          if (hasIsDeleted) {
          ...
          const rows = await dbHelpers
            .find(table, TRASH_QUERY, 500)
            .catch(() => []);
          let subPurged = 0;
          for (const row of rows) {
            const id = row.id || row._id;
            if (id) {
              const ok = await dbHelpers
                .deleteById(table, id)
                .catch(() => false);
              if (ok) subPurged += 1;
            }
    ...
    res.json({
      success: true,
      message: `${purged} item(s) permanently deleted`,
      data: { purged },
    });
  } catch (error) {
    res
