...
// Server-side analytics gate — GET /admin/analytics/* returns 503 when
// features.analytics is off. Previously only tracking-ID exposure was gated.
const requireAnalyticsEnabled = async (req, res, next) => {
  try {
    ...
    // fail-open on settings lookup error to avoid cascading outage
  }
  next();
};
    ...
    try {
      ...
      try {
        ...
        const subscribedUsers =
          parseInt(subscribedUsersResult.rows[0].count) || 0;
        ...
        const funnelStages = [
          ...
          },
          {
            ...
            conversion_rate:
              totalUsers > 0
                ? ((enrolledUsers / totalUsers) * 100).toFixed(2)
                : 0,
            drop_off_rate:
              activeUsers > 0
                ? (((activeUsers - enrolledUsers) / activeUsers) * 100).toFixed(
                    2,
                  )
                : 0,
          ...
          },
          {
            ...
            count: testTakers,
            conversion_rate:
              totalUsers > 0 ? ((testTakers / totalUsers) * 100).toFixed(2) : 0,
            drop_off_rate:
              enrolledUsers > 0
                ? (
                    ((enrolledUsers - testTakers) / enrolledUsers) *
                    100
                  ).toFixed(2)
                : 0,
          ...
          },
          {
            ...
            conversion_rate:
              totalUsers > 0
                ? ((subscribedUsers / totalUsers) * 100).toFixed(2)
                : 0,
            drop_off_rate:
              testTakers > 0
                ? (((testTakers - subscribedUsers) / testTakers) * 100).toFixed(
                    2,
                  )
                : 0,
          ...
          data: {
            ...
            summary: {
              ...
              biggest_dropoff: funnelStages.reduce((prev, curr) =>
                parseFloat(curr.drop_off_rate) > parseFloat(prev.drop_off_rate)
                  ? curr
                  : prev,
              ),
            },
    ...
    try {
      ...
      const safeMonths =
        Number.isFinite(parsedMonths) && parsedMonths > 0
          ? Math.min(Math.floor(parsedMonths), 120)
          : 6;
      const intervalDays =
        period === "weekly" ? safeMonths * 4 * 7 : safeMonths * 30;
      ...
      `;
        ...
        const { rows: cohorts } = await client.query(cohortQuery, [
          dateTrunc,
          intervalDays,
          safeMonths,
        ]);
  ...
  async (req, res) => {
    try {
      ...
      `;
        ...
        const { rows: dauData } = await client.query(dauQuery, [
          dateTrunc,
          days,
        ]);
      ...
      `;
        ...
        const { rows: testAttemptsData } = await client.query(
          testAttemptsQuery,
          [dateTrunc, days],
        );
      ...
      `;
        ...
        const { rows: enrollmentsData } = await client.query(enrollmentsQuery, [
          dateTrunc,
          days,
        ]);

        // Calculate summary statistics
        const totalDau = dauData.reduce(
          (sum, row) => sum + parseInt(row.daily_active),
          0,
        );
        const avgDau =
          dauData.length > 0 ? (totalDau / dauData.length).toFixed(2) : 0;

        const totalAttempts = testAttemptsData.reduce(
          (sum, row) => sum + parseInt(row.attempts),
          0,
        );
        const avgScore =
          testAttemptsData.length > 0
            ? (
                testAttemptsData.reduce(
                  (sum, row) => sum + parseFloat(row.avg_score || 0),
                  0,
                ) / testAttemptsData.length
              ).toFixed(2)
            : 0;

        const totalEnrollments = enrollmentsData.reduce(
          (sum, row) => sum + parseInt(row.new_enrollments),
          0,
        );
