...
// ===== TAG CONFIGS =====
router.get("/tag-configs", responseCache("admin-tag-configs", 60), asyncHandler(async (req, res) => {
  const tags = await dbHelpers.find("tagConfigs", {});
  res.json({ success: true, data: tags });
}));

router.post("/tag-configs", asyncHandler(async (req, res) => {
  const { name, label, color, icon, route, filterKey, filter_key, filterValue, filter_value, description, displayOrder, display_order, isActive, is_active } = req.body;
  const tagName = (name || label || "").trim();
  if (!tagName) {
    return res.status(400).json({ success: false, message: "Tag name or label is required" });
  }
  const payload = {
    name: tagName,
...
router.put("/tag-configs/:id", asyncHandler(async (req, res) => {
  const { name, label, color, icon, route, filterKey, filter_key, filterValue, filter_value, description, displayOrder, display_order, isActive, is_active } = req.body;
  const payload = {};
  ...
  if (filterKey !== undefined || filter_key !== undefined) payload.filter_key = filterKey || filter_key;
  if (filterValue !== undefined || filter_value !== undefined) payload.filter_value = filterValue ?? filter_value;
...
router.delete("/tag-configs/:id", asyncHandler(async (req, res) => {
