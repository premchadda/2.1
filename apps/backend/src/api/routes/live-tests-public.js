...
// PERF-02: SQL-level filtering for Live Tests & Live Quizzes.
// @route   GET /api/live-tests
router.get('/', async (req, res) => {
  try {
    const { limit = 50, page = 1, type, status, category, difficulty } = req.query;
    ...
    if (type === 'quiz' || type === 'quizzes') {
      params.push('%quiz%');
      conditions.push(`(t.type ILIKE $${params.length} OR t.test_type ILIKE $${params.length} OR t.category ILIKE $${params.length} OR t.sub_category ILIKE $${params.length} OR 'quiz' = ANY(t.tags) OR 'quizzes' = ANY(t.tags) OR 'live-quiz' = ANY(t.tags))`);
    } else if (type === 'test' || type === 'tests' || type === 'mock') {
      conditions.push(`(t.type NOT ILIKE '%quiz%' AND (t.test_type IS NULL OR t.test_type NOT ILIKE '%quiz%') AND (t.category IS NULL OR t.category NOT ILIKE '%quiz%') AND NOT ('quiz' = ANY(t.tags)))`);
    }
    ...
    const liveItems = testsRes.rows.map((row) => {
      ...
      const isQuiz = Boolean(
        ...
        String(camel.category || '').toLowerCase().includes('quiz') ||
        String(camel.subCategory || '').toLowerCase().includes('quiz') ||
        (Array.isArray(camel.tags) && camel.tags.some(t => String(t).toLowerCase().includes('quiz')))
      );
