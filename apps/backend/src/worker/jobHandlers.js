...
export const handlersByQueue = {
  [QUEUE_NAMES.ANALYTICS]: async (job) => {
    ...
    if (job.name === "analytics.test-submitted") {
      ...
      return defaultResult({
        action: "analytics-test-submitted",
        ...processed,
      });
    }
  ...
  [QUEUE_NAMES.NOTIFICATIONS]: async (job) => {
    ...
    if (job.name === "notifications.scheduled-reminders") {
      ...
      return defaultResult({
        action: "notifications-scheduled-reminders",
        ...result,
      });
    }

    const dispatched = await notificationService.handleNotificationJob(
      job.name,
      payload,
    );
  ...
  [QUEUE_NAMES.RECOMMENDATIONS]: async (job) => {
    ...
    const payload = job.data?.payload || {};
    const refreshed =
      await recommendationService.refreshRecommendationsFromEvent({
