...
const queueConcurrency = {
  analytics: Number.parseInt(
    process.env.WORKER_ANALYTICS_CONCURRENCY || "4",
    10,
  ),
  leaderboard: Number.parseInt(
    process.env.WORKER_LEADERBOARD_CONCURRENCY || "2",
    10,
  ),
  notifications: Number.parseInt(
    process.env.WORKER_NOTIFICATIONS_CONCURRENCY || "5",
    10,
  ),
  recommendations: Number.parseInt(
    process.env.WORKER_RECOMMENDATIONS_CONCURRENCY || "2",
    10,
  ),
...
const start = async () => {
  try {
    ...
    if (!isRedisReady()) {
      ...
      throw new Error(
        `Redis is required for worker mode. Status: ${status.message}`,
      );
    }
    ...
    try {
      ...
      logger.error(
        "[Worker] Database connection failed — jobs touching the DB will fail:",
        dbErr.message,
      );
