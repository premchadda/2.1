...
const adaptiveDifficultyService = {
  ...
  async getDifficulty(userId, topicId) {
    ...
    if (cacheRedis) {
      try {
        ...
        if (resetRaw) {
          let totalAttempts = 0;
          try {
            totalAttempts = JSON.parse(resetRaw).totalAttempts || 0;
          } catch {
            /* ignore */
          }
          return {
    ...
    if (stats.rows.length > 0) {
      ...
      score = typeof prevEma === "number" ? prevEma : recentAccuracy;
    } else {
      totalAttempts = 0;
      recentAccuracy = 0;
      score = DEFAULT_DIFFICULTY;
    ...
    const data = {
      ...
      ...(totalAttempts < MIN_ATTEMPTS_FOR_CONFIDENCE
        ? { lowConfidence: true }
        : {}),
    };
  ...
  async resetDifficulty(userId, topicId) {
    ...
    const totalAttempts =
      stats.rows.length > 0 ? parseInt(stats.rows[0].total_attempts) || 0 : 0;
    localCache.delete(memKey);
