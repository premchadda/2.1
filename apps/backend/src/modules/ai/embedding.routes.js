 ...
 */
router.post('/search', protect, aiRateLimiter, async (req, res) => {
  try {
 ...
 */
router.post('/index', protect, admin, aiRateLimiter, async (req, res) => {
  try {
...
router.post('/index/batch', protect, admin, aiRateLimiter, async (req, res) => {
  try {
    ...
    // Bulk cap (18): slice to 50 with an explicit truncated flag.
    const truncated = items.length > 50
    const capped = truncated ? items.slice(0, 50) : items

    const results = await embeddingService.indexBatch(capped)
...
router.post('/index/all-unindexed', protect, admin, aiRateLimiter, async (req, res) => {
  try {
    const { contentType, limit, afterId, maxPages } = req.body
    if (!contentType) {
      return res.status(400).json({
        success: false,
    ...
    const results = await embeddingService.indexAllUnindexed(
      contentType,
      parseInt(limit) || 100,
 ...
 */
router.delete('/:contentType/:contentId', protect, admin, aiRateLimiter, async (req, res) => {
  try {
 ...
 */
router.get('/:contentType/:contentId', protect, admin, aiRateLimiter, async (req, res) => {
  try {
    const { contentType, contentId } = req.params
