...
router.post(
  "/generate-hindi/:questionId",
  protect,
  admin,
  aiRateLimiter,
  async (req, res) => {
    try {
      const result = await aiExplanationService.generateHindiExplanation(
        req.params.questionId,
        {
          ...req.body,
          userId: req.user.id,
...
router.get("/model-stats", protect, admin, aiRateLimiter, async (req, res) => {
  try {
