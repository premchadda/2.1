...
async function postChatCompletions(url, apiKey, body, { timeoutMs = 30000, retries = 1 } = {}) {
  let lastError = null
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      lastError = new Error(`AI request failed: ${networkErr?.message || networkErr}`)
      lastError.status = 0
    ...
    const err = new Error(`AI API error: ${response.status}${errBody ? ` - ${errBody.slice(0, 500)}` : ''}`)
    err.status = response.status
    lastError = err
    if (!TRANSIENT_STATUSES.has(response.status) || attempt >= retries) throw err
...
async function callAI(messages, options = {}) {
  ...
  if (!config.apiKey) {
    throw new Error('AI API key is not configured (set AI_API_KEY or OPENROUTER_API_KEY)')
  }
  ...
  if (!choice?.message || typeof choice.message.content !== 'string' || !choice.message.content.trim()) {
    const err = new Error(`AI API returned no content (model: ${data?.model || config.model})`)
    err.status = 502
    throw err
  }
...
async function callAIWithFallback(messages, options = {}) {
  try {
    ...
    if (!FALLBACK_CONFIG.apiKey) {
      console.warn('Primary AI provider failed, no fallback key configured:', primaryError.message)
      const err = new Error(`AI service unavailable (primary: ${primaryError.status || 'error'} ${primaryError.message})`)
    ...
    try {
      const sameUrl = (FALLBACK_CONFIG.baseUrl || '') === (AI_CONFIG.baseUrl || '')
      const sameModel = (FALLBACK_CONFIG.model || '') === (options.model || AI_CONFIG.model || '')
      const sameKey = (FALLBACK_CONFIG.apiKey || '') === (options.apiKey || AI_CONFIG.apiKey || '')
    ...
    try {
      ...
      const err = new Error(`AI service unavailable (primary: ${primaryError.status || '?'} ${primaryError.message} | fallback: ${fallbackError.status || '?'} ${fallbackError.message})`)
...
async function generateEmbedding(text, options = {}) {
  ...
  if (!response.ok) {
    const errBody = await response.text().catch(() => '')
    throw new Error(`Embedding API error: ${response.status}${errBody ? ` - ${errBody.slice(0, 500)}` : ''}`)
  }
  ...
  if (!Array.isArray(embedding) || embedding.length === 0) {
    throw new Error(`Embedding API returned no vector (model: ${options.model || 'text-embedding-3-small'})`)
  }
  return embedding
}
