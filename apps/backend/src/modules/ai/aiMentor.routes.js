...
function minuteLimitExceeded(res) {
  ...
  return res.status(429).json({
    success: false,
    message:
      "AI rate limit exceeded (burst guard). Please try again in a minute.",
...
async function checkTokenBudget(userId, tokensRequested = 0, charge = true) {
  ...
  if (redis) {
    try {
      ...
      const { default: budgetLogger } =
        await import("../../infrastructure/logger/logger.js").catch(() => ({
          default: console,
        }));
      budgetLogger.error?.(
        { err },
...
Be friendly, encouraging, and provide practical advice. Keep responses concise but helpful.`;
  ...
  return [
    ...
    {
      role: "user",
      content: sanitizeForPrompt(typeof message === "string" ? message : ""),
    },
  ];
}
...
router.post("/study-plan", protect, aiRateLimiter, async (req, res) => {
  try {
    ...
    const { days } = req.body;
    if (
      !(await checkTokenBudget(req.user.id, estimateTokens("study plan", days)))
    ) {
      return res
        .status(429)
        .json({ success: false, error: "Daily token budget exceeded." });
    }
  ...
  try {
    ...
    if (
      !(await checkTokenBudget(
        req.user.id,
        estimateTokens(question, topic, subject),
      ))
    ) {
      return res
        .status(429)
    ...
    const result = await aiMentorService.answerDoubt(req.user.id, question, {
      topic,
      subject,
    });
    res.json({ success: true, data: result });
...
router.post("/exam-strategy", protect, aiRateLimiter, async (req, res) => {
  try {
    ...
    const result = await aiMentorService.generateExamStrategy(
      req.user.id,
      examType,
    );
    res.json({ success: true, data: result });
...
router.post("/chat", protect, aiRateLimiter, async (req, res) => {
  try {
    ...
    const result = await aiMentorService.chat(
      req.user.id,
      message,
      conversationId,
    );
...
router.post("/chat/stream", protect, aiRateLimiter, async (req, res) => {
  try {
    ...
    if (
      !(await checkTokenBudget(
        req.user.id,
        estimateTokens(message, safeJson(req.body.messages).slice(0, 4000)),
      ))
    ) {
      return res
        .status(429)
    ...
    try {
      ...
      const estTokens = Math.max(
        1,
        Math.ceil(assistantResponseText.length / 4),
      );
      const estInput = estimateTokens(
        message,
        safeJson(history).slice(0, 8000),
      );
...
router.post("/socratic-hint", protect, aiRateLimiter, async (req, res) => {
  try {
    ...
    if (!questionText) {
      return res
        .status(400)
        .json({ success: false, message: "questionText is required" });
    }
