  ...
  try {
    ...
    if (String(text).length > MAX_TEXT_LEN)
      return res.status(400).json({
        success: false,
        message: `Text too long (max ${MAX_TEXT_LEN} chars)`,
      });
...
router.post("/render-batch", protect, mathLimiter, async (req, res) => {
  try {
    ...
    if (!Array.isArray(texts))
      return res
        .status(400)
        .json({ success: false, message: "Texts array is required" });
    if (texts.length > MAX_BATCH)
      return res.status(400).json({
        success: false,
        message: `Batch too large (max ${MAX_BATCH})`,
      });
    if (texts.some((t) => String(t ?? "").length > MAX_TEXT_LEN))
      return res.status(400).json({
        success: false,
        message: `Each text max ${MAX_TEXT_LEN} chars`,
      });
...
router.post("/validate", protect, mathLimiter, async (req, res) => {
  try {
    ...
    if (!expression)
      return res
        .status(400)
        .json({ success: false, message: "Expression is required" });
