...
router.get("/", protect, admin, async (req, res) => {
  try {
    ...
    const rows = Array.isArray(logs)
      ? logs.slice(offset, offset + safeLimit)
      : logs;
    res.json({
      success: true,
      data: rows,
...
router.get("/stats/cost", protect, admin, async (req, res) => {
  try {
    ...
    const stats = await aiGenerationLogService.getCostSummary(
      startDate,
      endDate,
    );
    res.json({ success: true, data: stats });
  ...
  try {
    ...
    switch (period) {
      ...
      case "yesterday": {
        ...
        data = await aiGenerationLogService.getUsageByPeriod(
          yesterday,
          tomorrow,
        );
        break;
      ...
      case "365d": {
        ...
        threeHundredSixtyFiveDaysAgo.setFullYear(
          threeHundredSixtyFiveDaysAgo.getFullYear() - 1,
        );
        data = await aiGenerationLogService.getUsageByPeriod(
