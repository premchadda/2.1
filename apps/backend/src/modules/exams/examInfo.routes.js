...
router.get("/", responseCache("exam-info", 120), async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      count: examsWithAliases.length,
      data: examsWithAliases,
    });
  } catch (error) {
...
router.post("/report-error", createRateLimiter("strict"), async (req, res) => {
  ...
  if (!reason) {
    return res
      .status(400)
      .json({ success: false, message: "Report category is required" });
  }
  try {
    await pool.query(
...
router.get("/:id", async (req, res) => {
  try {
    const exam = await findEntityByIdentifier(
      dbHelpers,
      "exams",
      req.params.id,
      {
    ...
    if (!exam || !exam.isActive) {
      return res
        .status(404)
        .json({ success: false, message: "Exam information not found" });
    }
...
// Helper to resolve numeric exam_id from integer, slug, or public_id
async function resolveExamId(identifier) {
  if (!identifier) return null;
  const numId = parseInt(identifier, 10);
  if (!isNaN(numId) && String(numId) === String(identifier).trim()) {
    return numId;
  }
  const exam = await findEntityByIdentifier(dbHelpers, "exams", identifier, {
    slugFields: ["slug", "exam_id"],
  });
  return exam ? (exam.id ?? exam._id ?? null) : null;
}
...
router.get("/:examId/updates", async (req, res) => {
  try {
    const examId = await resolveExamId(req.params.examId);
    if (!examId) {
      return res.json({ success: true, count: 0, data: [] });
    }
    const updates = await dbHelpers.find(
      "exam_updates",
      {
        exam_id: examId,
        is_active: true,
...
router.get("/:examId/yearly-data", async (req, res) => {
  try {
    ...
    const yearlyData = await dbHelpers.find(
      "exam_yearly_data",
      {
        exam_id: examId,
        is_active: true,
