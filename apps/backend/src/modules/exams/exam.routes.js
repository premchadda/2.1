...
router.get('/:slug/compare', async (req, res) => {
  try {
    ...
    const yearData = years.map(year => {
      const info = examInfos.find(ei => ei.year === year || ei.year === parseInt(year))
      return {
        year,
        notification: info?.notification || `June ${parseInt(year) - 1}`,
        applicationStart: info?.applicationStart || `${parseInt(year) - 1}-07-01`,
...
// @access  Public
router.get('/:slug/year', async (req, res) => {
  try {
    const { slug } = req.params
    ...
    res.json({
      ...
      data: {
        ...
        vacancy: info?.vacancy ?? null,
        description: info?.description || `Complete information about ${exam.title || exam.name} ${year} examination including important dates, syllabus, and preparation strategy.`,
        eligibility: info?.eligibility || "Bachelor's degree in any discipline",
        ageLimit: info?.ageLimit || '18-32 years',
        patternChanges: info?.patternChanges || 'No major changes from previous year',
        syllabusChanges: info?.syllabusChanges || 'Some topics added to General Awareness section',
        previousYearPapers: previousYearPapers.length > 0 ? previousYearPapers : [
