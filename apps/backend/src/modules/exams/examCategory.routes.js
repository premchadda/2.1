...
router.get('/', responseCache("exam-categories", 120), async (req, res) => {
  try {
    ...
    const sortedCategories = categories.sort((a, b) => (a.displayOrder ?? a.display_order ?? 0) - (b.displayOrder ?? b.display_order ?? 0))
...
// @route   GET /api/exam-categories/
// @desc    Get all exams (acting as subcategories - exams table is the source)
// @access  Public
router.get('/subcategories/all', async (req, res) => {
  try {
    // Get all exams from database - exams act as subcategories
    let exams = await dbHelpers.find('exams', { isActive: true })
    
    if (!exams) exams = []
    
    // Sort by displayOrder (toCamel converts display_order → displayOrder)
    const sortedExams = exams.sort((a, b) => (a.displayOrder ?? a.display_order ?? 0) - (b.displayOrder ?? b.display_order ?? 0))
    
    // Map exams to subcategory format for frontend compatibility
    const subcategories = sortedExams.map(exam => ({
...
router.get('/:id', async (req, res) => {
  try {
    ...
    let category = await findEntityByIdentifier(dbHelpers, 'examCategories', id, {
      slugFields: ['slug']
    })
...
router.get('/:id/exams', async (req, res) => {
  try {
    ...
    const sortedExams = exams.sort((a, b) => (a.display_order || 0) - (b.display_order || 0))
