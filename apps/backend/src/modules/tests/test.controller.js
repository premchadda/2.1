...
router.put("/:id/state", protect, admin, async (req, res) => {
  try {
    const { to } = req.body;
    const result = await testService.transitionState(
      req.params.id,
      to,
      req.user.id,
    );
...
router.get("/:testId/result/:attemptId", protect, async (req, res) => {
  try {
    ...
    const leaderboardIndex = leaderboard.findIndex(
      (e) => String(e.userId) === String(attempt.userId),
    );
    const totalParticipants = leaderboard.length;
    const rank =
      leaderboardIndex >= 0 ? leaderboardIndex + 1 : totalParticipants + 1;
    const percentile =
      totalParticipants > 0 && leaderboardIndex >= 0
      ...
      data: {
        ...
        questions: questions.map((q) => {
          ...
          return {
            ...
            options: q.options,
            correctAnswer:
              q.correct_answer ?? q.correctOption ?? q.correctAnswer,
