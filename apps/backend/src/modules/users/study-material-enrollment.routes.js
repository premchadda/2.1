...
router.delete(
  ...
  async (req, res) => {
    try {
      const { materialId } = req.params;
      console.log(
        "[Unenroll Study Material] Request received for materialId:",
        materialId,
        "userId:",
        req.user.id,
      );
      ...
      if (!material) {
        console.log(
          "[Unenroll Study Material] Material not found:",
          materialId,
        );
        return res.status(404).json({
          success: false,
      ...
      console.log(
        "[Unenroll Study Material] Material found:",
        material._id || material.id,
        material.title,
      );
      ...
      try {
        ...
        if (studyHistory.length > 0) {
          ...
          console.log(
            `[Unenroll Study Material] Archived ${studyHistory.length} study progress records`,
          );
        }
      } catch (archiveError) {
        console.error(
          "[Unenroll Study Material] Error archiving history:",
          archiveError,
        );
      }
      ...
      if (!unenrolled) {
        console.log(
          "[Unenroll Study Material] User not enrolled in this material",
        );
      ...
      res.json({
        success: true,
        message:
          "Successfully unenrolled from study material. Your history has been archived.",
        data: enrolledMaterialIds.map((value) =>
          mapLookupId(value, enrolledStudyMaterialsLookup, value),
        ),
      });
...
router.get("/enrolled-study-materials", protect, async (req, res) => {
  try {
    ...
    const populatedMaterials =
      enrolledMaterialIds.length > 0
        ? await dbHelpers.find("studyMaterials", {
            id: { $in: enrolledMaterialIds },
          })
        : [];
