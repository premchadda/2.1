...
router.get("/learning-path", protect, aiRateLimiter, async (req, res, next) => {
  try {
    const rootNodeId = req.query.rootNodeId
      ? parseInt(req.query.rootNodeId, 10)
      : null;
    const learningPath = await nodeEngineService.generateLearningPath(
      req.user.id,
      rootNodeId,
    );
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const dueTopics = await nodeEngineService.getSpacedRepetitions(
        req.user.id,
      );
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const parsedNodeId =
        typeof nodeId === "number" ? nodeId : Number(String(nodeId).trim());
      if (
        !Number.isInteger(parsedNodeId) ||
        parsedNodeId <= 0 ||
        String(nodeId).trim() !== String(parsedNodeId)
      ) {
        return res.status(400).json({
          success: false,
