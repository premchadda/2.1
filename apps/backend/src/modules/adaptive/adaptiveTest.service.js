...
const adaptiveTestService = {
  ...
  async getNextQuestion(sessionId, userId) {
    ...
    try {
      ...
      if (metadata.topicId) {
        try {
          const userDifficulty = await adaptiveDifficultyService.getDifficulty(
            userId,
            metadata.topicId,
          );
      ...
      const fallbackDifficulties = difficultyOrder.slice(
  ...
  async submitAnswer(sessionId, userId, questionId, selectedOption, timeSpent) {
    ...
    try {
      ...
      const isCorrect =
        ...
        Number(selectedOption) === Number(correctOption);
      const marks = Number(question.rows[0].marks ?? 2);
      const negRaw = Number(question.rows[0].negative_marks ?? 0);
      const negativeMarks =
        negRaw > 0 ? negRaw : marks === 2 ? 0.5 : marks * 0.25;
      ...
      if (isCorrect && metadata.correctCount > metadata.wrongCount) {
        ...
        else if (metadata.currentDifficulty === "medium")
          newDifficulty = "hard";
        else if (metadata.currentDifficulty === "hard")
      ...
      if (metadata.topicId) {
        try {
          await adaptiveDifficultyService.updatePerformance(
            userId,
            metadata.topicId,
            isCorrect,
            timeSpent,
