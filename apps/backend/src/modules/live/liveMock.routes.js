...
router.post("/:id/save-answer", protect, async (req, res) => {
  try {
    const result = await liveMockService.saveAnswer(
      req.user.id,
      req.params.id,
      req.body,
    );
...
router.get("/:id/live-rank", protect, async (req, res) => {
  try {
    const results = await liveMockService.getResults(
      req.user.id,
      req.params.id,
    );
    if (!results) {
      return res.status(404).json({
        success: false,
        message: "Live rank is not available until you have a completed result",
      });
    }
