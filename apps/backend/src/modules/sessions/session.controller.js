...
const ensureUserSessionsTable = async () => {
  try {
    ...
    try {
      ...
      `);
      if (
        idColCheck.rows.length > 0 &&
        (idColCheck.rows[0].data_type === "integer" ||
          idColCheck.rows[0].data_type === "bigint")
      ) {
        console.log(
          "[Sessions] Migrating user_sessions.id from SERIAL to VARCHAR",
        );
        await pool.query(
          `ALTER TABLE user_sessions ALTER COLUMN id TYPE VARCHAR(255)`,
        );
    ...
    if (cols.includes("userId")) {
      console.log(
        "[Sessions] Migrating user_sessions columns from camelCase → snake_case",
      );
      const renames = [
        ['"userId"', "user_id"],
      ...
      for (const [from, to] of renames) {
        if (cols.includes(from.replace(/"/g, ""))) {
          try {
            await pool.query(
              `ALTER TABLE user_sessions RENAME COLUMN ${from} TO ${to}`,
            );
          } catch (_) {
...
const sessionController = {
  ...
  revokeSession: async (req, res) => {
    try {
      ...
      if (
        String(check.rows[0].user_id) !== userId &&
        req.user.role !== "admin"
      ) {
        return res
          .status(403)
  ...
  revokeAllSessions: async (req, res) => {
    try {
      ...
      for (const row of sessionsToRevoke.rows) {
        await invalidateSession(row.session_id, userId);
        revokedCount++;
      }
      ...
      logAuditEvent({
        ...
        details: {
          ...
          keptCurrent: Boolean(currentSessionId),
        },
      });
  ...
  try {
    if (limit) {
      ...
      if (activeSessions.rows.length >= limit) {
        ...
        const sessionsToRevoke = activeSessions.rows.slice(
        ...
        console.log(
          `[Sessions] Revoked ${sessionsToRevoke.length} old session(s) for user ${userId} to enforce limit of ${limit}`,
        );
      }
    }
