...
export const storeCsrfToken = async (authToken, csrfToken) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    console.error('CSRF: Both DB and Redis unavailable in production. Token NOT stored.');
    return false;
  }
...
async function storePrevCsrfToken(authTokenHash) {
  ...
  const now = Date.now();
  const validItems = items.filter(item => item && item.token && item.expiresAt > now);
  if (validItems.length === 0) return;

  const prevHashKey = crypto.createHash('sha256').update(`prev:${authTokenHash}`).digest('hex');
...
export const csrfCleanupInterval = setInterval(cleanupCsrfTokens, CSRF_TOKEN_CLEANUP_INTERVAL_MS)
...
async function isRecentCsrfToken(authTokenHash, csrfToken) {
  ...
  if (Array.isArray(list)) {
    const now = Date.now()
    if (list.some(item => item && item.token === csrfToken && item.expiresAt > now)) {
      return true
    }
  ...
  if (redis) {
    try {
      const isMember = await redis.sismember(`csrf:prev:${authTokenHash}`, csrfToken)
...
export const validateCsrfToken = async (req, res, next) => {
  ...
  const csrfExemptPaths = [
    ...
    '/api/auth/reset-password', '/api/auth/verify-email', '/api/auth/google',
    '/api/auth/login/2fa', '/api/auth/2fa/status', '/api/auth/2fa/enroll',
    '/api/auth/2fa/verify', '/api/auth/2fa/backup-codes/regenerate', '/api/auth/2fa/disable',
    '/api/auth/resend-verification',
    '/api/payments', '/payments'
  ]
  const fullUrl = req.originalUrl || req.path || ''
  if (csrfExemptPaths.some(p => fullUrl.startsWith(p) || req.path.startsWith(p))) {
    return next()
  }
  ...
  if (storedToken) {
    ...
    const validList = (Array.isArray(rawList) ? rawList : [rawList])
