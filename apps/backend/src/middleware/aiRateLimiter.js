...
const checkFallbackLimit = (userId, limit) => {
  ...
  for (const [k, v] of fallbackBuckets) {
    if (v.resetAt <= now || fallbackBuckets.size > FALLBACK_MAX_KEYS)
...
export const aiRateLimiter = async (req, res, next) => {
  ...
  if (
    ...
    process.env.AI_RATE_LIMIT_ENFORCE !== "true"
  )
    return next();
  ...
  const useFallback = (reason) => {
    ...
    logger.warn(
      `[AI RateLimiter] ${reason} — per-process fallback caps active`,
    );
    const fb = checkFallbackLimit(userId, limit);
    if (!fb.allowed) {
      const retryAfter = Math.max(
        1,
        Math.ceil((fb.resetAt - Date.now()) / 1000),
      );
