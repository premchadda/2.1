...
async function setCachedSession(sessionId, session) {
  ...
  if (redis) {
    try {
      await redis.set(`session:${sessionId}`, JSON.stringify(session), 'EX', 300)
    } catch (err) {
  ...
  localSessionCache.set(String(sessionId), { session, expiresAt: Date.now() + SESSION_CACHE_TTL })
}
...
const defaultIdleMin = process.env.NODE_ENV === 'development' ? '10080' : '30'
export const ADMIN_IDLE_TIMEOUT_MS = parseInt(process.env.ADMIN_IDLE_TIMEOUT_MIN || defaultIdleMin, 10) * 60_000
export const SESSION_IDLE_TIMEOUT_MS = parseInt(process.env.SESSION_IDLE_TIMEOUT_MIN || defaultIdleMin, 10) * 60_000
export const SESSION_ABSOLUTE_TIMEOUT_MS = parseInt(process.env.SESSION_ABSOLUTE_TIMEOUT_DAYS || '30', 10) * 24 * 60 * 60 * 1000
...
export function isSessionExpired(sessionRow, idleThresholdMs, absoluteThresholdMs, now = Date.now()) {
  if (!sessionRow) return { expired: false }
  const createdAt = sessionRow.created_at ? new Date(sessionRow.created_at).getTime() : null
...
async function enforceSessionExpiry(decoded, user) {
  ...
  const idleThreshold = IDLE_ENFORCED_ROLES.has(user.role)
  ...
  try {
    ...
    const expired = isSessionExpired(row, idleThreshold, SESSION_ABSOLUTE_TIMEOUT_MS, now)
    if (expired.expired) throw { sessionExpired: true, reason: expired.reason }
    ...
    try {
      ...
      console.warn('[Auth] Session activity write failed (fail-open):', writeErr.message)
    }
    ...
    if (err && err.code === '42703') {
      console.warn('[Auth] user_sessions activity columns missing — expiry check disabled until migration runs')
    } else {
      console.warn('[Auth] Session expiry lookup failed (fail-open):', err && err.message)
    }
    idleActivityCache.set(key, { activityTs: now, createdAt: now, lastOp: now })
  }
}
...
export const protect = async (req, res, next) => {
  ...
  try {
    ...
    if (decoded.type && !['session', 'web', 'phone', undefined, null].includes(decoded.type)) {
      return res.status(401).json({
        success: false,
      ...
      if (session !== null) {
        ...
        try {
          ...
          if (sessionCheck.rows.length === 0) {
            ...
            console.warn('[Auth] Session row missing during validation; failing open (JWT is valid)')
          ...
          if (sessionErr.code === '42P01') {
            console.warn('[Auth] user_sessions table missing, skipping session check')
            await setCachedSession(decoded.sessionId, { isActive: true })
          } else if (isTransientDbError(sessionErr)) {
            console.warn('[Auth] Session validation temporarily unavailable:', sessionErr.message)
            ...
            console.error('[Auth] Session validation query failed (fatal):', sessionErr.message)
            throw sessionErr
          }
        }
      }
    ...
    if (!user) {
      try {
        ...
        if (isTransientDbError(dbErr)) {
          ...
          console.warn('[Auth] User lookup temporarily unavailable during protect check:', dbErr.message)
          return res.status(503).json({
            success: false,
            code: 'SERVICE_UNAVAILABLE',
            message: 'Service temporarily unavailable. Please try again.',
    ...
    try {
      ...
      if (expiryErr && expiryErr.sessionExpired) {
        return res.status(401).json({
          success: false,
          code: expiryErr.reason === 'absolute' ? 'SESSION_EXPIRED' : 'SESSION_IDLE_TIMEOUT',
          message: expiryErr.reason === 'absolute'
    ...
    if (isTransientDbError(error)) {
      console.warn('[Auth] Protect check temporarily unavailable; returning 503:', error.message)
      return res.status(503).json({
        success: false,
        code: 'SERVICE_UNAVAILABLE',
        message: 'Service temporarily unavailable. Please try again.',
...
export const requireImageAuth = async (req, res, next) => {
  ...
  try {
    ...
    if (!token) {
      return res.status(401).json({ success: false, message: 'Authentication required to view this asset' })
    }
    ...
    if (decoded.sessionId) {
      ...
      if (cached !== null) {
        ...
        try {
          ...
          if (sessionErr.code !== '42P01') {
            console.warn('[ImageAuth] session check failed (fail-open):', sessionErr.message)
          }
        }
      }
    }
...
export const optionalAuth = async (req, res, next) => {
  ...
  try {
    ...
    if (token && process.env.JWT_SECRET) {
      ...
      const user = await getCachedUser(decoded.id) || await dbHelpers.findById('users', decoded.id)
      const isVerified = user ? (user.isEmailVerified ?? user.emailVerified ?? false) : false
      if (user && user.isActive !== false && (isVerified === true || process.env.NODE_ENV !== 'production')) {
        const { password, ...userWithoutPassword } = user
        req.user = {
          ...userWithoutPassword,
...
export const superAdmin = (req, res, next) => {
  if (req.user && (req.user.role === ROLES.SUPER_ADMIN || req.user.role === ROLES.ADMIN || req.user.isAdmin)) {
    next()
  } else {
    res.status(403).json({
      success: false,
