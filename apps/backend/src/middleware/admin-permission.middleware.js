...
const resourceForPath = (path = '') => {
  ...
  if (['users', 'enrollments', 'sessions', 'roles', 'permissions'].includes(resource)) return 'users'
  if (['tests', 'test-series', 'questions', 'quizzes', 'sections', 'stages', 'exam-categories', 'exam-info', 'exams'].includes(resource)) return 'tests'
  if (['settings', 'system-settings', 'system', 'backups', 'navigation', 'two-factor'].includes(resource)) return 'settings'
  if (['payments', 'subscription-plans', 'plans', 'coupons', 'promotions'].includes(resource)) return 'monetization'
  if (['banners', 'faqs', 'notifications', 'email-templates'].includes(resource)) return 'communications'
  if (['moderation'].includes(resource)) return 'moderation'
  if (['audit', 'audit-trail', 'results'].includes(resource)) return 'audit'
  if (['analytics', 'deep-analytics', 'leaderboards'].includes(resource)) return 'analytics'
  return 'content'
}
...
export const loadAdminPermissions = async (req, res, next) => {
  ...
  try {
    ...
    console.error('[RBAC] Error loading permissions for user', req.user?.id, ':', error.message)
    // FAIL-CLOSED: on permission load error, deny access to prevent potential security bypass
    return res.status(403).json({
      success: false,
      message: 'Access denied: Unable to verify permissions due to a system error',
...
export const requireAdminPermission = (req, res, next) => {
  ...
  if (permissions.includes('*') || req.user?.role === 'super_admin') return next()
  ...
  // Direct match or resource wildcard
  if (permissions.includes(required) || permissions.includes(`${resource}:*`)) return next()

  // Action aliases: view <-> read, create/edit <-> write
  const aliasActions = action === 'view' ? ['read'] : action === 'read' ? ['view'] : (action === 'create' || action === 'edit') ? ['write'] : action === 'write' ? ['create', 'edit'] : []
  if (aliasActions.some((act) => permissions.includes(`${resource}:${act}`))) return next()

  // Content fallback for general content resources
  if (resource === 'content' && (permissions.includes('content:read') || permissions.includes('content:view'))) {
    return next()
  }

  console.error('[RBAC] DENIED:', required, '- path:', req.path, '- method:', req.method, '- permissions:', permissions, '- user:', req.user?.id)
  return res.status(403).json({
    success: false,
