...
export const EXAM_DISTRIBUTION_PRIORS = {
  ...
  railway: { meanRatio: 0.60, stdRatio: 0.15, nationalApplicants: 5000000 },
...
export const getExamPrior = (categoryOrSlug = '') => {
  ...
  if (clean.includes('ntpc') || clean.includes('rrb') || clean.includes('railway')) return EXAM_DISTRIBUTION_PRIORS['rrb-ntpc']
  if (clean.includes('ibps') || clean.includes('sbi') || clean.includes('bank')) return EXAM_DISTRIBUTION_PRIORS.banking
  if (clean.includes('upsc') || clean.includes('civil') || clean.includes('ias')) return EXAM_DISTRIBUTION_PRIORS.upsc
  if (clean.includes('gate')) return EXAM_DISTRIBUTION_PRIORS.gate
  if (clean.includes('nda') || clean.includes('cds') || clean.includes('defense') || clean.includes('defence')) return EXAM_DISTRIBUTION_PRIORS.defense
...
export const calculateCalibratedPercentile = ({
  ...
  if (n < 5) {
    const predictedAIR = Math.max(1, Math.round(((100 - parametricPercentile) / 100) * prior.nationalApplicants))
    return {
  ...
  const validScores = liveCohortScores.map((s) => safeNumber(s, 0)).sort((a, b) => a - b)
  const strictlyBelow = validScores.filter((s) => s < numericScore).length
  const equalTo = validScores.filter((s) => s === numericScore).length
  const empiricalPercentile = Math.max(0.01, Math.min(99.99, ((strictlyBelow + 0.5 * equalTo) / n) * 100))
  ...
  const blendedPercentile = weight * empiricalPercentile + (1 - weight) * parametricPercentile
  const finalPercentile = Math.max(0.01, Math.min(99.99, Number(blendedPercentile.toFixed(2))))
  const predictedAIR = Math.max(1, Math.round(((100 - finalPercentile) / 100) * prior.nationalApplicants))
...
export const predictRankForScore = async ({ testId, score, totalStudents = null, examCategory = null }) => {
  ...
  // Try to resolve test total marks and category if test exists
  let totalMarks = 200
  ...
  try {
    ...
    if (test) {
      ...
      category = category || test.category || test.seriesSlug || test.examType || 'default'
    }
  } catch {
    // fallback defaults
  }
  ...
  const mean = scores.length > 0 ? scores.reduce((sum, value) => sum + value, 0) / scores.length : totalMarks * 0.55
  ...
  return {
    ...
    cohortRank: Math.max(1, Math.round(((100 - calibrated.percentile) / 100) * participants)),
    expectedCutoff: Number(cutoff.toFixed(2)),
    isCalibrated: calibrated.isCalibrated,
    distribution: {
      mean: Number(mean.toFixed(2)),
      stdDev: Number(spread.toFixed(2)),
      p25: Number((percentileValue(scores, 25) || totalMarks * 0.42).toFixed(2)),
      p50: Number((percentileValue(scores, 50) || totalMarks * 0.55).toFixed(2)),
      p75: Number((percentileValue(scores, 75) || totalMarks * 0.70).toFixed(2)),
      p90: Number((percentileValue(scores, 90) || totalMarks * 0.82).toFixed(2)),
    },
  }
}
