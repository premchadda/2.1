...
export const getRecommendationsForUser = async (userId, { limit = 6 } = {}) => {
    ...
    attempts
      .filter(
        (attempt) =>
          ...
          String(attempt.status || "").toLowerCase() === "completed",
      )
  ...
  const allTopics = await dbHelpers
    .find("topics", { isActive: true })
    .catch(() => []);
...
export const refreshRecommendationsFromEvent = async ({ userId, testId }) => {
  if (!userId) {
    ...
    // Fetch only distinct user IDs who attempted this specific test — no full-table scan.
    const { rows } = await pool.query(
      `SELECT DISTINCT user_id FROM test_attempts WHERE test_id = $1`,
      [testId],
    );
