...
const getBestAttemptByUser = (attempts) => {
  ...
  attempts.forEach((attempt) => {
    ...
    if (scoreDiff === 0 && safeNumber(attempt.timeSpent, Number.MAX_SAFE_INTEGER) < safeNumber(existing.timeSpent, Number.MAX_SAFE_INTEGER)) {
      bestByUser.set(String(userId), attempt)
    }
  })
...
const rankEntries = (entries) => {
  const sorted = [...entries].sort((left, right) => {
    ...
    return safeNumber(left.timeSpentSeconds, Number.MAX_SAFE_INTEGER) - safeNumber(right.timeSpentSeconds, Number.MAX_SAFE_INTEGER)
  })
...
const attemptTimeSpent = (attempt) => safeNumber(attempt.timeSpent ?? attempt.timeTaken ?? attempt.timeSpentSeconds)

const attemptSubmittedAt = (attempt) => new Date(attempt.submittedAt || attempt.updatedAt || attempt.createdAt || 0).getTime()
...
const aggregateAttemptsByUser = (attempts) => {
  ...
  return Array.from(aggregateByUser.values()).map((entry) => ({
    ...
    accuracy: entry.entries > 0 ? Number((entry.accuracyTotal / entry.entries).toFixed(2)) : 0,
    timeSpentSeconds: entry.timeSpentSeconds,
  }))
}

const buildAttemptLeaderboard = ({ attempts, type, scopeKey, batchDate, page, limit, sortBy = null }) => {
  const ranked = rankEntries(aggregateAttemptsByUser(attempts))
  const sorted =
    sortBy === 'time'
      ? [...ranked].sort((left, right) => safeNumber(left.timeSpentSeconds, Number.MAX_SAFE_INTEGER) - safeNumber(right.timeSpentSeconds, Number.MAX_SAFE_INTEGER))
      : sortBy === 'accuracy'
        ? [...ranked].sort((left, right) => safeNumber(right.accuracy) - safeNumber(left.accuracy) || safeNumber(right.score) - safeNumber(left.score))
        : ranked

  const rankedForDisplay = sorted.map((entry, index) => ({ ...entry, rank: index + 1 }))
...
const upsertLeaderboardEntries = async (entries, { type, scopeKey, batchDate, testId = null }) => {
  ...
  for (const entry of entries) {
    await pool.query(
      ...
      DO UPDATE SET
        ...
        updated_at = NOW()
      `,
      [testId, entry.userId, entry.score, entry.accuracy, entry.timeSpentSeconds, entry.rank, entry.percentile, batchDate]
    )
  }
}
...
export const recalculateDailyLeaderboard = async (date = new Date()) => {
  ...
  const dailyAttempts = completed.filter((attempt) => {
    const submitted = new Date(attempt.submittedAt || attempt.updatedAt || attempt.createdAt || 0).getTime()
  ...
  dailyAttempts.forEach((attempt) => {
    const userId = String(attempt.userId)
    const aggregate = aggregateByUser.get(userId) || { userId, score: 0, accuracyTotal: 0, entries: 0, timeSpentSeconds: 0 }
    aggregate.score += safeNumber(attempt.score)
    aggregate.accuracyTotal += safeNumber(attempt.accuracy)
    aggregate.entries += 1
...
export const getLeaderboard = async ({ type = 'test', testId = null, seriesId = null, sortBy = null, page = 1, limit = 50, date = null } = {}) => {
  const normalizedType = ['overall', 'test', 'series', 'daily', 'weekly'].includes(type) ? type : 'overall'
  ...
  if (normalizedType === 'overall' || normalizedType === 'series') {
    ...
    return withUserNames(buildAttemptLeaderboard({
      attempts: filtered,
      type: normalizedType,
      scopeKey: normalizedType === 'series' ? `series:${seriesId}` : 'overall',
      batchDate,
      page: normalizedPage,
  ...
  if (normalizedType === 'daily' || normalizedType === 'weekly') {
    ...
    return withUserNames(buildAttemptLeaderboard({
      attempts: filtered,
      type: normalizedType,
      scopeKey: normalizedType === 'daily' ? `daily:${batchDate}` : `weekly:${toDateString(range.monday)}`,
      batchDate,
      page: normalizedPage,
      limit: normalizedLimit,
      sortBy,
