...
export class TestAttemptController {
  ...
  async submitAttempt(attemptId, userId, finalData = {}) {
    ...
    try {
      ...
      const lockRes = await client.query('SELECT * FROM attempts WHERE id = $1 AND user_id = $2 FOR UPDATE', [attemptId, userId])
      ...
      if (inactiveSeconds > MAX_INACTIVE_SECONDS) {
        await client.query(`UPDATE attempts SET status = 'paused' WHERE id = $1`, [attemptId])
        throw new Error('Session expired due to inactivity')
      }
  ...
  async getAnalytics(attemptId) {
    const attempt = await this.db.query('SELECT id, user_id, test_id, series_id, status, score, total_marks, time_taken, is_completed, is_reattempt, is_active, started_at, submitted_at, completed_at, last_activity, last_question_id, marked_for_review, question_results, solutions, section_scores, section_times, section_timers, percentile, rank, attempted, incorrect, skipped, created_at, updated_at FROM attempts WHERE id = $1', [attemptId])
