...
export class NodeEngineService {
  ...
  // 2. Compute time-decay (staleness 0.0 to 1.0) with an exponential
  // forgetting curve. tau (days) grows with ease_factor, so well-learned
  // cards decay slower. When next_review_date is in the future the item is
  // not yet due → staleness is scaled down by remaining time.
  getTimeDecay(lastAttemptedAt, nextReviewDate = null, easeFactor = 2.5) {
    if (!lastAttemptedAt) return 1.0; // Never attempted = maximum freshness needed
    const daysSince =
      (Date.now() - new Date(lastAttemptedAt).getTime()) /
      (1000 * 60 * 60 * 24);
    const ease = Number(easeFactor) > 0 ? Number(easeFactor) : 2.5;
    // tau ≈ 6 days at ease 2.5, scaling linearly with ease factor.
    const tau = Math.max(1, (ease / 2.5) * 6);
    let staleness = 1 - Math.exp(-Math.max(0, daysSince) / tau);
    if (nextReviewDate) {
      const daysToReview =
        (new Date(nextReviewDate).getTime() - Date.now()) /
        (1000 * 60 * 60 * 24);
      if (daysToReview > 0) {
        // Not due yet: damp staleness proportionally to time remaining.
        staleness *= Math.max(0, 1 - daysToReview / (tau * 2));
      }
    }
    return Math.min(1.0, Math.max(0.0, Number(staleness.toFixed(4))));
  }

  // 3. Compute AI recommendation score for a node & student
  getRecommendationScore(node, userSkill) {
    const aiMeta =
      typeof node.ai_meta === "string"
        ? JSON.parse(node.ai_meta || "{}")
        : node.ai_meta || {};
    const mastery = userSkill?.mastery_score ?? 0.0;
    const difficulty = aiMeta.difficulty_score ?? 0.5;
    const freshness = this.getTimeDecay(
      userSkill?.last_attempted_at,
      userSkill?.next_review_date,
      userSkill?.ease_factor,
    );
  ...
  shouldRevise(userSkill) {
    ...
    if (
      userSkill.next_review_date &&
      new Date(userSkill.next_review_date).getTime() > Date.now()
    ) {
      return false;
    }
    const daysSince =
      (Date.now() - new Date(userSkill.last_attempted_at).getTime()) /
      (1000 * 60 * 60 * 24);
  ...
  async generateLearningPath(userId, rootNodeId = null) {
    ...
    const skillsRes = await this.db.query(
      `SELECT * FROM user_node_skill WHERE user_id = $1`,
      [userId],
    );
    ...
    const path = nodesRes.rows.map((node) => {
      ...
      return {
        ...
        status:
          (skill?.mastery_score || 0) >= 0.8
            ? "mastered"
            : (skill?.attempt_count || 0) > 0
              ? "in_progress"
              : "locked",
      };
    });
  ...
  async recordAttempt(userId, nodeId, isCorrect, timeSpentSeconds = 45) {
    // nodeId MUST be a positive integer id of a `nodes` row (the learning
    // graph). Callers historically passed topic ids, question ids, or even
    // topic-name strings — each creates garbage user_node_skill rows keyed
    // to ids that never join back. Reject non-ids up front.
    const numericNodeId = Number(nodeId);
    if (!Number.isInteger(numericNodeId) || numericNodeId <= 0) {
      logger.warn(
        `[NodeEngine] recordAttempt rejected non-numeric nodeId: ${String(nodeId).slice(0, 50)}`,
      );
      return { success: false, skipped: true, reason: "invalid_node_id" };
    }
    ...
    try {
      ...
      // Verify the node EXISTS before writing anything. user_node_skill has
      // a hard FK to nodes(id), and callers pass topic ids from questions —
      // a topic id with no matching nodes row would otherwise FK-fail (23503)
      // every call, or worse, collide with an unrelated node's id. Skipping
      // unknown ids keeps the engine quiet instead of error-spamming.
      const nodeRes = await client.query(
        `SELECT ai_meta FROM nodes WHERE id = $1 FOR UPDATE`,
        [nodeId],
      );
      if (nodeRes.rows.length === 0) {
        await client.query("COMMIT");
        logger.debug?.(
          `[NodeEngine] recordAttempt skipped: node ${nodeId} not in learning graph`,
        );
        return { success: false, skipped: true, reason: "node_not_found" };
      }
      ...
      // Update node difficulty signal in ai_meta (nodeRes fetched above)
      {
        const aiMeta =
          typeof nodeRes.rows[0].ai_meta === "string"
        ...
        await client.query(
          `UPDATE nodes SET ai_meta = $1, updated_at = NOW() WHERE id = $2`,
          [JSON.stringify(updatedMeta), nodeId],
        );
      }

      await client.query("COMMIT");
      logger.info(
        `[NodeEngine] Updated skill & node difficulty for user ${userId}, node ${nodeId} (mastery: ${newMastery})`,
      );
