...
const getFrontendUrl = () => {
  ...
  return (
    process.env.FRONTEND_URL || `${isHttps ? "https" : "http"}://localhost:3000`
  );
};
...
class EmailService {
  ...
  setupProvider() {
    if (isDisabledProvider(this.provider)) {
      if (process.env.NODE_ENV === "production") {
        logger.warn(
          "[Email] Transactional email provider is disabled (EMAIL_PROVIDER=none) in production. Set EMAIL_PROVIDER to sendgrid, aws, or smtp when ready.",
        );
      } else {
      ...
      case "sendgrid":
        try {
          ...
          if (!apiKey || !apiKey.startsWith("SG.")) {
            logger.warn(
              "SendGrid API key not configured or invalid. Email sending disabled.",
            );
            this.sgMail = null;
            return;
          }
  ...
  async sendWelcomeEmail(email, userName) {
    ...
    const htmlContent = this.getHtmlWrapper(
      "Welcome to Trstprep",
      bodyContent,
      actionButton,
    );
  ...
  async send(email, subject, htmlContent) {
    ...
    try {
      const { isQueueEnabled, addJob, QUEUE_NAMES } =
        await import("../infrastructure/queue/queueManager.js").catch(
          () => ({}),
        );
      if (isQueueEnabled && isQueueEnabled() && !isWorker) {
        try {
          ...
          logger.info(
            `[Email] Enqueued email job to ${email} for background processing`,
          );
          return { success: true, queued: true };
        } catch (queueErr) {
          logger.warn(
            "Failed to queue email job, falling back to direct delivery:",
            queueErr.message,
          );
        }
      }
  ...
  /**
   * Queue email accepting mailOptions format
   */
  async queueEmail(mailOptions) {
    if (!mailOptions) return { success: false, error: "Invalid mail options" };
    return this.send(
      mailOptions.to,
      mailOptions.subject,
