...
class MessageBroker {
  ...
  async init() {
    ...
    if (!isRedisReady()) {
      logger.warn('[MessageBroker] Redis not ready. Running in local in-memory mode.')
      return
    }
    ...
    try {
      ...
      logger.info(`[MessageBroker] Decoupled pub/sub initialized on channel "${this.channel}"`)
    } catch (err) {
      logger.error('[MessageBroker] Initialization failed, falling back to local mode:', err.message)
    }
  }
  ...
  async publish(eventName, payload = {}) {
    ...
    if (this.initialized && this.publisherClient) {
      try {
        await this.publisherClient.publish(this.channel, JSON.stringify(envelope))
      } catch (err) {
        logger.error(`[MessageBroker] Failed to publish event "${eventName}" to Redis:`, err.message)
      }
    }
  }
  ...
  async enqueue(eventName, payload = {}) {
    ...
    try {
      ...
      logger.error(`[MessageBroker] Queue failure for event "${eventName}", falling back to publish:`, err.message)
  ...
  handleInboundMessage(messageStr) {
    try {
      ...
      // Prevent double trigger of local listeners if publisher client triggered them locally
      this.triggerLocalSubscribers(envelope.name, envelope.payload, true)
    } catch (err) {
      logger.error('[MessageBroker] Failed to parse inbound event message:', err.message)
    }
  }
  ...
  async triggerLocalSubscribers(eventName, payload, isExternalSource = false) {
    ...
    const promises = handlers.map(async (handler) => {
      try {
        ...
        logger.error(`[MessageBroker] Error in subscriber for event "${eventName}":`, err)
      }
    })
