...
export const startWorkers = (handlersByQueue = {}, concurrencyByQueue = {}) => {
  ...
  for (const queueName of queueNames) {
    ...
    worker.on('completed', (job) => {
      logger.info(`[Worker][${queueName}] Completed job ${job.id} (${job.name})`)
    })
    ...
    worker.on('failed', async (job, error) => {
      ...
      logger.error(`[Worker][${queueName}] Job ${jobId} failed:`, error.message)

      if (attemptsMade >= maxAttempts) {
        logger.warn(`[Worker][${queueName}] Job ${jobId} exhausted ${maxAttempts} attempts — moving to dead letter queue`)
        try {
          ...
          logger.error(`[Worker][${queueName}] Failed to write to DLQ:`, dlqError.message)
        }
      }
    })
...
export const retryDeadLetterJob = async (queueName, jobId) => {
  ...
  try {
    ...
    await targetQueue.add(job.name, job.data, { attempts: DEFAULT_JOB_OPTIONS.attempts })
    await job.remove()
    return true
  } finally {
    await dlq.close()
