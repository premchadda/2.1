...
const MAX_RETRIES = 3;
const SPOOL_DIR =
  process.env.EMAIL_SPOOL_DIR || path.join(process.cwd(), ".email-spool");
...
const appendToSpool = (job) => {
  ensureSpool();
  return runSpoolOp(() =>
    fs.promises.appendFile(SPOOL_FILE, JSON.stringify(job) + "\n"),
  );
};
...
const replaySpool = () => {
  try {
    ...
    if (emailQueue.length) {
      console.log(
        `[Email] Replaying ${emailQueue.length} persisted mail job(s) from spool`,
      );
    }
  } catch (e) {
...
const createTransporter = () => {
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "SMTP credentials not configured. Cannot send email in production. Set SMTP_HOST, SMTP_USER, SMTP_PASS.",
    );
  }
  ...
  while (emailQueue.length > 0) {
    ...
    try {
      ...
      if (t) {
        ...
        console.log(
          `[Email] Sent to ${emailJob.to} (subject: ${emailJob.subject})`,
        );
      } else {
        console.log(
          `[Email] (Dev) To: ${emailJob.to} | Subject: ${emailJob.subject}`,
        );
      }
      ...
      if ((emailJob._retries || 0) < MAX_RETRIES) {
        ...
        console.log(
          `[Email] Re-queued (attempt ${emailJob._retries}/${MAX_RETRIES})`,
        );
      } else {
        console.error(
          `[Email] Dropped after ${MAX_RETRIES} retries: ${emailJob.to}`,
        );
        if (emailJob._id) removeFromSpool(emailJob._id);
      }
    }
...
/**
 * Helper: check if transactional email is allowed by admin notification settings.
 * Kept best-effort (fail open) so a DB outage never blocks auth flows.
 */
async function isEmailAllowedFor(type) {
  try {
    const { isNotificationEnabled } =
      await import("../../services/SettingsService.js");
    if (type === "verification" || type === "welcome") {
      return isNotificationEnabled("emailOnRegistration");
    }
    if (type === "payment") return isNotificationEnabled("emailOnPayment");
    return true;
  } catch {
    return true;
  }
...
export const sendVerificationEmail = async (to, name, token) => {
  if (!(await isEmailAllowedFor("verification"))) {
    console.log(
      `[Email] Skipped verification email to ${to} (emailOnRegistration disabled)`,
    );
    return {
      success: false,
      skipped: true,
      reason: "emailOnRegistration disabled",
    };
  }
  ...
  const frontendUrl =
    ...
    `${isHttps ? "https" : "http"}://localhost:3000`;
...
export const sendPasswordResetEmail = async (to, name, token) => {
  ...
  try {
    ...
    console.error(
      "[Email] Failed to queue password reset email:",
      error.message,
    );
    return { success: false, error: error.message };
  }
...
export const sendWelcomeEmail = async (to, name) => {
  if (!(await isEmailAllowedFor("welcome"))) {
    console.log(
      `[Email] Skipped welcome email to ${to} (emailOnRegistration disabled)`,
    );
    return {
      success: false,
      skipped: true,
...
export const drainEmailQueue = async () => {
  ...
  if (emailQueue.length) {
    console.warn(
      `[Email] Shutdown with ${emailQueue.length} unsent mail job(s) still in spool (will replay on next boot)`,
    );
  }
};
